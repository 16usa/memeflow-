...
PATCH='MEMEFLOW_WS_ONLY_PREOPEN_RPC_V1_1'
...
"""
    ...
    x['app']=replace_once(x['app'],'processing:discQueue.processing','processing:0','app/discovery-status-processing')
...
"""
    ...
    for needle in ['createDexPaidVerifier','dexPaidVerifier','__mfDexPaid','makeHolderQueue','enrichHolders','enrichToken','makeDiscoveryQueue','discQueue','processSignature(sig)']:
        if needle in x['app']: raise RuntimeError(f'PREFLIGHT obsolete app path remains: {needle}')
...
run(['git','commit','-m','[MEMEFLOW_WS_ONLY_PREOPEN_RPC_V1_1] Rebase WS-only scanner cleanup on token-age fix'],cwd=root)
run(['git','push','origin','HEAD'],cwd=root)

print()
print('='*76)
print(' MEMEFLOW_WS_ONLY_PREOPEN_RPC_V1_1 INSTALLED SUCCESSFULLY')
print('='*76)
