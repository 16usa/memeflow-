#!/usr/bin/env python3
"""
MEMEFLOW canonical header patch v1.1
Baseline: 283f28e39a40fa4ec9bb2ce4cadd678b56944fdc

Run from the repository root:
    python3 apply_memeflow_header_unify_v1_1.py
"""
...
# Only block when one of the audited SOURCE files that this patch will touch
# has local edits. Runtime data (sqlite WAL/SHM, state.json, etc.) is intentionally
# ignored and is never modified by this patch.
source_paths = [f"memeflow-app/{name}" for name in required]
status_cmd = ["git", "status", "--porcelain", "--untracked-files=no", "--", *source_paths]
dirty_source = sh(*status_cmd).stdout.strip()
if dirty_source:
    die(
        "One or more SOURCE files used by the header patch already have local changes:\n"
        f"{dirty_source}\n\n"
        "Commit/stash only those source edits first. Runtime data changes are allowed."
    )
