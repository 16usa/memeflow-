...
PATCH_NAME="MEMEFLOW_DATA_INTEGRITY_V1_3_2_ADAPTIVE"
...
sig=re.compile(r"export\s+function\s+evaluate\s*\(\s*token(?:\s*=\s*\{\})?\s*,[^)]*\)\s*\{")
ms=list(sig.finditer(e))
if len(ms)!=1:
    candidates=re.findall(r"export\s+function\s+[A-Za-z_$][\w$]*\s*\([^\n]{0,180}",e)
    raise SystemExit(f'evaluate signature: expected 1, found {len(ms)}; exports={candidates[:8]}')
