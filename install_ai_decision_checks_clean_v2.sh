...
AI_V2="$APP/ai-analysis-state-clean-v2.js"

[[ -f "$INDEX" ]] || { echo "ERROR: index.html not found."; exit 1; }
[[ -f "$AI_V1" ]] || { echo "ERROR: ai-analysis-state-clean-v1.js not found."; exit 1; }

if grep -q 'MF_AI_DECISION_CHECKS_CANONICAL_V2' "$INDEX"; then
  echo "AI Decision Checks V2 is already installed."
  exit 0
fi

PATCH_DIR="$APP/.memeflow-patches/ai-decision-checks-v2"
mkdir -p "$PATCH_DIR"
...
AI_WORK="$PATCH_DIR/ai-analysis-state-clean-v2.js.$STAMP.work"
AI_CHECK="$PATCH_DIR/ai-analysis-state-clean-v2.$STAMP.check.js"
...
rollback(){
  ...
  rm -f "$AI_V2" "$INDEX_WORK" "$AI_WORK" "$AI_CHECK"
}
trap 'echo "ERROR: AI Decision Checks V2 failed; restoring exact pre-install files."; rollback' ERR
...
# The audited current production core no longer renders #decisionTree.
# Refuse another runtime writer instead of creating a duplicate owner.
writer_patterns = (
...
writers = [pattern for pattern in writer_patterns if pattern in src]
if writers:
    raise SystemExit(
        "Unexpected inline decisionTree writer: "
        + ", ".join(writers)
        + ". Nothing changed."
    )
...
src = script_ref.sub(
    r'\g<prefix>./ai-analysis-state-clean-v2.js?v=2.0.0\g<suffix>',
    src,
    count=1
)
...
new_guard = (
    "if (window.__MEMEFLOW_AI_ANALYSIS_STATE_V2__) return;\n"
    "  window.__MEMEFLOW_AI_ANALYSIS_STATE_V2__ = true;"
)
...
required = (
    "window.__MEMEFLOW_AI_ANALYSIS_STATE_V2__",
    "const AI_DECISION_CHECKS_V2 = [",
...
echo
echo "OK: AI DECISION CHECKS V2 installed cleanly."
echo
echo "Decision checks owner: ai-analysis-state-clean-v2.js ONLY"
