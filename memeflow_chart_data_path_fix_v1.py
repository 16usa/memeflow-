...
TAG = "MEMEFLOW_CHART_DATA_PATH_FIX_V1"
...
def main():
    ...
    # Refuse to layer over an uncommitted app-server edit, but do not care about
    # runtime data or other UI files.
    if (repo / ".git").exists():
        ...
        if dirty or staged:
            ...
            raise RuntimeError("app-server.mjs already has uncommitted edits; commit/stash them before this repair")
    ...
    if TAG in original:
        ...
        backup = app / ".patch-backups" / f"chart-data-path-fix-v1-{stamp}"
  ...
  return __mfOpenChartStream(req,res,url.searchParams.get('tokenAddress'));
    ...
    # Commit only this source file; do not stage runtime state or unrelated UI work.
    if (repo / ".git").exists():
        rel = str(server.relative_to(repo))
        run("git", "add", "--", rel, cwd=repo)
        if run("git", "diff", "--cached", "--quiet", "--", rel, cwd=repo, check=False).returncode != 0:
            run("git", "commit", "-m", "fix: restore Trading chart history and SSE data path", "--", rel, cwd=repo)
            branch = run("git", "branch", "--show-current", cwd=repo).stdout.strip()
            if branch:
                push = run("git", "push", "origin", branch, cwd=repo, check=False)
                if push.returncode != 0:
                    log("source is fixed and committed locally; git push failed (see output above)")
        else:
            log("no new git diff to commit")
