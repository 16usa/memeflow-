...
echo "[MEMEFLOW] Mayhem hard block + 1-second mutable cards v17.3-compat"
...
cleanup() {
  ...
  if [[ $code -ne 0 ]]; then
    echo
    echo "[FAILED] v17.3 made no commit/push."
...
echo "[check] compatibility markers before regressions"
grep -q 'MEMEFLOW_LIVE_TOKEN_TELEMETRY_V9' system-tokens.js
grep -q 'MEMEFLOW_SCANNER_STATUS_V9' system-tokens.js
grep -q 'MEMEFLOW_LIVE_TOKEN_FEED_DIAGNOSTICS_V13' system-tokens.js
grep -q 'MEMEFLOW_SYSTEM_TOKENS_ONE_SECOND_V17' system-tokens.js
...
if git merge-base --is-ancestor "$LOCAL_HEAD" "$NEW_SHA" 2>/dev/null; then
  ...
  if git merge --ff-only "$NEW_SHA"; then
    echo "[local] workspace fast-forwarded to verified v17.3.1"
  else
