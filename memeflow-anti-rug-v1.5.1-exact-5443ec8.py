...
def patch_html(text: str) -> str:
    ...
    # Current MEMEFLOW builds have changed label text / attribute ordering several
    # times. Anchor on stable control IDs instead of one byte-for-byte HTML string.
    dev_inputs = list(re.finditer(
    ...
    if len(dev_inputs) != 1:
        raise RuntimeError(
            f"Settings HTML maxDeveloper input count={len(dev_inputs)}"
        )

    dev_input = dev_inputs[0]
    prefix = text[:dev_input.start()]
    field_starts = list(re.finditer(
        r"<div\b[^>]*\bclass=['\"][^'\"]*\bsetting-field\b[^'\"]*['\"][^>]*>",
        prefix,
        flags=re.I,
    ))
    if not field_starts:
        raise RuntimeError("Settings HTML could not locate maxDeveloper setting-field wrapper")
    field_end = text.find('</div>', dev_input.end())
    if field_end < 0:
        raise RuntimeError("Settings HTML maxDeveloper setting-field has no closing </div>")
    field_end += len('</div>')
    ...
    text = text[:field_end] + extra_fields + text[field_end:]

    # Insert boolean gates immediately before the existing requireSocial toggle.
    social_inputs = list(re.finditer(
        r"<input\b[^>]*\bid=['\"]requireSocial['\"][^>]*>",
        text,
        flags=re.I,
    ))
    if len(social_inputs) != 1:
        raise RuntimeError(
            f"Settings HTML requireSocial input count={len(social_inputs)}"
        )
    social_input = social_inputs[0]
    social_prefix = text[:social_input.start()]
    toggle_starts = list(re.finditer(
        r"<div\b[^>]*\bclass=['\"][^'\"]*\btoggle-row\b[^'\"]*['\"][^>]*>",
        social_prefix,
        flags=re.I,
    ))
    if not toggle_starts:
        raise RuntimeError("Settings HTML could not locate requireSocial toggle-row wrapper")
    toggle_start = toggle_starts[-1].start()
    ...
    text = text[:toggle_start] + toggles + text[toggle_start:]

    # Locate the actual Settings ids array. Allow multiline arrays / quote style.
    arrays = []
    for match in re.finditer(
        ...
        if re.search(r"['\"]maxDeveloper['\"]", body) and re.search(r"['\"]requireSocial['\"]", body):
            arrays.append(match)
    if len(arrays) != 1:
        raise RuntimeError(f"Settings HTML settings ids array count={len(arrays)}")
    ...
    quote = token.group('q')
    had_comma = token.group('comma') is not None
    values = [
        'maxDeveloper',
        'maxSuspectedRiskyWalletsPct',
        'maxInsidersPct',
    ...
    replacement = ','.join(f'{quote}{value}{quote}' for value in values)
    if had_comma:
        replacement += ','
    ...
    for key in NEW_KEYS:
        if len(re.findall(rf"id=['\"]{re.escape(key)}['\"]", text)) != 1:
            raise RuntimeError(f"Settings HTML post-check failed for control id {key}")
