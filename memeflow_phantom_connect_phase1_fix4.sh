...
echo "== MEMEFLOW Phantom Connect Phase 1 — FIX 4 =="
...
BACKUP=".phantom-connect-phase1-fix4-backup-$STAMP"
mkdir -p "$BACKUP"

for f in settings.html phantom-connect-client.mjs phantom-connect-client.bundle.js phantom-connect.css; do
  [[ -f "$f" ]] && cp -p "$f" "$BACKUP/$f"
done
...
# Bump only the direct-load URL so Safari cannot reuse FIX 3.
python3 - <<'PY_BUMP'
...
s=s.replace(
    ...
    "/phantom-connect-client.bundle.js?v=phase1-fix4-20260827"
)

s=s.replace(
    "/phantom-connect.css?v=phase1-fix3-20260827",
    "/phantom-connect.css?v=phase1-fix4-20260827"
)

p.write_text(s)
PY_BUMP

echo "Building FIX 4 browser bundle..."
...
echo
echo "== PHANTOM CONNECT PHASE 1 FIX 4 INSTALLED =="
echo
echo "FIX 4 specifically fixes:"
echo "  Phantom returned no Solana address"
echo
echo "It now:"
echo "  - uses BrowserSDK provider=injected even without PHANTOM_APP_ID"
echo "  - accepts Phantom BrowserSDK addresses[] response"
echo "  - falls back to sdk.getAddresses() / sdk.solana.getPublicKey()"
echo "  - falls back again to the raw injected Phantom provider"
echo "  - keeps the mobile Open-in-Phantom flow"
echo "  - keeps embedded/deeplink login locked until PHANTOM_APP_ID"
echo
echo "STOP -> RUN -> reload System Settings."
echo "Then press Use Phantom wallet again."
