...
NEWTEST="memeflow-app/tests/canonical-live-score-pipeline-v20_8.mjs"

for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST"; do
  [[ -f "$f" ]] || { echo "ERROR missing $f"; exit 1; }
done

[[ "$(git branch --show-current)" == "$BRANCH" ]] || { echo "V20.8 REFUSED: wrong branch"; exit 1; }
[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V20.8 REFUSED: audited HEAD mismatch";
...
for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST"; do
  git diff --quiet -- "$f" || { echo "V20.8 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V20.8 REFUSED: staged changes in $f"; exit 1; }
done

echo "=== V20.8 AUDITED PRECHECK ==="
python3 - <<'PY'
...
BACKUP=".memeflow-backups/canonical-live-score-v20_8-$STAMP"
...
r=rep(r,"""      const liveCandidate = isLiveCandidateState(view.state);
      ...
      return {
        ...view,
        decisionScore,
        // MEMEFLOW_CANONICAL_VISIBLE_AI_SCORE_V20_8
        score: decisionScore,
        feedScore: relevanceScore,
        relevanceScore,
        statePriority: statePriority(view.state)
...
new="""// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8
...
route=route[:vs]+"""  // MEMEFLOW_UNIFIED_FULL_LIVE_VIEW_V20_8
...
const __mfLiveStatesResponseCache=new Map();""","""// MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8
...
u=rep(u,"""const __MF_TOKEN_IDENTITY_KEYS_V20_2=[
  ...
  // MEMEFLOW_STATIC_IDENTITY_ONLY_V20_8
  'name','metadataName','symbol','metadataSymbol',
  'image','imageUrl','imageUri','logo','logoUrl','logoURI'
];""",'identity keys')
...
u=rep(u,"""    const incomingByMint=new Map(
    ...
    // MEMEFLOW_VISIBLE_INVALIDATION_SCOPE_V20_8
...
u=rep(u,"""        const scoreA =
        ...
        // MEMEFLOW_SCORE_FIRST_TIEBREAK_V22""","""        // MEMEFLOW_SMART_HIDDEN_FEED_RANK_V20_8
...
h=HTML.read_text(); h,n=re.subn(r'src="/system-tokens\.js\?v=[^"]+"','src="/system-tokens.js?v=canonical-live-score-v20-8-20260903"',h,count=1)
if n!=1: raise SystemExit('asset cache-bust anchor missing')
HTML.write_text(h)
print('V20_8_TRANSFORM_OK')
PY
...
assert.match(rankSource,/MEMEFLOW_CANONICAL_VISIBLE_AI_SCORE_V20_8/);
assert.doesNotMatch(rankSource,/score:\s*liveCandidate\s*\?/);

const ds=app.indexOf('// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8');
...
const fs=app.indexOf("if(url.pathname==='/api/system/live-token-states'");
const fe=app.indexOf("if(url.pathname==='/api/ai/decisions')",fs);
const full=app.slice(fs,fe);
assert.match(full,/__mfLiveDecisionForUserV14\(/);
assert.match(full,/__mfLiveCardViewV14\(/);
assert.match(full,/MEMEFLOW_UNIFIED_FULL_LIVE_VIEW_V20_8/);
...
assert.match(ui,/MEMEFLOW_VISIBLE_INVALIDATION_SCOPE_V20_8/);
assert.match(ui,/if\(!requestedMints\.has\(mint\)\)return previous;/);
assert.match(ui,/MEMEFLOW_SMART_HIDDEN_FEED_RANK_V20_8/);
assert.match(html,/system-tokens\.js\?v=canonical-live-score-v20-8-20260903/);

console.log('CANONICAL_LIVE_SCORE_PIPELINE_V20_8_OK');
TESTJS
...
echo "=== V20.8 TARGETED TESTS ==="
(
 cd memeflow-app
 node tests/canonical-live-score-pipeline-v20_8.mjs
...
BAD="$(git diff --cached --name-only | grep -Ev '^memeflow-app/(app-server\.mjs|system-tokens\.js|system-tokens\.html|src/feed-ranking\.mjs|tests/feed-ranking\.mjs|tests/live-truth-no-dynamic-cache-v20_3\.mjs|tests/canonical-live-score-pipeline-v20_8\.mjs)$' || true)"
