/* MEMEFLOW AI Direct Evaluator Sheet v20.0
...
(() => {
  ...
  if (window.__MEMEFLOW_AI_DIRECT_V20__) return;
  window.__MEMEFLOW_AI_DIRECT_V20__ = true;

  const OPEN_BTN_ID = 'mfManualAiButton';
  const SHEET_ID = 'sheet-ai-direct-v20';
  const STYLE_ID = 'mf-ai-direct-v20-style';
  const MORE_PROXY_ID = 'mf-mobile-more-proxy';
  const AI_NAV_ID = 'mf-ai-center-nav-v20';
  const HEADER_WALLET_ID = 'mf-header-wallet-v20';
...
@media(max-width:820px){
  ...
  /* V20: the large Open AI assistant CTA stays in DOM for logic, but is not shown on mobile. */
  #${OPEN_BTN_ID}{
  ...
  .mobile-nav>#${AI_NAV_ID}{grid-column:3!important}
  ...
  #${AI_NAV_ID}{
    ...
    border:1px solid rgba(84,221,255,.36)!important;
    border-radius:16px!important;
    background:
      radial-gradient(circle at 50% 22%,rgba(112,228,255,.12),transparent 52%),
      linear-gradient(180deg,rgba(16,27,38,.98),rgba(8,14,21,.98))!important;
  ...
  #${AI_NAV_ID} .mf-ai-center-star{
    ...
    text-shadow:0 0 13px rgba(84,221,255,.28)!important;
  ...
  #${AI_NAV_ID}.active{
    border-color:rgba(112,229,255,.74)!important;
    background:
      radial-gradient(circle at 50% 22%,rgba(112,228,255,.20),transparent 54%),
      linear-gradient(180deg,rgba(18,31,43,.99),rgba(8,15,22,.99))!important;
    box-shadow:
      0 8px 24px rgba(0,0,0,.30),
      0 0 0 3px rgba(84,221,255,.07),
      0 0 22px rgba(84,221,255,.10)!important;
  }
  ...
  #mfAiV20DirectResultWrap{display:none;width:100%;margin-top:2px}
  #mfAiV20DirectResultWrap.show{display:block}
  #mfAiV20DirectResultHead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:2px 0 8px}
  #mfAiV20DirectResultHead b{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#54ddff}
  #mfAiV20DirectResultHead span{font-size:9px;color:#8290a2}
  #mfAiV20DirectResult{display:grid;gap:10px;width:100%;min-width:0}
  #mfAiV20DirectResult .mf-v20-summary{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:13px}
  #mfAiV20DirectResult .mf-v20-summary h3{margin:0 0 5px;font-size:15px;color:#f4f8fb}
  #mfAiV20DirectResult .mf-v20-summary p{margin:0;color:#93a1b2;font-size:11px;line-height:1.5}
  #mfAiV20DirectResult .mf-v20-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  #mfAiV20DirectResult .mf-v20-metric{min-width:0;border:1px solid #1d2936;border-radius:12px;background:#070c12;padding:11px}
  #mfAiV20DirectResult .mf-v20-metric small{display:block;color:#8290a2;font-size:8px;letter-spacing:.09em;text-transform:uppercase}
  #mfAiV20DirectResult .mf-v20-metric b{display:block;margin-top:5px;color:#f4f8fb;font-size:13px;overflow-wrap:anywhere}
  #mfAiV20DirectResult .mf-v20-section{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:12px}
  #mfAiV20DirectResult .mf-v20-section>small{display:block;margin-bottom:8px;color:#54ddff;font-size:8px;letter-spacing:.1em;text-transform:uppercase}
  #mfAiV20DirectResult .mf-v20-row{display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-top:1px solid rgba(255,255,255,.055);font-size:10px}
  #mfAiV20DirectResult .mf-v20-row:first-of-type{border-top:0}
  #mfAiV20DirectResult .mf-v20-row span{color:#8290a2}
  #mfAiV20DirectResult .mf-v20-row b{color:#f4f8fb;text-align:right;overflow-wrap:anywhere}
  #mfAiV20DirectResult .mf-v20-pre{margin:0;white-space:pre-wrap;overflow-wrap:anywhere;color:#dce6ef;font:10px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}
  #mfAiV20DirectResult .mf-v20-loading{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:14px;color:#8290a2;font-size:11px;line-height:1.5}
  #mfAiV20DirectResult .mf-v20-error{border:1px solid rgba(255,101,118,.28);border-radius:14px;background:#070c12;padding:14px;color:#ff6576;font-size:11px;line-height:1.5}
...
`;
  ...
  function ensureHeaderWalletButton() {
    if (innerWidth > 820) return false;
  ...
  function ensureCenterAiNavButton() {
    ...
    const openButton = document.getElementById(OPEN_BTN_ID);
    if (!nav || !openButton) return false;
    ...
    if (!ai) {
      ...
      ai.innerHTML = '<span class="mf-ai-center-star" aria-hidden="true">✦</span>';
  ...
  function ensureSheet() {
    ...
    sheet.innerHTML = `
      <div class="sheet-top">
        ...
        <button id="mfAiV20Close" class="close-sheet" type="button" aria-label="Close AI">×</button>
      </div>

      <div class="mf-ai-body">
        <div id="mfAiV20Status" class="mf-ai-status">Ready.</div>
        ...
        <button id="mfAiV20Ask" class="btn mf-ai-ask" type="button">Ask AI</button>

        <div id="mfAiV15Output" class="mf-ai-output">Ready.</div>

        <div id="mfAiV20DirectResultWrap">
          <div id="mfAiV20DirectResultHead">
            <b>AI Analysis &amp; Market Data</b>
            <span id="mfAiV20DirectResultState">Waiting</span>
          </div>
          <div id="mfAiV20DirectResult"></div>
        </div>
      </div>
    `;
    ...
    const close = sheet.querySelector('#mfAiV20Close');
    ...
    const mint = sheet.querySelector('#mfAiV20Mint');
    const prompt = sheet.querySelector('#mfAiV20Prompt');
    const ask = sheet.querySelector('#mfAiV20Ask');
  ...
  function forceHideLegacyBackend(root) {
    ...
    root.classList.add('mf-ai-v20-backend-hidden');
    ...
    if (backendOverlay && backendOverlay !== root) {
      backendOverlay.classList.add('mf-ai-v20-backend-hidden');
  ...
  function directConfig() {
    return window.__MEMEFLOW_AI_DIRECT_V20_CONFIG__ || {};
  }

  function setDirectResultState(label) {
    const state = document.getElementById('mfAiV20DirectResultState');
    if (state) state.textContent = label || '';
  }

  function directResultHost() {
    return {
      wrap: document.getElementById('mfAiV20DirectResultWrap'),
      host: document.getElementById('mfAiV20DirectResult')
    };
  }
  ...
  function showDirectLoading() {
    ...
    if (host) host.innerHTML = '<div class="mf-v20-loading">Running MEMEFLOW evaluator directly. MANUAL AI SCAN remains untouched.</div>';
  }
  ...
  function fmtMetric(label, value) {
    ...
    return `<div class="mf-v20-metric"><small>${escapeHtml(label)}</small><b>${escapeHtml(shown)}</b></div>`;
  }
  ...
  function renderRowsSection(title, value) {
    ...
    return `<div class="mf-v20-section"><small>${escapeHtml(title)}</small>${rows.map(([k,v]) => `<div class="mf-v20-row"><span>${escapeHtml(k)}</span><b>${escapeHtml(safeString(v))}</b></div>`).join('')}</div>`;
  }
  ...
  function renderDirectResult(raw) {
    ...
    const summary = `<div class="mf-v20-summary"><h3>${escapeHtml(decision || 'Analysis complete')}</h3><p>${escapeHtml(reason || 'MEMEFLOW evaluator returned a result for this token.')}</p></div>`;
    const grid = metricHtml ? `<div class="mf-v20-grid">${metricHtml}</div>` : '';
    const evidenceHtml = renderRowsSection('Evidence', evidence);
    const timelineHtml = renderRowsSection('Timeline', timeline);
    const rawHtml = `<details class="mf-v20-section"><summary style="cursor:pointer;color:#8290a2;font-size:9px;letter-spacing:.08em;text-transform:uppercase">Raw evaluator result</summary><pre class="mf-v20-pre">${escapeHtml(JSON.stringify(raw, null, 2))}</pre></details>`;
  ...
  function renderDirectError(message) {
    const { wrap, host } = directResultHost();
    wrap?.classList.add('show');
    if (host) host.innerHTML = `<div class="mf-v20-error">${escapeHtml(message)}</div>`;
  ...
  async function runDirectEvaluatorFromAi() {
    const sheet = document.getElementById(SHEET_ID);
    const aiMint = sheet?.querySelector('#mfAiV20Mint');
  ...
  function setStatus(value) {
    const el = document.getElementById('mfAiV20Status');
  ...
  function bindManualButton() {
    ...
    if (btn.dataset.mfAiDirectV20 === '1') return true;

    btn.dataset.mfAiDirectV20 = '1';
