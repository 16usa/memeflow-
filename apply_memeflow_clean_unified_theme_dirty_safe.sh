...
# Dirty-worktree-safe mode:
# runtime JSON, old backups and untracked installer files are allowed.
# Only the four files this patch owns must be clean.
...
if [[ "$LOCAL_HEAD" != "$REMOTE_HEAD" ]]; then
  echo "ERROR: main is not identical to origin/main." >&2
  echo "Local : $LOCAL_HEAD" >&2
  echo "Remote: $REMOTE_HEAD" >&2
  echo "Because runtime files are dirty, this installer will not auto-pull." >&2
  exit 1
fi

if ! git diff --quiet -- "${TARGETS[@]}"; then
  echo "ERROR: One of the four visual target files already has local edits:" >&2
  git status --short -- "${TARGETS[@]}"
  exit 1
fi

if ! git diff --cached --quiet -- "${TARGETS[@]}"; then
  echo "ERROR: One of the visual target files is already staged:" >&2
  git status --short -- "${TARGETS[@]}"
  exit 1
fi

# Never accidentally include previously staged unrelated files in this commit.
STAGED_OTHER="$(git diff --cached --name-only || true)"
if [[ -n "$STAGED_OTHER" ]]; then
  echo "ERROR: Unrelated files are already staged. Unstage them before running this patch:" >&2
  printf '%s\n' "$STAGED_OTHER" >&2
  exit 1
fi

echo "main matches origin/main."
echo "Unrelated runtime/backup changes will be left untouched."
...
if git diff --cached --quiet -- "$CSS" "$SYSTEM" "$TOKENS" "$TRADING"; then
  echo "No changes to commit."
else
  echo "Staged visual files only:"
  git diff --cached --name-only
  git commit -m "Extend single Mission Control theme to standalone pages" -- "$CSS" "$SYSTEM" "$TOKENS" "$TRADING"
fi
