...
echo "[MEMEFLOW] Live Token States repair v7-worktree-safe"
...
# This installer DOES NOT require a clean Replit workspace.
# All patching/testing/committing happens in a temporary clean Git worktree
# created from origin/main. Existing M / D / ?? files are left untouched.
echo "[git] fetch origin/main"
git fetch origin main

TMP="$(mktemp -d /tmp/memeflow-v7-XXXXXX)"
BACKUP_DIR="$ROOT/.memeflow-v7-recovery-$(date +%Y%m%d-%H%M%S)"
PATCH_FILES=(
...
cleanup() {
  ...
  if [[ $code -ne 0 ]]; then
    echo
    echo "[FAILED] nothing was committed or pushed by v7."
    echo "[FAILED] your existing Replit M / D / ?? files were not modified."
  fi
  exit "$code"
}
trap cleanup EXIT
...
new_route = r""" if(url.pathname==='/api/system/live-token-states'&&req.method==='GET'){
  // MEMEFLOW_LIVE_TOKEN_VISIBILITY_V7_WORKTREE_SAFE
  //
  // Scanner inventory and trading eligibility are intentionally separate.
  //
  // DISPLAY:
  //   ADMITTED -> normal Logic state
  //   PENDING  -> WAITING with Entry Filter reason
  //   REJECTED -> BLOCKED with Entry Filter reason
  //   OPEN     -> always visible
  //
  // TRADE:
  ...
  // Keep cache identity tied to response shape.
  ...
  for(const _token of _rawTokens){
    ...
    // Large UI snapshots yield to Pump WebSocket callbacks.
    _processed++;
    ...
    if(!_eligible&&!_isOpen){
      ...
      // Entry Filters still block trading; the observability page simply shows
      // the actual admission state instead of deleting the row.
  ...
  const _payload={
    ...
    limit:_limit,
    source:'system-live-token-states-transparent-v7',
...
if "MEMEFLOW_LIVE_TOKEN_VISIBILITY_V7_WORKTREE_SAFE" not in strict:
    i = strict.find(start)
    j = strict.find(end, i if i >= 0 else 0)
    if i < 0 or j < 0:
        raise SystemExit("[error] strict-entry-admission anchors not found")

    old_comment = "// Entry admission controls BOTH card visibility and trading eligibility."
    ci = strict.rfind(old_comment, 0, i)
    if ci >= 0 and i-ci < 500:
        i = ci

    block = r"""// MEMEFLOW_LIVE_TOKEN_VISIBILITY_V7_WORKTREE_SAFE
...
assert.match(liveStatesRoute,/MEMEFLOW_LIVE_TOKEN_VISIBILITY_V7_WORKTREE_SAFE/);
...
if "MEMEFLOW_LIVE_TOKEN_VISIBILITY_V7_WORKTREE_SAFE" not in fresh:
...
assert.match(liveRoute,/MEMEFLOW_LIVE_TOKEN_VISIBILITY_V7_WORKTREE_SAFE/);
...
assert.match(liveRoute,/system-live-token-states-transparent-v7/);
...
# ---------------------------------------------------------------------------
# Fix an already-stale settings architecture test in current main.
# It must validate a versioned module URL, not one obsolete cache-buster value.
# ---------------------------------------------------------------------------
...
if ri < 0 or rj < 0:
    raise SystemExit("[verify] route boundaries missing")
...
for needle in [
    ...
    "preAdmissionHidden:0",
    "state:_blocked?'BLOCKED':'WAITING'",
    "system-live-token-states-transparent-v7",
]:
...
print("[verify] core invariants OK")
PY
...
if git diff --cached --quiet; then
  echo "DONE: origin/main already contains v7; no commit needed."
...
# ---------------------------------------------------------------------------
# LOCAL REPLIT RECOVERY
# The remote fix is already verified/pushed above.
# Now clean ONLY residues left by older v4/v5/v6 installers.
# Unknown local edits are preserved.
# ---------------------------------------------------------------------------
cd "$ROOT"
mkdir -p "$BACKUP_DIR"
...
sync_known_residue() {
  ...
  if grep -Eq "$pattern" "$file" 2>/dev/null; then
    ...
    git restore --source="$NEW_SHA" --worktree --staged -- "$file"
    echo "[local] replaced known old-installer residue: $file"
    return 0
  fi
...
sync_known_residue \
  "memeflow-app/tests/settings-architecture-v2.mjs" \
  'Scanner scans all · filters classify cards and gate trading|settings-page\\\.js\\\?v=\[A-Za-z0-9'

sync_known_residue \
  "memeflow-app/system-tokens.js" \
  'MEMEFLOW_REALTIME_COALESCE_V[456]_|MEMEFLOW_LIVE_DIAGNOSTICS_V[456]_'

sync_known_residue \
  "memeflow-app/system-tokens.html" \
  'live-visibility-v[456]'
...
echo
echo "DONE"
echo "- patch/test/commit ran in a CLEAN temporary worktree"
echo "- unrelated Replit M / D / ?? files never blocked the install"
...
echo "- known v4/v5/v6 residue was backed up then synced to verified main"
