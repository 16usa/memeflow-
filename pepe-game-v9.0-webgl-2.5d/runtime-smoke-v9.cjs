...
global.visualViewport={width:390,height:844,addEventListener(){},removeEventListener(){}};
global.scrollTo=()=>{};
document.fullscreenElement=null;
document.webkitFullscreenElement=null;
document.exitFullscreen=async()=>{};
document.webkitExitFullscreen=async()=>{};
docEl.requestFullscreen=async()=>{};
docEl.webkitRequestFullscreen=async()=>{};
...
(async()=>{
  ...
  assert(get('preflightTitle').textContent==='Searching for BUY READY','searching preflight title not synced');
  assert(get('preflightKicker').textContent.startsWith('SCANNING'),'searching preflight kicker not synced');
  console.log('V7.11 runtime smoke: PASS');process.exit(0);
