(()=>{
  ...
  function create(options={}){
    ...
    const moteTarget=()=>Math.max(14,Math.round((w<500?26:38)*quality));
    ...
    function draw(ts,dt){
      ...
      const travel=live?(28+energy*255+rank*16):(mode==='searching'?14:5.5);
      ...
      for(const s of stars){
        ...
        const visible=rank===0?(mode==='searching'?.46:.26):clamp(.30+rank*.14,0,1);
      ...
      }else if((mode==='idle'||mode==='searching')&&!reduced()&&Math.random()<(mode==='searching'?.22:.14)*quality){
      ...
      scroll=(scroll+travel*.082*scale)%420;drift=Math.sin(ts/1300)*(1.2+energy*7)+(Number(state.acceleration)||0)*90;
