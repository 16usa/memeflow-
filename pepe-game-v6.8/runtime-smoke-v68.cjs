...
(async()=>{
  ...
  assert(get('multiplier').innerHTML.includes('1.52'),'live multiplier did not render 1.52x');
  ...
  const doneAt=Date.now();statusBalance=10052;statusSession={...statusSession,revision:2,updatedAt:doneAt,completedAt:doneAt,state:'COMPLETE',payout:152,profit:52,reason:'AUTO_CASH_OUT',currentPrice:1,entryPrice:.5,priceUpdateCount:2,maxDrawdownPct:0,maxAdverseExcursionPct:0,timeToPeakMs:500};
  emit('state',{engineEpoch:'v68',serverTime:doneAt,eventSeq:120,stateRevision:120,balance:10000,history:[],stats:{rounds:1,wins:1,losses:0,netProfit:52,bestMultiplier:1.52},session:statusSession});
  await sleep(360);
  ...
  assert(get('multiplier').innerHTML.includes('1.00'),'Play Again did not reset multiplier to 1.00x');
  ...
  get('multiplier').innerHTML='1.52<span>×</span>';get('peakHud').textContent='1.52×';get('thrustHud').textContent='100%';get('stageLabel').textContent='ORBIT';
  get('startBtn').dispatch('click');await sleep(40);
  assert(startCalls>0,'search did not start');
  assert(get('multiplier').innerHTML.includes('1.00'),'SCANNING retained previous multiplier');
  ...
  console.log('V6.8 runtime smoke: PASS');process.exit(0);
