...
echo "=== MEMEFLOW TOKEN INTELLIGENCE V23.1 ==="
...
mf_clear_stale_index_lock(){
  ...
  if [[ -n "$active" ]]; then
    echo "V23.1 REFUSED: .git/index.lock exists and active git is running:"
    echo "$active"
    echo "Nothing changed."
    return 1
  fi
  echo "V23.1: removing stale .git/index.lock"
  rm -f -- "$lock"
  [[ ! -e "$lock" ]] || {
    echo "V23.1 REFUSED: unable to remove stale .git/index.lock"
    return 1
  }
}
...
[[ "$(git branch --show-current)" == "$BRANCH" ]] || {
  echo "V23.1 REFUSED: expected branch $BRANCH"
  echo "actual: $(git branch --show-current)"
  exit 1
}

[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V23.1 REFUSED: audited HEAD changed"
...
for f in "${MODIFIED[@]}"; do
  [[ -f "$f" ]] || { echo "V23.1 REFUSED: missing $f"; exit 1; }
  git diff --quiet -- "$f" || { echo "V23.1 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V23.1 REFUSED: staged changes in $f"; exit 1; }
done

[[ ! -e "$TEST" ]] || {
  echo "V23.1 REFUSED: $TEST already exists"
  exit 1
}
...
for file,markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(
                f"V23.1 REFUSED: audited marker missing in {file}: {marker}"
            )
...
for forbidden in [
    ...
    if forbidden in shadow or forbidden in app:
        raise SystemExit(
            f"V23.1 REFUSED: feature already appears installed: {forbidden}"
        )
...
BACKUP=".memeflow-backups/token-intelligence-v23-1-$STAMP"
mkdir -p "$BACKUP"
...
def once(text,old,new,label):
    ...
    if count!=1:
        raise SystemExit(
            f"V23.1 REFUSED: {label}: expected 1 exact match, got {count}"
        )
...
if pkg.count(needle)!=1:
    raise SystemExit(
        f"V23.1 REFUSED: package insertion anchor count={pkg.count(needle)}"
    )
...
echo
echo "=== V23.1 FULL PROJECT TEST SUITE ==="
(
  cd memeflow-app
  npm test
)
...
if "shadowOnly:true" not in test:
    errors.append("V23.1 regression does not assert shadow-only")

if errors:
    raise SystemExit(
        "V23_1_CONTRACT_FAILED:\n- " +
        "\n- ".join(errors)
    )

print("V23_1_CONTRACT_OK")
PY
...
git commit -m "feat: add shadow token specialists and monitor v23.1"
