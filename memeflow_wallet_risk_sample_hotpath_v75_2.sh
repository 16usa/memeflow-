...
# MEMEFLOW_WALLET_RISK_SAMPLE_HOTPATH_V75_2_INSTALLER
#
# Corrected V75 installer (backup/stash fix).
#
...
restore_created_stash_on_failure(){
  ...
  if git stash apply --index "$LOCAL_STASH" >/dev/null 2>&1; then
    echo "Original local work restored after failed V75.2 attempt."
  else
    git reset --hard "$LOCAL_BASE" >/dev/null 2>&1 || true
    if git stash apply "$LOCAL_STASH" >/dev/null 2>&1; then
      echo "Original local work restored (content mode) after failed V75.2 attempt."
    else
...
echo "==> V75.2 exact branch / remote preflight"
...
if [[ -n "$(git status --porcelain=v1 --untracked-files=all)" ]]; then
  git stash push --include-untracked \
    -m "MEMEFLOW V75.2 preserve current local work $STAMP" >/dev/null
  ...
  while read -r ref; do
    ...
    case "$msg" in
      *"MEMEFLOW V75 preserve current local work"*|*"MEMEFLOW V75.2 preserve current local work"*)
...
# IMPORTANT: create recovery evidence only AFTER stash handling.
# git stash -u can temporarily remove untracked .patch-backups content,
# so creating this directory before the stash would make it disappear.
mkdir -p "$BACKUP"
...
echo "==> Create isolated V75.2 patch worktree from exact V74"
...
def once(old,new,label):
    ...
    if n!=1:
        raise SystemExit(
            f"V75.2 REFUSED: {label}: expected exactly 1 target, found {n}"
        )
    text=text.replace(old,new,1)

if mode=="server":
    if "MEMEFLOW_WALLET_RISK_SAMPLE_HOTPATH_V75" in text:
        raise SystemExit("V75.2 REFUSED: server marker already exists")
...
"wallet-risk holderRows normalization"
    ...
    if 'node tests/wallet-risk-sample-hotpath-v75.mjs' in text:
        raise SystemExit("V75.2 REFUSED: package already contains V75")
...
echo "==> Targeted V75.2 regressions"
