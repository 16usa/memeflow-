...
APP = Path(sys.argv[1])
CANON = "memeflow-x-canonical-v185.css"
    ...
    while i<n:
        ...
        if header.startswith("@"):
            ...
            if not any(low.startswith(x) for x in SKIP_AT_BLOCKS):
                records.extend(scan_context(inner,label+(normalize_at_context(header),)))
            i=close+1; continue
...
for page in PAGES:
    ...
    if sum(x==CANON for x in links)!=1:
        errors.append(f"{page}: V185 canonical count != 1")
    if not links or links[-1]!=CANON:
        errors.append(f"{page}: V185 is not LAST stylesheet")
    ...
    for stale in (
        ...
        "memeflow-x-canonical-v180.css","memeflow-x-canonical-v181.css","memeflow-x-canonical-v182.css","memeflow-x-canonical-v183.css",
    ):
        if stale in html:
...
if dup_keys:
    for (ctx,sel,prop),rows in dup_keys[:80]:
        ...
        errors.append(
            f"canonical duplicate text owner [{' > '.join(ctx) if isinstance(ctx,tuple) else ctx}] {sel[:90]} {prop}: {vals}"
        )
...
for marker in (
    ...
    "memeflow_text_canonical_v185_start",
    "inter",
    "system-ui",
):
...
JS_TEXT_CUSTOM_PROP_RE = re.compile(
    r'\.style\.setProperty\(\s*["\']--[\w-]*(?:'
    r'font|line-height|line-h|letter-spacing|word-spacing|text-transform|'
    r'text-decoration|text-shadow|text-rendering'
    r')[\w-]*["\']', re.I)
...
JS_EMBEDDED_TEXT_STYLE_RE = re.compile(
    r'<style\b[^>]*>[\s\S]*?(?:font(?:-family|-size|-weight|-style)?\s*:|line-height\s*:|letter-spacing\s*:|word-spacing\s*:|text-(?:transform|decoration|shadow|rendering)\s*:|color\s*:)[\s\S]*?</style>',
    re.I
)
JS_LITERAL_CANVAS_FONT_RE = re.compile(r'\b(?:ctx|context|c)\.font\s*=\s*["\'][^"\']+["\']', re.I)
...
for page in PAGES:
    ...
    for si,script in enumerate(re.findall(r'<script\b(?![^>]*\bsrc=)[^>]*>([\s\S]*?)</script>',html,re.I),1):
        if (JS_DIRECT_PROP_RE.search(script) or JS_SET_PROPERTY_RE.search(script)
            or JS_TEXT_CUSTOM_PROP_RE.search(script) or JS_STYLE_ATTR_RE.search(script) or JS_CSSTEXT_RE.search(script) or JS_EMBEDDED_TEXT_STYLE_RE.search(script) or JS_LITERAL_CANVAS_FONT_RE.search(script)):
...
for name in sorted(active_js):
    ...
    if (JS_DIRECT_PROP_RE.search(js) or JS_SET_PROPERTY_RE.search(js)
        or JS_TEXT_CUSTOM_PROP_RE.search(js) or JS_STYLE_ATTR_RE.search(js) or JS_CSSTEXT_RE.search(js) or JS_EMBEDDED_TEXT_STYLE_RE.search(js) or JS_LITERAL_CANVAS_FONT_RE.search(js)):
...
for required in (
    ...
    if required.lower() not in canon.lower():
        errors.append(f"V185 runtime canonical rule missing {required}")


for required in (
    'MEMEFLOW_RUNTIME_TEXT_FINAL_V185_START',
    '#mf-final-chart-host .name',
    '#mf-final-chart-host .price',
    '#allocationBadge[data-mf-tone="allocation-ok"]',
    '#allocationBadge[data-mf-tone="allocation-bad"]',
):
    if required.lower() not in canon.lower():
        errors.append(f'V185 canonical runtime component rule missing {required}')

market_js=(APP/'market-chart-final-v5.js').read_text(encoding='utf-8',errors='replace')
for forbidden in ('attachShadow','shadowRoot','.style.cssText','<style>'):
    if forbidden in market_js:
        errors.append(f'market-chart-final-v5.js: legacy isolated style owner remains: {forbidden}')
trading_js=(APP/'trading.js').read_text(encoding='utf-8',errors='replace')
if re.search(r'node\.style\.color\s*=', trading_js):
    errors.append('trading.js: allocation badge still owns style.color')

if errors:
    print("V185 DEEP TEXT CONFLICT AUDIT: FAIL")
    for e in errors[:180]:
        print(" -",e)
    if len(errors)>180:
...
print("V185 DEEP TEXT CONFLICT AUDIT: PASS")
print(" - production pages fully audited: 11")
print(f" - active stylesheets audited: {len(active)}")
print(" - ONE text appearance owner: memeflow-x-canonical-v184.css")
