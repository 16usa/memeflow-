...
echo "=== MEMEFLOW V4.4 · TRUTH MOTION ==="

test -f "$HTML"
test -f "$CSS"

[ -f "$JS" ] && cp "$JS" "${JS}.before-v44.${STAMP}.bak" || true
cp "$CSS" "${CSS}.before-v44.${STAMP}.bak"
cp "$HTML" "${HTML}.before-v44.${STAMP}.bak"
...
s=re.sub(
    ...
    '/memeflow-flow-v4.css?v=4.4-truth-motion',
    s
)

s=re.sub(
    r'/memeflow-flow-v4\.js\?v=[^"\']+',
    '/memeflow-flow-v4.js?v=4.4-truth-motion',
    s
)
...
echo
echo "============================================"
echo " MEMEFLOW V4.4 · TRUTH MOTION INSTALLED"
echo "============================================"
echo
echo "INGEST movement:"
echo "  real eventCount delta / real elapsed time"
echo
echo "DECODE movement:"
echo "  real tradeCount delta / real elapsed time"
echo
echo "Decision counts:"
echo "  /api/ai/decisions?scope=all&limit=200"
echo
echo "Decision movement:"
echo "  ONLY actual state changes / recent new decisions"
echo
echo "Initial decision snapshot:"
echo "  baseline only — NO fake pulse"
echo
echo "No new telemetry:"
echo "  NO input motion"
echo
echo "High throughput:"
echo "  visual batching enabled"
echo "  exact streak scale is displayed in the UI"
echo
