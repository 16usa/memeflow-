...
const expected={"game.html":"971125d44159695220ef7223800042f81a8b6c288e59f087c6716eb81b1288e1","game.css":"2a3b73861a6b838e35b927dafe59b3a975f48dbc719c6bf037719cdc553adeff","game.js":"f8575f7752245ed5058302b8414b1be77dd0bd314d6aff70d6121bc156fac9aa"};
...
pass('namespaced V6.7 package directory',path.basename(here)==='pepe-game-v6.7');pass('payload contains visual files only',JSON.stringify(fs.readdirSync(payload).sort())===JSON.stringify([...files].sort()));
...
pass('V6.7 client version',js.includes("CLIENT_VERSION='6.7'"));pass('V6.7 cache bust',html.includes('/game.js?v=67')&&html.includes('/game.css?v=67'));
pass('dynamic visualViewport JS removed',!js.includes('visualViewport')&&!js.includes("'--vvh'")&&!js.includes('scheduleViewportHeight')&&!js.includes('syncViewportHeight'));
pass('stable small viewport height used',css.includes('--stable-vh:100svh')&&css.includes('min-height:var(--stable-vh)'));
pass('old dynamic viewport variable removed',!css.includes('--vvh')&&!css.includes('var(--vvh'));
pass('mobile dock backdrop blur removed',css.includes('.mobile-dock{')&&css.includes('-webkit-backdrop-filter:none;backdrop-filter:none'));
pass('mobile dock promoted stable composite',css.includes('transform:translate3d(0,0,0)')&&css.includes('contain:layout paint style')&&css.includes('backface-visibility:hidden'));
pass('mobile topbar blur disabled',css.includes('.topbar{height:62px;padding:0 12px;-webkit-backdrop-filter:none;backdrop-filter:none'));
pass('mobile dock text width stabilized',css.includes('.mobile-dock button>div{min-width:0}')&&css.includes('text-overflow:ellipsis'));
pass('mobile pointer focus released',js.includes("[ui.mobileStart,ui.mobileCash].forEach(button=>button?.addEventListener?.('pointerup',()=>button.blur()"));
pass('launch panel compact mobile polish',css.includes('/* V6.7 mobile stability + launch control polish */')&&css.includes('.launch-panel .stake-input{height:48px}'));
pass('wake lock request is serialized',js.includes('game.wakeRequestPending')&&js.includes('const seq=++game.wakeRequestSeq'));
pass('stream hidden/offline guard',js.includes("if(!game.pageVisible||game.lifecyclePaused||navigator.onLine===false)return;"));
pass('search background wait event based',js.includes('function waitForSearchResume(seq)')&&!js.includes('await wait(500);'));
...
const latest=path.join(app,'.memeflow-patches','pepe-game-v67','latest.json');pass('V6.7 protection metadata exists',fs.existsSync(latest));if(fs.existsSync(latest)){const meta=JSON.parse(fs.readFileSync(latest,'utf8'));for(const f of ['src/game-engine.mjs','app-server.mjs','index.html'])pass('protected unchanged '+f,sha(path.join(app,f))===meta.protectedBefore[f]);}
execFileSync(process.execPath,[path.join(here,'runtime-smoke-v67.cjs'),app],{stdio:'inherit'});pass('runtime smoke',true);
console.log(`PEPE GAME V6.7 VERIFY: PASS (${checks} checks)`);
