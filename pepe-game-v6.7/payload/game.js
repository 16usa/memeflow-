(()=>{
  ...
  function bind(){
    ...
    ui.start.addEventListener('click',startRound);ui.mobileStart.addEventListener('click',startRound);ui.cash.addEventListener('click',cashOut);ui.mobileCash.addEventListener('click',cashOut);[ui.mobileStart,ui.mobileCash].forEach(button=>button?.addEventListener?.('pointerup',()=>button.blur(),{passive:true}));ui.playAgain.addEventListener('click',playAgain);ui.clearHistory.addEventListener('click',clearHistory);
    ...
    addEventListener('beforeunload',()=>{clearTimeout(game.historyClearTimer);clearTimeout(game.wakeRetryTimer);clearTimeout(game.resultFocusTimer);game.wakeRetryTimer=null;game.resultFocusTimer=null;game.wakeRequestSeq++;game.searchResumeCleanup?.();game.searchResumeCleanup=null;stopClock();clearVisualTimers();game.completionSeq++;game.pendingResultId=null;stopVisualLoop();game.stream?.close?.();game.stream=null;stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }
