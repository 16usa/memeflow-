...
# Confirm stale Settings palette/blur values are gone ONLY from Settings rules.
# Generic system-page classes such as `.glass` are unrelated to Settings and
# must not make the Settings audit fail.
SETTINGS_MARKERS = (
...
system_no_comments = re.sub(r'/\\*[\\s\\S]*?\\*/', '', system)

for m in re.finditer(r'([^{}]+)\\{([^{}]*)\\}', system_no_comments):
    selector = re.sub(r'\\s+', ' ', m.group(1)).strip()
    body = m.group(2)
    low_selector = selector.lower()

    if not any(marker in low_selector for marker in SETTINGS_MARKERS):
        continue

    for stale in STALE_SETTINGS_VISUALS:
        if stale in body:
            errors.append(
                f"system.css stale Settings visual value {stale} in {selector[:140]}"
            )
