...
APP=Path(sys.argv[1])
CANON="memeflow-x-canonical-v178.css"
...
for name in PAGES:
    ...
    if sum(x==CANON for x in clean)!=1:
        errors.append(f"{name}: V178 canonical count != 1")
    if not clean or clean[-1]!=CANON:
        errors.append(f"{name}: V178 is not LAST stylesheet")
...
if "memeflow_neutral_text_v178_start" not in canon:
    errors.append("V178 grayscale palette section missing")
...
if errors:
    print("V178 PURE NEUTRAL TEXT AUDIT: FAIL")
    for e in errors[:120]:
        print(" -",e)
    if len(errors)>120:
...
print("V178 PURE NEUTRAL TEXT AUDIT: PASS")
...
print(" - V178 canonical is LAST everywhere")
