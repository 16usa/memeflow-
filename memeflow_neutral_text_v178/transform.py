...
if not SOURCE.exists():
    raise SystemExit("ERROR: V176 canonical missing before V178 transform")
...
css=re.sub(
    r'/\* MEMEFLOW_NEUTRAL_TEXT_V178_START \*/[\s\S]*?'
    r'/\* MEMEFLOW_NEUTRAL_TEXT_V178_END \*/',
    '',
    css,
)

PALETTE = r'''
/* MEMEFLOW_NEUTRAL_TEXT_V178_START */
...
/* MEMEFLOW_NEUTRAL_TEXT_V178_END */
'''

css=css.rstrip()+"\n\n"+PALETTE.strip()+"\n"

target=APP/"memeflow-x-canonical-v178.css"
...
for name in PAGES:
    ...
    for line in text.splitlines():
        if "memeflow-x-canonical-v176.css" in line:
            continue
        if "memeflow-x-canonical-v178.css" in line:
            continue
        lines.append(line)
    text="\n".join(lines)
    if "</head>" not in text:
        raise SystemExit(f"ERROR: </head> missing in {p}")
    link='<link rel="stylesheet" href="/memeflow-x-canonical-v178.css?v=pure-neutral-text-v178-20260922">'
