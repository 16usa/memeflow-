...
APP="memeflow-app"
WORK=".memeflow-neutral-text-v178-work"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP=".memeflow-neutral-text-v178-backup-$STAMP"
POINTER=".memeflow-neutral-text-v178-last-backup"
...
rm -rf ".memeflow-neutral-text-v177-work" "$WORK"
...
if [ ! -f "$WORK/memeflow-app/memeflow-x-canonical-v176.css" ]; then
  if [ ! -f "$WORK/memeflow-app/memeflow-x-canonical-v175.css" ] && \
    ...
    python3 memeflow_neutral_text_v178/v175_transform.py "$WORK/memeflow-app"
    python3 memeflow_neutral_text_v178/v175_audit.py "$WORK/memeflow-app"
  fi

  echo "Applying bundled V176 readability in temporary worktree..."
  python3 memeflow_neutral_text_v178/v176_transform.py "$WORK/memeflow-app"
  python3 memeflow_neutral_text_v178/v176_audit.py "$WORK/memeflow-app"
fi

python3 memeflow_neutral_text_v178/transform.py "$WORK/memeflow-app"
python3 memeflow_neutral_text_v178/audit.py "$WORK/memeflow-app"
...
python3 memeflow_neutral_text_v178/audit.py "$APP"

echo
echo "Installed MEMEFLOW PURE NEUTRAL TEXT V178"
echo "Backup: $BACKUP"
