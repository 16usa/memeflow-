...
echo "[MEMEFLOW] Live Token Feed bridge v11"
...
TMP="$(mktemp -d /tmp/memeflow-v11-XXXXXX)"
...
cleanup() {
  ...
  if [[ $code -ne 0 ]]; then
    echo
    echo "[FAILED] v11 made no commit/push."
...
new_route = r""" if(url.pathname==='/api/system/live-token-states'&&req.method==='GET'){
  ...
  // MEMEFLOW_LIVE_TOKEN_FEED_BRIDGE_V11
  //
  // SCANNER:
  ...
  const _payload={
    ...
    feedVersion:'MEMEFLOW_LIVE_TOKEN_FEED_BRIDGE_V11',
...
new_state = """  loading: false,
  ...
  // MEMEFLOW_LIVE_TOKEN_FEED_DIAGNOSTICS_V11
  feedReturned: 0,
  feedWorkingSet: 0,
  feedRawScanner: 0,
  feedViewErrors: 0,
...
if "live-token-feed-v11-20260827" not in html:
    html2, count = re.subn(
        r'(/system-tokens\.js\?v=)[^"\']+',
        r'\1live-token-feed-v11-20260827',
        html,
        count=1
    )
    if count != 1:
    ...
    html = html2
    print("[apply] system-tokens.js v11 cache-buster")
else:
    print("[skip] system-tokens.js v11 cache-buster: already installed")
...
fresh_anchor = """assert.doesNotMatch(liveRoute,/_rawTokens\\.slice\\(0,_lim\\)/);
"""
...
assert.match(liveRoute,/MEMEFLOW_LIVE_TOKEN_FEED_BRIDGE_V11/);
...
assert.match(liveRoute,/feedVersion:'MEMEFLOW_LIVE_TOKEN_FEED_BRIDGE_V11'/);
...
new_html_assert = """assert.match(tokenHtml,/system-tokens\\.js\\?v=live-token-feed-v11-20260827/);"""
...
assert.match(route,/MEMEFLOW_LIVE_TOKEN_FEED_BRIDGE_V11/);
assert.match(tokenUi,/MEMEFLOW_LIVE_TOKEN_FEED_DIAGNOSTICS_V11/);
...
if "live-token-feed-v11-20260827" not in html:
    raise SystemExit("[verify] v11 frontend cache-buster missing")
...
echo "[check] exact regressions first"
...
if git diff --cached --quiet; then
  echo "[git] v11 is already present on origin/main"
...
BACKUP_DIR="$ROOT/.memeflow-v11-recovery-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
if git merge-base --is-ancestor "$LOCAL_HEAD" "$NEW_SHA" 2>/dev/null; then
  ...
  if git merge --ff-only "$NEW_SHA"; then
    echo "[local] workspace fast-forwarded to verified v11"
  else
    echo "[local] fast-forward blocked; syncing only v11 files"
    git restore --source="$NEW_SHA" --worktree -- "${PATCH_FILES[@]}"
  fi
else
  echo "[local] local branch is not a clean ancestor; syncing only v11 files"
  git restore --source="$NEW_SHA" --worktree -- "${PATCH_FILES[@]}"
fi
