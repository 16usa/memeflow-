(()=>{
  ...
  const ui={
    ...
    tracePath:$('#tracePath'),traceDot:$('#traceDot'),autoTrigger:$('#autoTrigger'),autoTriggerLabel:$('#autoTriggerLabel'),stopTrigger:$('#stopTrigger'),stopTriggerLabel:$('#stopTriggerLabel'),rocket:$('#rocket'),moonBeacon:$('#moonBeacon'),
  ...
  function updateStoryFlight(multiplier,direction,danger,stage,velocity){
    ...
    let mood='ready';
    if(game.mode==='searching'){mood='scan';}
    else if(ui.game.dataset.outcome==='secure'){mood='secure';}
    else if(ui.game.dataset.outcome==='crash'){mood='crash';}
    else if(game.mode==='live'||game.mode==='settling'){if(danger==='high'){mood='danger';}else if(direction==='down'&&velocity<-.012){mood='dip';}else if(stage==='moon'||stage==='deep'||stage==='hyper'){mood='moon';}else if(direction==='up'||velocity>.012){mood='boost';}else{mood='cruise';}}
    ui.rocket.dataset.mood=mood;
    let hue='0deg',bright='1',sat='1.1';if(game.mode==='searching'){hue='135deg';bright='1.02';sat='1.05';}else if(danger==='high'){hue='-82deg';bright='1.05';sat='1.45';}else if(direction==='down'){hue='-58deg';bright='.92';sat='1.18';}else if(stage==='moon'||stage==='deep'||stage==='hyper'){hue='72deg';bright='1.18';sat='1.35';}else if(direction==='up'){hue='48deg';bright='1.16';sat='1.28';}
    ...
    ui.rocket.style.transform=`translate(calc(-50% + var(--flight-x)),calc(-1 * var(--flight-y))) rotate(${game.visualTilt.toFixed(2)}deg) scale(var(--flight-scale))`;
  }
