...
# MEMEFLOW_LIVE_CARD_BATCH_MINT_HOTPATH_V76_1_INSTALLER
#
# Exact baseline:
...
restore_start_state_on_failure(){
  # Only restore the original branch/workspace automatically when nothing was
  # pushed. After a successful push, keep the V76 safety stash intact if a
  # later local reconstruction step needs manual recovery.
  [[ "$PUSHED" == "0" ]] || return 0
  ...
  if [[ "$SWITCHED_TO_TARGET" == "1" ]]; then
    # Ensure the target branch is clean before returning to the original ref.
    git reset --hard "$V75" >/dev/null 2>&1 || true

    if [[ -n "$START_BRANCH" ]]; then
      git switch "$START_BRANCH" >/dev/null 2>&1 || true
      git reset --hard "$START_HEAD" >/dev/null 2>&1 || true
    else
      git switch --detach "$START_HEAD" >/dev/null 2>&1 || true
    fi
  fi

  if [[ -n "$LOCAL_STASH" ]]; then
    if git stash apply --index "$LOCAL_STASH" >/dev/null 2>&1; then
      echo "Original local work restored after failed V76.1 attempt."
    else
      git reset --hard "$START_HEAD" >/dev/null 2>&1 || true
      if git stash apply "$LOCAL_STASH" >/dev/null 2>&1; then
        echo "Original local work restored (content mode) after failed V76.1 attempt."
      else
...
cleanup(){
  local rc=$?
  set +e
  [[ "$rc" != "0" ]] && restore_start_state_on_failure
...
echo "==> V76.1 remote-first preflight"

START_BRANCH="$(git branch --show-current)"
START_HEAD="$(git rev-parse HEAD)"

echo "Starting branch: ${START_BRANCH:-DETACHED}"
...
echo "==> Preserve ALL current local work BEFORE branch correction"

if [[ -n "$(git status --porcelain=v1 --untracked-files=all)" ]]; then
  git stash push -u -m "MEMEFLOW V76.1 preserve current local work $STAMP" >/dev/null
...
echo "==> Correct local branch to exact target V75"
...
[[ "$(git branch --show-current)" == "$BRANCH" ]] || die "could not switch to target branch"
[[ "$(git rev-parse HEAD)" == "$V75" ]] || die "target branch is not exact V75 after correction"

echo "Local target branch exact V75: CONFIRMED"
