...
echo "== MEMEFLOW permanent scanner registry + restart continuity + filtered display =="
...
direct_success="""  discMetrics.lastSuccessfulScanAt=Date.now();
  try{void __mfWsMetadataEnrich(e.mint,e.uri)}catch{}
  return token;
}
"""
direct_success_new="""  discMetrics.lastSuccessfulScanAt=Date.now();
...
"""
if direct_success in app:
    app=app.replace(direct_success,direct_success_new,1)
elif "'pump-live-create-v1'" not in app:
    raise SystemExit("PATCH FAILED: direct CREATE success anchor changed")
