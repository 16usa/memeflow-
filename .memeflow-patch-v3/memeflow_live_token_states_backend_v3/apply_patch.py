...
PATCH_ID = "MEMEFLOW_LIVE_SYSTEM_SSE_BACKEND_V3"
...
// MEMEFLOW_LIVE_SYSTEM_SSE_BACKEND_V3
...
'''
    ...
    route = r'''
 // MEMEFLOW_LIVE_SYSTEM_SSE_BACKEND_V3_ROUTE
 if(url.pathname==='/api/system/stream'&&req.method==='GET'){
  res.writeHead(200,{
...
'''
    chart_route_pattern = r"if\s*\(\s*url\.pathname\s*===\s*['\"]\/api\/chart\/stream['\"]\s*\)"
    ...
    m = matches[0]
    publish_hook = r'''
  // V3 System View: actual backend publish cadence drives the 3D/token-flow impulse.
...
'''
    ...
    indent = a.group(1)
    create_hook = (
        f"{indent}// V3 System View: accepted real Pump CREATE event.\n"
