...
NEWTEST="memeflow-app/tests/canonical-live-score-pipeline-v20_8_2.mjs"

for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST"; do
  [[ -f "$f" ]] || { echo "ERROR missing $f"; exit 1; }
done

[[ "$(git branch --show-current)" == "$BRANCH" ]] || { echo "V20.8.2 REFUSED: wrong branch"; exit 1; }
[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V20.8.2 REFUSED: audited HEAD mismatch";
...
for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST"; do
  git diff --quiet -- "$f" || { echo "V20.8.2 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V20.8.2 REFUSED: staged changes in $f"; exit 1; }
done

echo "=== V20.8.2 AUDITED PRECHECK ==="
python3 - <<'PY'
...
BACKUP=".memeflow-backups/canonical-live-score-v20_8_2-$STAMP"
mkdir -p "$BACKUP/memeflow-app/src" "$BACKUP/memeflow-app/tests"
for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST"; do cp "$f" "$BACKUP/$f"; done

rollback(){
 rc=$?
 if [[ $rc -ne 0 ]]; then
  echo "=== V20.8.2 FAILED — RESTORING ==="
  for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST"; do cp "$BACKUP/$f" "$f" || true; done
  rm -f "$NEWTEST"
...
from pathlib import Path
import re
APP=Path('memeflow-app/app-server.mjs'); UI=Path('memeflow-app/system-tokens.js'); HTML=Path('memeflow-app/system-tokens.html'); RANK=Path('memeflow-app/src/feed-ranking.mjs'); TEST=Path('memeflow-app/tests/feed-ranking.mjs'); OLD=Path('memeflow-app/tests/live-truth-no-dynamic-cache-v20_3.mjs'); CANON=Path('memeflow-app/tests/canonical-score-state-v20_7.mjs'); DETAIL=Path('memeflow-app/tests/card-details-live-authority-v20_5.mjs')
...
r=rep(r,"""      const liveCandidate = isLiveCandidateState(view.state);
      ...
      return {
        ...view,
        decisionScore,
        // MEMEFLOW_CANONICAL_VISIBLE_AI_SCORE_V20_8_2
        score: decisionScore,
        feedScore: relevanceScore,
        relevanceScore,
        statePriority: statePriority(view.state)
...
new="""// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_2
...
route=route[:vs]+"""  // MEMEFLOW_UNIFIED_FULL_LIVE_VIEW_V20_8_2
...
const __mfLiveStatesResponseCache=new Map();""","""// MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8_2
...
a=a[:ws]+"  // MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8_2\n  // Intentionally no live-state response cache write.\n"+a[we:]
APP.write_text(a)
...
u=rep(u,"""const __MF_TOKEN_IDENTITY_KEYS_V20_2=[
  ...
  // MEMEFLOW_STATIC_IDENTITY_ONLY_V20_8_2
  'name','metadataName','symbol','metadataSymbol',
  'image','imageUrl','imageUri','logo','logoUrl','logoURI'
];""",'identity keys')
...
u=rep(u,"""    const incomingByMint=new Map(
    ...
    // MEMEFLOW_VISIBLE_INVALIDATION_SCOPE_V20_8_2
...
u=rep(u,"""        const scoreA =
        ...
        // MEMEFLOW_SCORE_FIRST_TIEBREAK_V22""","""        // MEMEFLOW_SMART_HIDDEN_FEED_RANK_V20_8_2
...
# V20.8.2: migrate stale regressions that intentionally asserted deleted architecture.
# They now assert the canonical single-score contract instead.
t=TEST.read_text()
t=rep(t,"assert.match(app,/MEMEFLOW_WAITING_PREVIEW_SCORE_V21/);","assert.match(app,/MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_2/);",'feed test old preview marker')
TEST.write_text(t)

ct=CANON.read_text()
ct=ct.replace("assert.match(app,/MEMEFLOW_CANONICAL_SCORE_STATE_V20_7/);","assert.match(app,/MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_2/);")
ct=ct.replace("const ds=app.indexOf('const __v20truth=__mfCurrentEntryTruthV20_2');","const ds=app.indexOf('// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_2');")
...
h=HTML.read_text(); h,n=re.subn(r'src="/system-tokens\.js\?v=[^"]+"','src="/system-tokens.js?v=canonical-live-score-v20-8-2-20260903"',h,count=1)
if n!=1: raise SystemExit('asset cache-bust anchor missing')
HTML.write_text(h)
print('V20_8_2_TRANSFORM_OK')
PY
...
assert.match(rankSource,/MEMEFLOW_CANONICAL_VISIBLE_AI_SCORE_V20_8_2/);
assert.doesNotMatch(rankSource,/score:\s*liveCandidate\s*\?/);

const ds=app.indexOf('// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_2');
...
assert.match(full,/MEMEFLOW_UNIFIED_FULL_LIVE_VIEW_V20_8_2/);
...
assert.match(ui,/MEMEFLOW_VISIBLE_INVALIDATION_SCOPE_V20_8_2/);
assert.match(ui,/if\(!requestedMints\.has\(mint\)\)return previous;/);
assert.match(ui,/MEMEFLOW_SMART_HIDDEN_FEED_RANK_V20_8_2/);
assert.match(html,/system-tokens\.js\?v=canonical-live-score-v20-8-2-20260903/);

console.log('CANONICAL_LIVE_SCORE_PIPELINE_V20_8_2_OK');
TESTJS

echo "=== V20.8.2 REGRESSION CONTRACT PRECHECK ==="
...
echo "=== V20.8.2 TARGETED TESTS ==="
(
 cd memeflow-app
 node tests/canonical-live-score-pipeline-v20_8_2.mjs
...
echo "=== V20.8.2 FULL TEST SUITE ==="
(cd memeflow-app && npm test)

git diff --check -- "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$NEWTEST"

echo "=== V20.8.2 STATIC INVARIANTS ==="
python3 - <<'PY'
from pathlib import Path
...
echo "=== V20.8.2 STAGE ==="
git reset >/dev/null
git add "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$NEWTEST"
BAD="$(git diff --cached --name-only | grep -Ev '^memeflow-app/(app-server\.mjs|system-tokens\.js|system-tokens\.html|src/feed-ranking\.mjs|tests/feed-ranking\.mjs|tests/live-truth-no-dynamic-cache-v20_3\.mjs|tests/canonical-score-state-v20_7\.mjs|tests/card-details-live-authority-v20_5\.mjs|tests/canonical-live-score-pipeline-v20_8_2\.mjs)$' || true)"
