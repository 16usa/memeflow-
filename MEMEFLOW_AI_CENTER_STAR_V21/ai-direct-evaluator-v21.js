/* MEMEFLOW AI Direct Evaluator Sheet v21.0
...
(() => {
  ...
  if (window.__MEMEFLOW_AI_DIRECT_V21__) return;
  window.__MEMEFLOW_AI_DIRECT_V21__ = true;

  const OPEN_BTN_ID = 'mfManualAiButton';
  const SHEET_ID = 'sheet-ai-direct-v21';
  const STYLE_ID = 'mf-ai-direct-v21-style';
  const MORE_PROXY_ID = 'mf-mobile-more-proxy';
  const AI_NAV_ID = 'mf-ai-center-nav-v21';
  const HEADER_WALLET_ID = 'mf-header-wallet-v21';
...
@media(max-width:820px){
  ...
  #mfAiV21DirectResultWrap{display:none;width:100%;margin-top:2px}
  #mfAiV21DirectResultWrap.show{display:block}
  #mfAiV21DirectResultHead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:2px 0 8px}
  #mfAiV21DirectResultHead b{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#54ddff}
  #mfAiV21DirectResultHead span{font-size:9px;color:#8290a2}
  #mfAiV21DirectResult{display:grid;gap:10px;width:100%;min-width:0}
  #mfAiV21DirectResult .mf-v21-summary{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:13px}
  #mfAiV21DirectResult .mf-v21-summary h3{margin:0 0 5px;font-size:15px;color:#f4f8fb}
  #mfAiV21DirectResult .mf-v21-summary p{margin:0;color:#93a1b2;font-size:11px;line-height:1.5}
  #mfAiV21DirectResult .mf-v21-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  #mfAiV21DirectResult .mf-v21-metric{min-width:0;border:1px solid #1d2936;border-radius:12px;background:#070c12;padding:11px}
  #mfAiV21DirectResult .mf-v21-metric small{display:block;color:#8290a2;font-size:8px;letter-spacing:.09em;text-transform:uppercase}
  #mfAiV21DirectResult .mf-v21-metric b{display:block;margin-top:5px;color:#f4f8fb;font-size:13px;overflow-wrap:anywhere}
  #mfAiV21DirectResult .mf-v21-section{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:12px}
  #mfAiV21DirectResult .mf-v21-section>small{display:block;margin-bottom:8px;color:#54ddff;font-size:8px;letter-spacing:.1em;text-transform:uppercase}
  #mfAiV21DirectResult .mf-v21-row{display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-top:1px solid rgba(255,255,255,.055);font-size:10px}
  #mfAiV21DirectResult .mf-v21-row:first-of-type{border-top:0}
  #mfAiV21DirectResult .mf-v21-row span{color:#8290a2}
  #mfAiV21DirectResult .mf-v21-row b{color:#f4f8fb;text-align:right;overflow-wrap:anywhere}
  #mfAiV21DirectResult .mf-v21-pre{margin:0;white-space:pre-wrap;overflow-wrap:anywhere;color:#dce6ef;font:10px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}
  #mfAiV21DirectResult .mf-v21-loading{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:14px;color:#8290a2;font-size:11px;line-height:1.5}
  #mfAiV21DirectResult .mf-v21-error{border:1px solid rgba(255,101,118,.28);border-radius:14px;background:#070c12;padding:14px;color:#ff6576;font-size:11px;line-height:1.5}
...
`;
  ...
  function ensureSheet() {
    ...
    sheet.innerHTML = `
      <div class="sheet-top">
        ...
        <button id="mfAiV21Close" class="close-sheet" type="button" aria-label="Close AI">×</button>
      </div>

      <div class="mf-ai-body">
        <div id="mfAiV21Status" class="mf-ai-status">Ready.</div>
        ...
        <button id="mfAiV21Ask" class="btn mf-ai-ask" type="button">Ask AI</button>

        <div id="mfAiV15Output" class="mf-ai-output">Ready.</div>

        <div id="mfAiV21DirectResultWrap">
          <div id="mfAiV21DirectResultHead">
            <b>AI Analysis &amp; Market Data</b>
            <span id="mfAiV21DirectResultState">Waiting</span>
          </div>
          <div id="mfAiV21DirectResult"></div>
        </div>
      </div>
    `;
    ...
    const close = sheet.querySelector('#mfAiV21Close');
    ...
    const mint = sheet.querySelector('#mfAiV21Mint');
    const prompt = sheet.querySelector('#mfAiV21Prompt');
    const ask = sheet.querySelector('#mfAiV21Ask');
  ...
  function forceHideLegacyBackend(root) {
    ...
    root.classList.add('mf-ai-v21-backend-hidden');
    ...
    if (backendOverlay && backendOverlay !== root) {
      backendOverlay.classList.add('mf-ai-v21-backend-hidden');
  ...
  function directConfig() {
    return window.__MEMEFLOW_AI_DIRECT_V21_CONFIG__ || {};
  }

  function setDirectResultState(label) {
    const state = document.getElementById('mfAiV21DirectResultState');
    if (state) state.textContent = label || '';
  }

  function directResultHost() {
    return {
      wrap: document.getElementById('mfAiV21DirectResultWrap'),
      host: document.getElementById('mfAiV21DirectResult')
    };
  }
  ...
  function showDirectLoading() {
    ...
    if (host) host.innerHTML = '<div class="mf-v21-loading">Running MEMEFLOW evaluator directly. MANUAL AI SCAN remains untouched.</div>';
  }
  ...
  function fmtMetric(label, value) {
    ...
    return `<div class="mf-v21-metric"><small>${escapeHtml(label)}</small><b>${escapeHtml(shown)}</b></div>`;
  }
  ...
  function renderRowsSection(title, value) {
    ...
    return `<div class="mf-v21-section"><small>${escapeHtml(title)}</small>${rows.map(([k,v]) => `<div class="mf-v21-row"><span>${escapeHtml(k)}</span><b>${escapeHtml(safeString(v))}</b></div>`).join('')}</div>`;
  }
  ...
  function renderDirectResult(raw) {
    ...
    const summary = `<div class="mf-v21-summary"><h3>${escapeHtml(decision || 'Analysis complete')}</h3><p>${escapeHtml(reason || 'MEMEFLOW evaluator returned a result for this token.')}</p></div>`;
    const grid = metricHtml ? `<div class="mf-v21-grid">${metricHtml}</div>` : '';
    const evidenceHtml = renderRowsSection('Evidence', evidence);
    const timelineHtml = renderRowsSection('Timeline', timeline);
    const rawHtml = `<details class="mf-v21-section"><summary style="cursor:pointer;color:#8290a2;font-size:9px;letter-spacing:.08em;text-transform:uppercase">Raw evaluator result</summary><pre class="mf-v21-pre">${escapeHtml(JSON.stringify(raw, null, 2))}</pre></details>`;
  ...
  function renderDirectError(message) {
    const { wrap, host } = directResultHost();
    wrap?.classList.add('show');
    if (host) host.innerHTML = `<div class="mf-v21-error">${escapeHtml(message)}</div>`;
  ...
  async function runDirectEvaluatorFromAi() {
    const sheet = document.getElementById(SHEET_ID);
    const aiMint = sheet?.querySelector('#mfAiV21Mint');
  ...
  function setStatus(value) {
    const el = document.getElementById('mfAiV21Status');
  ...
  function bindManualButton() {
    ...
    if (btn.dataset.mfAiDirectV21 === '1') return true;

    btn.dataset.mfAiDirectV21 = '1';
