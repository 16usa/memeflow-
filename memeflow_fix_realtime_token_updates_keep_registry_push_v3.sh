...
echo "== MEMEFLOW realtime data repair V3: runtime-dirty safe installer =="
...
# MEMEFLOW_INSTALLER_DIRTY_RUNTIME_SAFE_V3
# Runtime files (data/state.json, ledgers, .patch-backups, etc.) are allowed
# to be dirty. They are NOT staged, reset, stashed, or committed by this patch.
#
# We stop only if one of the SOURCE FILES that this patch itself edits already
# has local changes, so we never overwrite somebody else's source work.
PATCH_PATHS=(
  "app-server.mjs"
...
for f in "${PATCH_PATHS[@]}"; do
  if ! git diff --quiet -- "$f" || ! git diff --cached --quiet -- "$f"; then
    ...
    echo "Runtime/data changes are allowed; source-code conflicts are not."
    git status --short -- "$f"
    exit 1
  fi
done
...
echo "Runtime/backup dirtiness is allowed and will be left untouched."
...
if [[ "$LOCAL_HEAD" != "$REMOTE_HEAD" ]]; then
  echo "Local main is behind/ahead of origin/main; attempting fast-forward only."
...
echo
echo "== Stage source files only (runtime data stays untouched) =="
git add \
  app-server.mjs \
  src/token-registry.mjs \
  src/pump-live-trade-feed.mjs \
...
echo "Runtime/data files are intentionally NOT staged:"
git status --short --untracked-files=no -- data .patch-backups 2>/dev/null || true
