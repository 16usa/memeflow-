...
if grep -q 'data-mf-ai-module-v1="1"' "$INDEX"; then
  echo "AI ANALYSIS STANDALONE MODULE V1 is already installed."
  exit 0
fi

PATCH_DIR="$APP/.memeflow-patches/ai-analysis-standalone-module-v1"
mkdir -p "$PATCH_DIR"
...
trap 'echo "ERROR: AI standalone module patch failed; restoring exact pre-install index.html."; rollback' ERR
...
if 'data-mf-ai-module-v1=' not in opening:
    opening = opening[:-1] + ' data-mf-ai-module-v1="1">'
...
# ---------------------------------------------------------------
# 3) Replace the existing contiguous AI-specific CSS source block.
#    No new <style> element is added.
# ---------------------------------------------------------------
style_re = re.compile(
    r'(<style\s+id=["\']memeflow-consolidated-css["\']>)(.*?)(</style>)',
    flags=re.I | re.S
)
sm = style_re.search(src)
if not sm:
    raise SystemExit("memeflow-consolidated-css not found.")

css = sm.group(2)
start_marker = "/* ─── AI Analysis & Market Data collapsible (inside Primary Candidate) ─────── */"
end_marker = "/* Workspace: only queue-panel remains after AI-Decision-Studio merge */"

start = css.find(start_marker)
end = css.find(end_marker)
if start < 0 or end < 0 or end <= start:
    raise SystemExit("Could not isolate the current AI-specific CSS block.")

replacement = (
    "/* ─── AI Analysis & Market Data · standalone module ─────────────────── */\n"
    + canonical_css
    + "\n\n"
    + end_marker
)
css = css[:start] + replacement + css[end + len(end_marker):]

# Update the one obsolete source comment elsewhere.
old_comment = "/* #decision-studio merged into #primary-candidate .ai-analysis-section */"
if css.count(old_comment) == 1:
    css = css.replace(
        old_comment,
        "/* AI Decision Studio lives in the standalone #ai-analysis module */",
        1
    )

src = src[:sm.start()] + sm.group(1) + css + sm.group(3) + src[sm.end():]
...
checks = {
    ...
    "one standalone marker": src.count('data-mf-ai-module-v1="1"') == 1,
    ...
    "one canonical CSS owner": src.count("MF_AI_STANDALONE_MODULE_V1") == 1,
...
grep -q 'data-mf-ai-module-v1="1"' "$WORK"
grep -q 'MF_AI_STANDALONE_MODULE_V1' "$WORK"
