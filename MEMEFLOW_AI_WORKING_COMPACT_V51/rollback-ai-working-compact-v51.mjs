...
if(!appDir){console.error('V51 rollback: project not found.');process.exit(1)}

const backup=path.join(appDir,'.memeflow-v51-backup');
...
if(!fs.existsSync(indexBackup)||!fs.existsSync(serverBackup)){
  console.error('V51 rollback backup missing. Nothing changed.');
  process.exit(1);
}
...
console.log('V51 ROLLBACK OK: exact pre-V51 frontend/server state restored.');
console.log('Restart server manually only if you want the restored server code loaded.');
