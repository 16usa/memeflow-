...
# V81.1 recovery-safe behavior: if the first V81 attempt already created a
# valid pre-install backup, keep using it instead of overwriting rollback truth
# with the partially-installed files from the failed attempt.
if [[ -f "$POINTER" ]] && [[ -d "$(cat "$POINTER")" ]]; then
  BACKUP="$(cat "$POINTER")"
  echo "Recovery mode: reusing original V81 backup: $BACKUP"
else
...
# Create the backup only if this is the first V81/V81.1 attempt.
if [[ ! -f "$BACKUP/git-head.txt" ]]; then
