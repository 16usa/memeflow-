...
PATCH_ID = "MEMEFLOW_PER_USER_DISCOVERY_V34_2"
STAMP = time.strftime("%Y%m%d-%H%M%S")


def log(msg: str) -> None:
    print(f"[DISCOVERY-V34.2] {msg}", flush=True)
...
BACKUP = ROOT / f".per-user-discovery-v34-2-backup-{STAMP}"
...
"""
  ...
  return {
   ...
   return json(res,200,{
    ...
    # ------------------------------------------------------------
    # 19) USER LIST ROUTE — no extra token-feed patch is needed.
    #
    # The real Live Token States UI reads /api/ai/decisions, which already
    # reads store.decisions(u.id). Per-user visibility is enforced upstream
    # by live evaluation, recovery and the per-user mode prune/reevaluation.
    # Do NOT introduce a second UI-side/global token source filter here.
    # ------------------------------------------------------------
    if "const _all=store.decisions(u.id);" not in app:
        raise RuntimeError(
            "real /api/ai/decisions per-user decision source not found"
        )
    ...
    validations = {
        ...
        "personal decision list": "const _all=store.decisions(u.id);" in final_app,
