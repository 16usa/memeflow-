(()=>{
  ...
  const ui={
    ...
    start:$('#startBtn'),cash:$('#cashoutBtn'),cashHint:$('#cashoutHint'),mobileStart:$('#mobileStart'),mobileCash:$('#mobileCashout'),mobileCashHint:$('#mobileCashoutHint'),stageCash:$('#stageCashoutBtn'),stageCashHint:$('#stageCashoutHint'),stagePayout:$('#stagePayout'),stageProfit:$('#stageProfit'),
  ...
  function syncButtons(){
    ...
    ui.cash.disabled=!can||settling;ui.mobileCash.disabled=!can||settling;if(ui.stageCash)ui.stageCash.disabled=!can||settling;const value=live?`${(num(game.session?.multiplier)||1).toFixed(2)}× · ${money((num(game.session?.bet)||0)*(num(game.session?.multiplier)||1))}`:'Waiting for launch';ui.cashHint.textContent=value;ui.mobileCashHint.textContent=live?value:'Waiting';if(ui.stageCashHint)ui.stageCashHint.textContent=live?value:(settling?'Locking…':'Waiting');renderCashoutTelemetry();
  ...
  function renderCashoutTelemetry(s=game.session){
    ...
    if(!s||s.state!=='LIVE'){
      if(ui.stagePayout)ui.stagePayout.textContent=money(currentBet());if(ui.stageProfit)ui.stageProfit.textContent='Profit $0.00';
    ...
    const m=Math.max(0,num(s.multiplier)||1),bet=Math.max(0,num(s.bet)||0),value=bet*m,pnl=value-bet,peak=Math.max(.000001,num(s.peak)||m),capture=clamp(m/peak*100,0,100);if(ui.stagePayout)ui.stagePayout.textContent=money(value);if(ui.stageProfit){ui.stageProfit.textContent=`${pnl>=0?'Profit +':'Loss '}${money(Math.abs(pnl))}`;ui.stageProfit.className=pnl>.005?'positive':pnl<-.005?'negative':'';}
  ...
  function bind(){
    ...
    ui.start.addEventListener('click',startRound);ui.mobileStart.addEventListener('click',startRound);ui.cash.addEventListener('click',cashOut);ui.mobileCash.addEventListener('click',cashOut);ui.stageCash?.addEventListener('click',cashOut);[ui.mobileStart,ui.mobileCash,ui.stageCash].forEach(button=>button?.addEventListener?.('pointerup',()=>button.blur(),{passive:true}));ui.playAgain.addEventListener('click',playAgain);ui.clearHistory.addEventListener('click',clearHistory);
