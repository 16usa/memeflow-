...
echo "=== MEMEFLOW WALLET REPUTATION / SMART MONEY MEMORY V23.2 ==="
...
mf_clear_stale_index_lock(){
  ...
  if [[ -n "$active" ]]; then
    echo "V23.2 REFUSED: .git/index.lock exists and active git is running:"
    echo "$active"
    echo "Nothing changed."
    return 1
  fi
  echo "V23.2: removing stale .git/index.lock"
  rm -f -- "$lock"
  [[ ! -e "$lock" ]] || {
    echo "V23.2 REFUSED: unable to remove stale .git/index.lock"
    return 1
  }
}
...
[[ "$(git branch --show-current)" == "$BRANCH" ]] || {
  echo "V23.2 REFUSED: expected branch $BRANCH"
  echo "actual: $(git branch --show-current)"
  exit 1
}

[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V23.2 REFUSED: audited HEAD changed"
...
for f in "${MODIFIED[@]}"; do
  [[ -f "$f" ]] || { echo "V23.2 REFUSED: missing $f"; exit 1; }
  git diff --quiet -- "$f" || { echo "V23.2 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V23.2 REFUSED: staged changes in $f"; exit 1; }
done

for f in "${NEW_FILES[@]}"; do
  [[ ! -e "$f" ]] || { echo "V23.2 REFUSED: $f already exists"; exit 1; }
done
...
for file,markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(
                f"V23.2 REFUSED: audited marker missing in {file}: {marker}"
            )
...
for forbidden in [
    ...
    if forbidden in shadow or forbidden in app:
        raise SystemExit(
            f"V23.2 REFUSED: feature already appears installed: {forbidden}"
        )
...
BACKUP=".memeflow-backups/wallet-reputation-v23-2-$STAMP"
mkdir -p "$BACKUP"
...
def once(text,old,new,label):
    ...
    if count!=1:
        raise SystemExit(
            f"V23.2 REFUSED: {label}: expected 1 exact match, got {count}"
        )
...
""",
    """    smartMoneySeed:{
      // Raw current-token cohort retained for learning provenance.
      candidateWallets:wallet.candidateWallets
    },
    // MEMEFLOW_SMART_MONEY_MEMORY_V23_2
...
if pkg.count(needle)!=1:
    raise SystemExit(
        f"V23.2 REFUSED: package insertion anchor count={pkg.count(needle)}"
    )
...
echo
echo "=== V23.2 FULL PROJECT TEST SUITE ==="
(
  cd memeflow-app
  npm test
)
...
git commit -m "feat: learn shadow smart money wallet memory v23.2"
