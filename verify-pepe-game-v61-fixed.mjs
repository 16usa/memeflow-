...
const sourceDir=path.join(pkg,'source');
for(const f of ['game.html','game.css','game.js']){
  pass('package visual source '+f,fs.existsSync(path.join(sourceDir,f))&&sha(path.join(sourceDir,f))===expected[f]);
}
// /home/runner/workspace/source is shared by every previously-unzipped updater.
// Extra stale files there are NOT part of the V6.1 archive and must not fail verification.
const zipCandidates=[
  path.join(pkg,'pepe-game-v6.1-stability-flight-polish.zip'),
  path.join(root,'pepe-game-v6.1-stability-flight-polish.zip')
];
const zipPath=zipCandidates.find(fs.existsSync);
if(zipPath){
  const listing=spawnSync('unzip',['-Z1',zipPath],{encoding:'utf8'});
  const entries=listing.status===0?listing.stdout.split(/\r?\n/).filter(Boolean):[];
  pass('V6.1 ZIP archive readable',listing.status===0);
  pass('V6.1 ZIP contains only visual source files',
    ['source/game.html','source/game.css','source/game.js'].every(f=>entries.includes(f)) &&
    !entries.some(f=>/(^|\/)(game-engine\.mjs|app-server\.mjs|evaluate\.mjs|liveeval\.mjs)$/.test(f))
  );
}
...
console.log(`PEPE GAME V6.1 VERIFY FIXED: PASS (${checks} checks)`);
