...
const MARK = 'MEMEFLOW_CANONICAL_TOKEN_IMAGE_V1';
...
if (
  ...
  console.log('[IMAGE-V1] Already installed.');
  process.exit(0);
}

const stamp = new Date().toISOString().replace(/[:.]/g, '-');
const backupDir = path.join(ROOT, '.patch-backups', 'canonical-token-image-v1-' + stamp);
fs.mkdirSync(backupDir, { recursive: true });

for (const p of Object.values(paths)) {
  fs.copyFileSync(p, path.join(backupDir, path.basename(p)));
}
console.log('[IMAGE-V1] Backup:', path.relative(ROOT, backupDir));
...
index = replaceOne(
  index,
  /async function getImage\(mint\)\{[\s\S]*?\n\s*\}\n\n\s*\/\* MEMEFLOW_CANDIDATE_CARD_LOGOS_SAFE_V2 \*\//,
  `async function getImage(mint){
    const value=String(mint||'').trim();
    if(!value)return '';
...
/* MEMEFLOW_CANDIDATE_CARD_LOGOS_SAFE_V2 */`,
  'dashboard candidate-card image source'
);
...
const rollbackPath = path.join(ROOT, 'rollback-canonical-token-image-v1.mjs');
...
console.log('\n[IMAGE-V1] Verification: PASS');
console.log('[IMAGE-V1] app-server.mjs syntax: PASS');
console.log('[IMAGE-V1] trading.js syntax: PASS');
console.log('[IMAGE-V1] system-tokens.js syntax: PASS');
console.log('[IMAGE-V1] Installed successfully.');
console.log('[IMAGE-V1] Canonical UI source: /api/token-image/<mint>');
console.log('[IMAGE-V1] Upstream source: Pump CreateEvent metadata URI');
console.log('[IMAGE-V1] Rollback: node rollback-canonical-token-image-v1.mjs');
