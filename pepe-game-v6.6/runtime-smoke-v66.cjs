...
global.fetch=async(url)=>{if(url==='/api/game/status'){statusCalls++;const n=100+statusCalls;return response({version:'5.7.1',engineEpoch:'v66',serverTime:Date.now(),eventSeq:n,stateRevision:n,balance:10000,history:[],stats:{rounds:0},session:statusSession});}return response({});};
...
(async()=>{
  await sleep(60);if(!lastES)throw new Error('EventSource not created');lastES.readyState=1;lastES.onopen?.();
  const t0=Date.now();emit('snapshot',{engineEpoch:'v66',serverTime:t0,eventSeq:1000,stateRevision:1000,balance:10000,history:[],stats:{rounds:0},session:null});await sleep(20);
  if(!/stream live/i.test(get('streamState').textContent))throw new Error('valid SSE did not become healthy');

  const liveAt=Date.now();statusSession={id:'live-v66',revision:1,updatedAt:liveAt,startedAt:liveAt,latestPriceAt:liveAt,priceAgeMs:0,state:'LIVE',bet:100,multiplier:1.01,peak:1.01,feedFresh:true,canCashout:true,autoCashout:8,stopLoss:.7,token:{symbol:'T',name:'Token'}};
  emit('state',{engineEpoch:'v66',serverTime:liveAt,eventSeq:1100,stateRevision:1100,balance:9900,history:[],stats:{rounds:0},session:statusSession});
  ...
  emit('state',{engineEpoch:'v66',serverTime:jumpAt,eventSeq:1200,stateRevision:1200,balance:9900,history:[],stats:{rounds:0},session:statusSession});
  await sleep(35);
  ...
  console.log('V6.6 runtime smoke: PASS');process.exit(0);
