/*
  MEMEFLOW — Mobile AI Bottom Navigation Patch v4.0
  UI-only patch.
  Keeps the existing AI launcher in the center of the mobile bottom navigation
  and applies final visual polish.
*/
(() => {
  'use strict';

  if (window.__MEMEFLOW_AI_NAV_PATCH_V4__) return;
  window.__MEMEFLOW_AI_NAV_PATCH_V4__ = true;

  const MOBILE_MAX = 820;
  const STYLE_ID = 'mf-ai-bottom-nav-patch-style-v4';
...
@media (max-width: 820px) {
  ...
  .mobile-nav.mf-ai-nav-ready > [data-sheet="more"] { display: none !important; }
  ...
  .mobile-nav .mf-ai-nav-button {
    ...
    border: 1px solid rgba(92, 215, 255, .34) !important;

    background:
      radial-gradient(circle at 32% 20%, rgba(106, 226, 255, .10), transparent 42%),
      linear-gradient(180deg, rgba(18, 28, 39, .97), rgba(9, 15, 22, .98)) !important;
    ...
    box-shadow:
      ...
      0 0 14px rgba(84, 221, 255, .06) !important;
  ...
  .mobile-nav .mf-ai-nav-button::before {
    ...
    line-height: 1;
    color: #72e3ff;
    text-shadow: 0 0 10px rgba(84, 221, 255, .22);
    pointer-events: none;
  }
  ...
  .mobile-nav .mf-ai-nav-button::after {
    ...
    bottom: 11px;
    transform: translateX(-50%);
    font: 800 10px/1 -apple-system, BlinkMacSystemFont, "Segoe UI", Inter, sans-serif;
    letter-spacing: .08em;
    color: #f5f8fb;
    pointer-events: none;
  }
  ...
  .mobile-nav .mf-ai-nav-button:focus-visible {
    ...
    border-color: rgba(117, 230, 255, .58) !important;
    box-shadow:
      0 8px 18px rgba(0,0,0,.28),
      0 0 0 2px rgba(84,221,255,.09),
      0 0 16px rgba(84,221,255,.10) !important;
  }

  .mobile-nav .mf-ai-nav-button[aria-expanded="true"],
  .mobile-nav .mf-ai-nav-button.active,
  .mobile-nav .mf-ai-nav-button.is-active {
    border-color: rgba(117, 230, 255, .50) !important;
    box-shadow:
      0 9px 20px rgba(0,0,0,.30),
      0 0 0 2px rgba(84,221,255,.07),
      0 0 18px rgba(84,221,255,.10) !important;
  }
...
`;
  ...
  function scoreAiCandidate(el) {
    if (!el) return -999;
    if (el.closest('.mobile-nav')) return -999;
  ...
  function createMoreProxy(nav) {
    ...
    const host = document.querySelector('.topbar .top-actions') || document.querySelector('.topbar') || document.querySelector('.top-left');
  ...
  function moveIntoNav() {
    ...
    if (positions && launcher.parentNode !== nav) nav.insertBefore(launcher, positions);
    else if (!positions && launcher.parentNode !== nav) nav.appendChild(launcher);

    ['position','right','left','top','bottom','inset','z-index','width','height','transform','margin','max-width','max-height','min-width','min-height'].forEach(p => {
  ...
  function restoreDesktop() {
    ...
    if (launcher.parentNode !== originalParent) {
      if (originalNext && originalNext.parentNode === originalParent) originalParent.insertBefore(launcher, originalNext);
      else originalParent.appendChild(launcher);
    }
