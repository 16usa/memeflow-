...
echo "[MEMEFLOW] Live scanner + cache repair v9"
...
TMP="$(mktemp -d /tmp/memeflow-v9-XXXXXX)"
...
cleanup() {
  ...
  if [[ $code -ne 0 ]]; then
    echo
    echo "[FAILED] v9 made no commit/push."
...
if git diff --cached --quiet; then
  echo "[git] v9 is already present on origin/main"
...
BACKUP_DIR="$ROOT/.memeflow-v9-recovery-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
if git merge-base --is-ancestor "$LOCAL_HEAD" "$NEW_SHA" 2>/dev/null; then
  ...
  if git merge --ff-only "$NEW_SHA"; then
    echo "[local] workspace fast-forwarded to verified v9"
  else
    echo "[local] fast-forward blocked by unrelated dirty files; syncing only v9 files"
    git restore --source="$NEW_SHA" --worktree -- "${PATCH_FILES[@]}"
  fi
else
  echo "[local] local branch is not a clean ancestor; syncing only v9 files"
  git restore --source="$NEW_SHA" --worktree -- "${PATCH_FILES[@]}"
fi
