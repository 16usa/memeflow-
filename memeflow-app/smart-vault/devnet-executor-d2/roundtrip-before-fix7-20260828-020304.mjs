...
async function executeThroughSmartVault({
  ...
  const sig = await program.methods
    .executePumpV2(Buffer.from(pumpInstruction.data))
    .accounts({
      policy,
      vault,
...
// Do not trust SDK return shape for tokenProgram.
// The mint account owner on Solana is the authoritative token program
...
if (!mintAccountInfo) {
  throw new Error("D2 test mint account is missing on Devnet");
}

const tokenProgram = mintAccountInfo.owner;

console.log(
  "D2 mint token program (on-chain owner):",
  tokenProgram.toBase58()
);

// V2 has a separate quote mint/token program.
// D2 is intentionally SOL-paired, so NATIVE_MINT is wrapped SOL.
...
if (!quoteMintAccountInfo) {
  throw new Error("Wrapped SOL mint account is missing on Devnet");
}

const quoteTokenProgram = quoteMintAccountInfo.owner;

console.log(
  "D2 quote token program (on-chain owner):",
...
const [sellState, global2, feeConfig2] = await Promise.all([
  sdk.fetchSellState(mint.publicKey, vault, tokenProgram),
  sdk.fetchGlobal(),
  sdk.fetchFeeConfig().catch(() => null),
]);
...
const sellIxs = await PUMP_SDK.sellV2Instructions({
  ...sellState,
  global: global2,
  mint: mint.publicKey,
  user: vault,
