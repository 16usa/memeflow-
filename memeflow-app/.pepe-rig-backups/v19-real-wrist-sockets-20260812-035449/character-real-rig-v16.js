...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  if (!parent) throw new Error('[PEPE V18] AUTHORED WRIST PIVOTS');
  ...
  function attachHandToArm(handName, armName) {
    ...
    if (!hand || !arm) {
      throw new Error(
        `[PEPE V18] missing ${handName}/${armName}`
      );
    }
    ...
    /*
      The original part definition already contains
      the authored hand pivot.

      Preserve that exact authored world transform.
    */
    const authoredHandWorld =
      hand.pivot.matrixWorld.clone();

    /*
      Convert the authored hand world transform
      into arm-local space.

      This becomes the real wrist joint.
    */
    const armWorldInverse =
      arm.pivot.matrixWorld.clone().invert();

    const wristLocalMatrix =
      armWorldInverse.multiply(authoredHandWorld);
    ...
    wristLocalMatrix.decompose(
      wristJoint.position,
      wristJoint.quaternion,
      wristJoint.scale
    );
    ...
    /*
      The wrist joint now occupies exactly the same
      world pose that the authored hand pivot had.

      Make the hand local to that joint with identity
      transform. No visual jump is introduced.
    */
    ...
    hand.pivot.quaternion.identity();
    ...
    /*
      Preserve the authored draw order:
      hand behind sleeve at the connection.
    */
  ...
  const ready = Promise.all(defs.map(addPart)).then(() => {
    ...
    console.log('[PEPE V18] AUTHORED WRIST PIVOTS');
    return true;
  });
