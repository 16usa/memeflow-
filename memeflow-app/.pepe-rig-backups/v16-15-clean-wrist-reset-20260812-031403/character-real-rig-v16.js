...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  if (!parent) throw new Error('[PEPE V16.14] FINAL WRIST ALIGNMENT');
  ...
  function attachHandToArm(handName, armName) {
    ...
    // V16_12_WRIST_DEPTH_START
    arm.pivot.updateMatrixWorld(true);
    hand.pivot.updateMatrixWorld(true);

    const wristWorld = new THREE.Vector3();
    hand.pivot.getWorldPosition(wristWorld);

    wristWorld.y += 0.115;

    hand.pivot.position.copy(
      arm.pivot.worldToLocal(wristWorld.clone())
    );

    if (handName === 'handLeft') {
      hand.mesh.renderOrder = 45;
      arm.mesh.renderOrder = 55;
    }

    if (handName === 'handRight') {
      hand.mesh.renderOrder = 46;
      arm.mesh.renderOrder = 56;
    }

    if (hand.mesh.material) {
      hand.mesh.material.depthWrite = false;
      hand.mesh.material.depthTest = true;
      hand.mesh.material.needsUpdate = true;
    }

    if (arm.mesh.material) {
      arm.mesh.material.depthWrite = false;
      arm.mesh.material.depthTest = true;
      arm.mesh.material.needsUpdate = true;
    }
    // V16_12_WRIST_DEPTH_END
    // V16_13_DEEP_WRIST_SEAT_START
    arm.pivot.updateMatrixWorld(true);
    hand.pivot.updateMatrixWorld(true);

    const deepSeatWorld = new THREE.Vector3();
    hand.pivot.getWorldPosition(deepSeatWorld);

    deepSeatWorld.y += 0.145;

    hand.pivot.position.copy(
      arm.pivot.worldToLocal(deepSeatWorld.clone())
    );

    if (handName === 'handLeft') {
      hand.pivot.rotation.z -= THREE.MathUtils.degToRad(2.5);
      hand.mesh.renderOrder = 44;
      arm.mesh.renderOrder = 60;
    }

    if (handName === 'handRight') {
      hand.pivot.rotation.z += THREE.MathUtils.degToRad(2.5);
      hand.mesh.renderOrder = 45;
      arm.mesh.renderOrder = 61;
    }
    ...
    // V16_13_DEEP_WRIST_SEAT_END
    // V16_14_FINAL_WRIST_ALIGN_START
    arm.pivot.updateMatrixWorld(true);
    hand.pivot.updateMatrixWorld(true);

    const finalWristWorld = new THREE.Vector3();
    hand.pivot.getWorldPosition(finalWristWorld);

    if (handName === 'handLeft') {
      finalWristWorld.x += 0.157;
      finalWristWorld.y -= 0.374;

      hand.mesh.position.z = 0.32;
      arm.mesh.position.z = 0.35;
    ...
    if (handName === 'handRight') {
      finalWristWorld.x -= 0.156;
      finalWristWorld.y -= 0.395;

      hand.mesh.position.z = 0.33;
      arm.mesh.position.z = 0.36;
    ...
    hand.pivot.position.copy(
      arm.pivot.worldToLocal(finalWristWorld.clone())
    );
