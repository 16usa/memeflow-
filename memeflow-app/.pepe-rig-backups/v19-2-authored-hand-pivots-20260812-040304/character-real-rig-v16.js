...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  if (!parent) throw new Error('[PEPE V19.1] RUNTIME CUFF SNAP');
  ...
  function attachHandToArm(handName, armName) {
    ...
    if (!hand || !arm || !cfg) {
      throw new Error(
        `[PEPE V19] missing ${handName}/${armName}`
      );
    }

    if (cfg.armName !== armName) {
      throw new Error(
        `[PEPE V19] arm mapping mismatch for ${handName}`
      );
    }

    /*
      Resolve the real wrist socket inside the arm image.
    */
    ...
    /*
      Resolve the real cuff center inside the hand image.
    */
    ...
    /*
      Move the hand artwork inside its own pivot so
    ...
    */
    ...
    /*
      The hand pivot itself now sits directly inside
      the wrist joint.
    */
    ...
    /*
      Preserve the original authored world-facing angle.

      Screen-left:
        hand = -10 deg
        arm  = +28 deg
        local wrist rotation = -38 deg
    ...
    */
    ...
    /*
      Keep each hand behind its sleeve so the sleeve
      naturally hides the connection seam.
    */
    ...
    /*
      V19_1_RUNTIME_CUFF_SNAP

      Resolve the actual cuff point after all parenting,
      scaling and rotation have already been applied.

      Then measure the cuff position in wrist-joint local
      coordinates and translate the hand by the exact
      opposite vector.

      Result:
          wrist joint origin == hand cuff center

      No guessed X/Y offsets.
      No guessed wrist angles.
      No world-space hand dragging.
    */
    hand.pivot.updateMatrixWorld(true);
    wristJoint.updateMatrixWorld(true);

    const liveCuffLocal = sourcePixelToLocal(
      hand,
      cfg.handCuff[0],
      cfg.handCuff[1]
    );

    const liveCuffWorld = liveCuffLocal.clone();
    hand.pivot.localToWorld(liveCuffWorld);

    const cuffInsideWrist = liveCuffWorld.clone();
    wristJoint.worldToLocal(cuffInsideWrist);

    hand.pivot.position.sub(cuffInsideWrist);
    ...
    /*
      Validation point:
      after correction the cuff must be extremely close
      to the wrist-joint origin.
    */
    const verifyCuffLocal = sourcePixelToLocal(
      hand,
      cfg.handCuff[0],
      cfg.handCuff[1]
    );

    const verifyCuffWorld = verifyCuffLocal.clone();
    hand.pivot.localToWorld(verifyCuffWorld);

    const verifyInsideWrist = verifyCuffWorld.clone();
    wristJoint.worldToLocal(verifyInsideWrist);

    if (verifyInsideWrist.length() > 0.01) {
      console.warn(
        '[PEPE V19.1] cuff snap residual',
        handName,
        verifyInsideWrist.toArray()
      );
    }
  ...
  const ready = Promise.all(defs.map(addPart)).then(() => {
    ...
    console.log('[PEPE V19.1] RUNTIME CUFF SNAP');
    return true;
  });
