...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  function attachHandToArm(handName, armName) {
    ...
    */
    ...
    /*
      V34
      Move the WHOLE hand outward along the actual
      forearm/sleeve axis.

      wristLocal is already expressed in arm-pivot
      coordinates, so it gives us the real direction
      from the arm pivot toward the sleeve opening.

      This does NOT use the PNG center anymore.
    */
    const sleeveOutDirection =
      wristLocal.clone();

    sleeveOutDirection.z = 0;

    if (sleeveOutDirection.lengthSq() > 1e-8) {
      sleeveOutDirection.normalize();

      hand.pivot.position.addScaledVector(
        sleeveOutDirection,
        0.035
      );
    }
