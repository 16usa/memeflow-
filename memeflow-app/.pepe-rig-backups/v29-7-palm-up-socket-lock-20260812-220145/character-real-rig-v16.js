...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  */
  ...
  function update(t, dt=1/60) {
    ...
    /* V29_6_FORCE_LEFT_HAND_UP_BEGIN */

    /*
      Force the screen-left hand into the final
      palm-up orientation after every setup patch
      has already completed.
    */

    const leftHand = parts.handLeft;

    if (leftHand && leftHand.pivot) {
      if (!leftHand._v296PoseCaptured) {
        leftHand._v296BasePosition =
          leftHand.pivot.position.clone();

        leftHand._v296BaseRotation =
          leftHand.pivot.rotation.z;

        leftHand._v296PoseCaptured = true;
      }

      const leftHandLift = 0.055;
