...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  const defs = [
    ...
    {n:'handLeft',  f:'hand_right.png', c:[435,570], s:.52, r:-10, p:[420,545], z:.30, order:38},
    {n:'handRight', f:'hand_left.png',  c:[565,570], s:.52, r: 10, p:[580,545], z:.31, order:39},
  ...
  function attachWrist(handName, armName) {
    const hand = parts[handName];
    const arm = parts[armName];

    if (!hand || !arm) {
      throw new Error(`[PEPE V16.4] Missing wrist pair: ${handName} -> ${armName}`);
    }
    ...
    hand.baseX = hand.pivot.position.x;
    hand.baseY = hand.pivot.position.y;
    hand.baseRot = hand.pivot.rotation.z;
  }

  const ready = Promise.all(defs.map(addPart)).then(() => {
    attachWrist('handLeft', 'armLeft');
    attachWrist('handRight', 'armRight');
    console.log('[PEPE V16.4] WRISTS ATTACHED · LEGS SWAPPED · REAL CANVAS');
    return true;
  });
  ...
  function update(t, dt=1/60) {
    ...
    // Hands are true wrist children now. Keep the seam locked.
    parts.handLeft.pivot.position.x = parts.handLeft.baseX;
    parts.handLeft.pivot.position.y = parts.handLeft.baseY;
    parts.handRight.pivot.position.x = parts.handRight.baseX;
    parts.handRight.pivot.position.y = parts.handRight.baseY;

    // Small counter-rotation keeps the palms readable without breaking the wrist seam.
    parts.handLeft.pivot.rotation.z =
      parts.handLeft.baseRot - armBob * .18 - brace * .10;
    parts.handRight.pivot.rotation.z =
      parts.handRight.baseRot + armBob * .18 + brace * .10;
