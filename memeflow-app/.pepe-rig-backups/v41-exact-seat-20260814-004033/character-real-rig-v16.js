...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  const wristSockets = {
    handLeft: {
      ...
      */
      ...
      // V40: measured directly from PNG alpha silhouette.
      armAxis:  [0.75554010, 0.65510240],
      handAxis: [-0.69817049, 0.71593154]
    },
    ...
    handRight: {
      ...
      */
      ...
      // V40: measured independently from right-side PNGs.
      armAxis:  [-0.80203964, 0.59727080],
      handAxis: [0.70054707, 0.71360619]
    }
  };
  ...
  function attachHandToArm(handName, armName) {
    ...
    */
    ...
    /*
      V40
      Forward extraction follows the measured forearm axis.
      No PNG-center guess.
    */
    const sleeveAxisLocal =
      new THREE.Vector2(
        cfg.armAxis[0],
        -cfg.armAxis[1]
      ).normalize();

    const handForward = 0.03;

    hand.pivot.position.set(
      sleeveAxisLocal.x * handForward,
      sleeveAxisLocal.y * handForward,
      0
    );
    ...
    */
    /*
      V40 — NO MANUAL DEGREES.

      handAxis is measured in source PNG coordinates.
      Convert image Y-down into Three Y-up.

      Then rotate that measured hand axis onto exact
      WORLD UP while accounting for arm.baseRot.
    */
    ...
    const sourceAxisAngle =
      Math.atan2(
        handAxisLocal.y,
        handAxisLocal.x
      );

    const targetAxisAngle =
      Math.atan2(
        targetLocalUp.y,
        targetLocalUp.x
      );
    ...
    console.log(
      ...
      {
        armAxis: cfg.armAxis,
        handAxis: cfg.handAxis,
        derivedLocalRotation:
          THREE.MathUtils.radToDeg(localHandAngle),
        forward: handForward
      }
    );
