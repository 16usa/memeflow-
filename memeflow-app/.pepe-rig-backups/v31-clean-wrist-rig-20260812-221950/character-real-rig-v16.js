...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  function attachHandToArm(handName, armName) {
    ...
    /*
      Resolve the real sleeve socket directly
      from the authored arm image.
    */
    ...
    /*
      Resolve the real hand cuff directly
      from the authored hand image.
    */
    ...
    /*
      Move the hand artwork so its cuff point
      becomes the exact local origin.
    */
    ...
    /*
      The pivot itself lives exactly inside
      the sleeve socket.
    */
    ...
    /*
      ...
      Keep the authored mirrored forearm relation,
      then flip the hand exactly 180 degrees.

      Screen-left:
          local = -56 + 180 = 124 deg

      Screen-right:
          local = +56 + 180 = 236 deg

      Both cuffs remain at their own sockets.
    */
    ...
    /*
      The real sleeve stays in front of the hand
      and hides the cuff seam naturally.
    */
    ...
    /*
      Exact final cuff verification.
      No handSeat offsets.
      No bridge.
      No synthetic rim.
      No runtime X/Y corrections.
    */
  ...
  const ready = Promise.all(defs.map(addPart)).then(() => {
    ...
    console.log(
      '[PEPE V31] CLEAN WRIST HIERARCHY'
    );
