...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  function attachHandToArm(handName, armName) {
    ...
    */
    ...
    /*
      V33 — derive rotation from the REAL hand artwork.

      hand.mesh.position is the authored vector from
      the cuff pivot toward the palm/fingers.

      Do not guess a sprite rotation angle anymore.
      Instead rotate that real artwork axis toward a
      controlled WORLD direction:

        screen-left  -> 100 deg = up + slightly outward
        screen-right ->  80 deg = up + slightly outward

      This keeps both palms upright without making
      the fingers collide in the center.
    */

    const naturalHandAxisAngle =
      Math.atan2(
        hand.mesh.position.y,
        hand.mesh.position.x
      );

    const desiredWorldAxisDeg =
      handName === 'handLeft'
        ? 100
        : 80;

    const desiredWorldAxisAngle =
    ...
    /*
      ...
      Therefore solve the local rotation directly.
    */

    const localHandAngle =
      desiredWorldAxisAngle -
      arm.baseRot -
      naturalHandAxisAngle;
