...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  /*
    V35
    Correct 2.5D wrist compositing.

    The full palm must render IN FRONT of the arm.
    Only the physical green rim of the sleeve opening
    renders in front of the palm base.

    We generate this foreground rim directly from
    the existing arm PNG. No new asset is required.
  */
  function createSleeveRimOverlay(handName, arm, cfg) {
    const sourceTexture =
      arm.mesh.material &&
      arm.mesh.material.map;
    ...
    if (
      ...
      throw new Error(
        '[PEPE V35] sleeve source texture unavailable'
      );
    }

    /*
      Small source-pixel window around the authored
      wrist-socket point.
    */
    const cropW = 60;
    const cropH = 44;
    ...
    /*
      Turn the crop into an elliptical sleeve RIM.

      The middle of the socket becomes transparent,
      so the hand can visibly emerge through it.
    */
    const pixels =
      ctx.getImageData(
        0,
    ...
    for (
      ...
      for (
        ...
        /*
          Outside outer ellipse = transparent.
          Inside socket hole = transparent.
        */
        if (
          outer > 1.0 ||
          inner < 1.0
    ...
    rimTexture.colorSpace =
      THREE.SRGBColorSpace;

    rimTexture.minFilter =
      THREE.LinearFilter;

    rimTexture.magFilter =
      THREE.LinearFilter;

    rimTexture.generateMipmaps =
      false;

    const armTextureWidth =
      image.width;

    const armTextureHeight =
      image.height;

    const armGeoWidth =
      arm.mesh.geometry.parameters.width;

    const armGeoHeight =
      arm.mesh.geometry.parameters.height;

    const rimGeo =
      new THREE.PlaneGeometry(
        armGeoWidth *
          (cropW / armTextureWidth),

        armGeoHeight *
          (cropH / armTextureHeight)
      );
    ...
    /*
      Position the cropped piece at the exact same
      source location from which it came.
    */
    const cropCenterLocal =
      sourcePixelToLocal(
        arm,
        sx + cropW / 2,
        sy + cropH / 2
  ...
  function attachHandToArm(handName, armName) {
    ...
    */
    ...
    /*
      V35 sleeve foreground.
      Created from the REAL arm texture.
    */
    hand.sleeveRim =
      createSleeveRimOverlay(
        handName,
        arm,
        cfg
      );
    ...
    */
    ...
    /*
      V34
      Keep the successful V32.4 hand orientation.

      Do NOT calculate orientation from PNG center.
      Only the sleeve-axis extension is new.
    */
    ...
    */
    ...
    if (handName === 'handLeft') {
      ...
      if (hand.sleeveRim) {
        hand.sleeveRim.renderOrder = 44;
      }
    } else {
      arm.mesh.renderOrder = 41;
      hand.mesh.renderOrder = 43;

      if (hand.sleeveRim) {
        hand.sleeveRim.renderOrder = 45;
      }
    }
