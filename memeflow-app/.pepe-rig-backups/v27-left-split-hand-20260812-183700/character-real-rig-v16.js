  ...
  function installLeftSplitHandV27() {
    ...
    */
    ...
    /*
      ...
      Move ONLY the screen-left hand deeper into
      its sleeve along the real forearm axis.

      wristJoint.position is the wrist location
      relative to the shoulder pivot.

      Therefore:
          -wristJoint.position
      points from the wrist back toward the shoulder.
    */

    const towardShoulder =
      hand.wristJoint.position
        .clone()
        .multiplyScalar(-1);
    ...
    if (
      towardShoulder.lengthSq() < 0.000001
    ) {
      throw new Error(
        '[PEPE V28] invalid forearm axis'
      );
    }

    towardShoulder.normalize();

    /*
      Insertion depth in Three.js scene units.
      This is not screen X/Y guessing.
    */
    const insertionDepth = 0.082;

    hand.pivot.position.copy(
      towardShoulder.multiplyScalar(
        insertionDepth
      )
    );
    ...
    console.log(
      '[PEPE V28] LEFT HAND INSERTED ALONG FOREARM AXIS'
    );


    console.log(
      '[PEPE V28] LEFT AXIS INSERT ACTIVE'
    );
  }
