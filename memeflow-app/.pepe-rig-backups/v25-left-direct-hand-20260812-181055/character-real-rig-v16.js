...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  if (!parent) throw new Error('[PEPE V24] LEFT WRIST SANDWICH');
  ...
  */
  ...
  function installLeftWristSandwichV24() {
    const arm =
      parts.armLeft;
    ...
    if (
      ...
      throw new Error(
        '[PEPE V24] left wrist hierarchy unavailable'
      );
    }

    /*
      Layer 1: original arm.
    */
    arm.mesh.renderOrder = 40;

    /*
      Layer 2: actual hand.
      The hand must be above the arm body.
    */
    hand.mesh.renderOrder = 41;
    ...
    /*
      Layer 3: small wrist bridge.
      It fills the dark cuff center.
    */
    const bridgeTexture =
      createWristBridgeTextureV24();
    ...
    /*
      Layer 4: original green sleeve rim.
      Only the rim is rendered above the hand.
    */
    const rimTexture =
    ...
    console.log(
      '[PEPE V24] LEFT WRIST SANDWICH ACTIVE'
    );
  }
  ...
  const ready = Promise.all(defs.map(addPart)).then(() => {
    ...
    */
    installLeftWristSandwichV24();

    console.log('[PEPE V24] LEFT WRIST SANDWICH');
    return true;
  });
