...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  if (!parent) throw new Error('[PEPE V25] LEFT DIRECT HAND');
  ...
  */
  ...
  function installLeftDirectHandV25() {
    ...
    if (
      ...
      throw new Error(
        '[PEPE V25] left wrist hierarchy unavailable'
      );
    }

    /*
      FINAL LEFT WRIST LAYER STACK

      arm body   = 40
      real hand  = 42
      sleeve rim = 43

      No synthetic wrist bridge.
      No green ball.
      No X/Y change.
      No rotation change.
    */
    ...
    /*
      Use only the original green sleeve rim
      above the real hand.
    */
    const rimTexture =
      createLeftSleeveRimTextureV24(arm);
    ...
    console.log(
      '[PEPE V25] LEFT DIRECT HAND ACTIVE'
    );
  }
  ...
  const ready = Promise.all(defs.map(addPart)).then(() => {
    ...
    */
    installLeftDirectHandV25();

    console.log('[PEPE V25] LEFT DIRECT HAND');
    return true;
  });
