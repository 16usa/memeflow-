...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  function attachHandToArm(handName, armName) {
    ...
    if (!hand || !arm) {
      throw new Error(
        `[PEPE V20] missing ${handName}/${armName}`
      );
    }

    /*
      The arm pivot is the shoulder.

      arm.mesh.position is the vector from the
      shoulder pivot to the visual center of the arm.

      For a cutout limb, the distal endpoint is the
      same vector continued one more time.

      shoulder -> arm center -> wrist
    */
    const wristLocal =
      arm.mesh.position
        .clone()
    ...
    /*
      The authored hand pivot is the attachment point.

      Do not solve another cuff point.
      Do not move the hand image independently.
    */
    ...
    /*
      Preserve the original world-facing hand angle
      while converting it to arm-local rotation.
    */
    ...
    /*
      The sleeve stays in front of the hand and hides
      the attachment seam.
    */
