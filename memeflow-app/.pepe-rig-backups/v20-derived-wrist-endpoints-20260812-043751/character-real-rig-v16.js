...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  function attachHandToArm(handName, armName) {
    ...
    if (!hand || !arm || !cfg) {
      throw new Error(
        `[PEPE V19.2 CLEAN] missing ${handName}/${armName}`
      );
    }

    if (cfg.armName !== armName) {
      throw new Error(
        `[PEPE V19.2 CLEAN] arm mapping mismatch for ${handName}`
      );
    }
    ...
    /*
      Single canonical cutout draw order.
      Hands stay behind sleeves.
    */
    ...
    /*
      Final cuff verification and correction.
    */
