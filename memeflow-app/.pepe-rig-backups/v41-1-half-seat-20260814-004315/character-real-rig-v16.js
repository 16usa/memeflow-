  ...
  */
  function detectFingerAxis(part, cuffPixel) {
    ...
    if (!image || !image.width || !image.height) {
      throw new Error(
        '[PEPE V41] hand image unavailable'
      );
    }
    ...
    if (!ctx) {
      throw new Error(
        '[PEPE V41] canvas context unavailable'
      );
    }
    ...
    if (points.length < 50) {
      throw new Error(
        '[PEPE V41] hand silhouette unavailable'
      );
    }
