...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  if (!parent) throw new Error('[PEPE V16.8] EXACT WRIST SOCKETS · LEGS SWAPPED · REAL CANVAS');
  ...
  function attachHandToArm(handName, armName) {
    ...
    if (!hand || !arm) {
      throw new Error(`[PEPE V16.8] EXACT WRIST SOCKETS · LEGS SWAPPED · REAL CANVAS'handLeft') {
  ...
  const ready = Promise.all(defs.map(addPart)).then(() => {
    ...
    console.log('[PEPE V16.8] EXACT WRIST SOCKETS · LEGS SWAPPED · REAL CANVAS');
    return true;
  });
