...
export function createPepeSkeletonV6({
  parent,
  baseUrl = '/game-assets/character/'
} = {}) {

  if (!parent) {
    throw new Error('[PEPE V6] parent required');
  }
  ...
  const ready = base.ready.then(() => {
    ...
    */
    ...
    parts.handLeft.mesh.scale.setScalar(1.28);

    parts.handRight.mesh.position.x -= 25 * UNIT;
    parts.handRight.mesh.position.y += 92 * UNIT;
    parts.handRight.mesh.scale.setScalar(1.28);
