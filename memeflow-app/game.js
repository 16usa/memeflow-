...
function fillToken(s){const t=s?.token||{};const symbol=text(t.symbol,t.name,'?').slice(0,8);renderTokenAvatar(s,t,symbol);ui.tokenName.textContent=text(t.name,t.symbol,'Filtered launch');ui.tokenMint.textContent=shortMint(s?.mint||t.mint||'');ui.tokenScore.textContent=num(t.score)===null?'—':Math.round(num(t.score));ui.tokenHolders.textContent=num(t.holderCount)===null?'—':Math.round(num(t.holderCount)).toLocaleString();ui.tokenTop10.textContent=num(t.top10Pct)===null?'—':`${num(t.top10Pct).toFixed(1)}%`;ui.tokenPressure.textContent=num(t.buyPressure)===null?'—':`${num(t.buyPressure).toFixed(2)}×`;ui.tokenState.textContent=s?.state==='LIVE'?'LOCKED':'SETTLED';const q=num(t.launchQuality,s?.marketShapeAtEntry?.quality),vol=num(t.volatilityPct,s?.marketShapeAtEntry?.volatilityPct),dd=num(t.recentDrawdownPct,s?.marketShapeAtEntry?.drawdownPct);ui.quality.textContent=q===null?text(t.primaryReason,'Passed active MEMEFLOW filters'):`Launch quality ${Math.round(q)}/100${vol===null?'':` · vol ${vol.toFixed(1)}%`}${dd===null?'':` · recent DD ${dd.toFixed(1)}%`}`;}
  function resetToken(){if(ui.roundId)ui.roundId.textContent='—';ui.tokenState.textContent='WAITING';ui.tokenAvatar.textContent='?';ui.tokenName.textContent='Ready for next launch';ui.tokenMint.textContent='Press START to scan your current BUY READY decisions.';ui.quality.textContent='MEMEFLOW settings control launch eligibility';ui.tokenScore.textContent='—';ui.tokenHolders.textContent='—';ui.tokenTop10.textContent='—';ui.tokenPressure.textContent='—';ui.feedQuality.textContent='IDLE';ui.feedAge.textContent='—';ui.priceAgeStrip.textContent='—';ui.selectorScore.textContent='—';ui.decisionAge.textContent='—';if(ui.holderAge)ui.holderAge.textContent='—';ui.autoDistance.textContent='—';ui.stopDistance.textContent='—';if(ui.flightAssist){ui.flightAssist.dataset.tone='neutral';ui.flightAssistState.textContent='STANDBY';ui.flightAssistText.textContent='Waiting for a server-locked launch.';}if(game.mode!=='searching'){setSelectorState('idle');ui.selectorStatus.dataset.step='decision';ui.selectorTitle.textContent='Selector ready';ui.selectorText.textContent='Uses your MEMEFLOW BUY READY decisions · no extra Game filters';}}
  ...
  async function startRound(){
    ...
    ui.result.hidden=true;ui.game.inert=false;ui.resultCard.dataset.tone='';game.showingResult=null;game.countdownSeq++;resetRoundPresentation({pulse:true});game.searchSeq++;const seq=game.searchSeq;game.requestId=requestId();game.searchStartedAt=Date.now();game.startCancelArmedAt=game.searchStartedAt+700;game.selectorDiag=null;setSelectorState('searching','SCAN');ui.selectorStatus.dataset.step='decision';if(ui.roundId)ui.roundId.textContent='SEARCH';ui.tokenState.textContent='SCANNING';ui.tokenName.textContent='Scanning for next launch…';ui.tokenMint.textContent='Checking BUY READY decisions from your saved settings.';ui.selectorTitle.textContent='Radar scanning';ui.selectorText.textContent='Checking current BUY READY decisions…';setMode('searching','Waiting for a BUY READY launch from your MEMEFLOW trading settings…');
  ...
  function showResult(s){
    if(!s||s.state!=='COMPLETE'||game.showingResult===s.id)return;

    /*
      V10.4:
      AUTO owns completed-round presentation while an AUTO
      session is active. The normal one-round result screen
      remains unchanged for MANUAL play.
    */
    try{
      if(
        globalThis.memeflowAutoPlay?.handleRoundResult?.(s)===true
      ){
        game.countdownSeq++;
        game.pendingResultId=null;

        ui.center.hidden=true;

        clearTimeout(showMilestone.timer);
        ui.milestone.hidden=true;

        game.showingResult=s.id;

        game.resultFxSeq++;

        ui.result.hidden=true;
        ui.game.inert=false;
        ui.resultCard.dataset.tone='';
        ui.game.dataset.cashpulse='idle';

        scheduleRoundSummaryRefresh();
        return;
      }
    }catch(error){
      console.error(
