...
async function saveSettings(mode = null) {
  ...
  const next = {
    ...
    maxPositionSize: Math.max(num(state.settings.maxPositionSize, sizeSol), sizeSol),
...
async function loadCandidates() {
  const payload = await api('/api/ai/decisions?scope=candidates&limit=100');
...
function rawPoints(mint) {
  if (!state.rawByMint.has(mint)) {
    let points = [];
    try {
      const saved = JSON.parse(sessionStorage.getItem(`mf30:${mint}`) || '[]');
      if (Array.isArray(saved)) points = saved.filter(p => finite(p.t) && finite(p.price)).slice(-1200);
    } catch {}
...
function addPoint(mint, t, price) {
  ...
  if (last && Math.abs(last.t - ts) < 350 && last.price === p) return;

  points.push({ t: ts, price: p });
  if (points.length > 1600) points.splice(0, points.length - 1600);

  try {
    sessionStorage.setItem(`mf30:${mint}`, JSON.stringify(points.slice(-900)));
  } catch {}
...
function connectChartStream(mint) {
  ...
  const source = new EventSource(`/api/chart/stream?tokenAddress=${encodeURIComponent(mint)}`);
  source.__mint = mint;
  const handler = event => {
    try {
      const payload = JSON.parse(event.data || '{}');
      const points = [];
      if (payload.point) points.push(payload.point);
      if (Array.isArray(payload.points)) points.push(...payload.points);
      for (const point of points) addPoint(mint, point.t || Date.now(), point.price);
    } catch {}
  };
  ...
  source.onerror = () => { $('feedState').textContent = 'RECONNECTING'; };
  source.onopen = () => { $('feedState').textContent = 'LIVE'; };
...
function candlesFor(points, timeframe) {
  ...
  if (timeframe === 'all') {
    ...
    interval = Math.max(1000, Math.ceil(span / 90));
  } else {
  ...
  for (const point of points) {
    const bucket = Math.floor(point.t / interval) * interval;
    let candle = buckets.get(bucket);
    if (!candle) {
      candle = { t: bucket, open: point.price, high: point.price, low: point.price, close: point.price };
  ...
  return [...buckets.values()].sort((a, b) => a.t - b.t).slice(-140);
}
...
function drawChart() {
  ...
  const pad = { left: 10, right: 72, top: 12, bottom: 22 };
  ...
  if (!(max > min)) {
    const spread = Math.max(max * .01, 1e-12);
    min -= spread;
    max += spread;
  }
  ...
  const bodyW = Math.max(1.3, Math.min(10, xStep * .62));
  ...
  for (let i = 0; i <= 5; i++) {
    ...
    ctx.strokeStyle = 'rgba(106,145,162,.08)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(pad.left, yy);
    ctx.lineTo(W - pad.right, yy);
    ctx.stroke();
    ctx.fillStyle = '#4e6874';
    ctx.fillText(formatPrice(price), W - pad.right + 6, yy);
  }

  candles.forEach((candle, i) => {
    const xx = pad.left + xStep * (i + .5);
    const up = candle.close >= candle.open;
    const color = up ? '#4de6a1' : '#ff6679';
    ...
    ctx.fillRect(xx - bodyW / 2, top, bodyW, bodyH);
  });
  ...
  for (const level of levels) {
    ...
    ctx.globalAlpha = .88;
    ctx.fillStyle = color;
    ctx.fillText(level.label, pad.left + 5, Math.max(8, Math.min(H - 8, yy - 6)));
  }
