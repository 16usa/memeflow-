...
http.createServer=function patchedCreateServer(listener,...rest){
  ...
  const wrapped=async(req,res)=>{
    ...
    if(req.method==='POST'&&pathname==='/api/live/confirm'){
      ...
      const body=await readJson(req).catch(error=>({__error:error}));if(body.__error)return json(res,body.__error.status||400,{error:body.__error.code||'INVALID_REQUEST',message:body.__error.message});
