...
(() => {
  ...
  const PATCH = 'MEMEFLOW_DISCOVERY_PLATFORM_DROPDOWN_V2';
  if (window.__mfDiscoveryPlatformDropdownV2) return;
  window.__mfDiscoveryPlatformDropdownV2 = true;

  const PUMP_ONLY_RULES = [
    {id:'bonding',test:t=>/\bbonding\s*curve\b/i.test(t),dex:'Pump.fun only · not used for DEX tokens',hybrid:'Pump.fun only · ignored on the DEX branch'},
    {id:'fees',test:t=>/\btotal\s+fees?\b/i.test(t),dex:'Pump.fun only · not used for DEX tokens',hybrid:'Pump.fun only · ignored on the DEX branch'},
    {id:'developer',test:t=>/\bdeveloper\b/i.test(t)&&(/\bshare\b|\bconcentration\b|\bwallet|\bblacklist\b|\bminimum\b|\bmaximum\b|\bmin\b|\bmax\b|%/i.test(t)),dex:'Creator identity is unavailable from the DEX pool · inactive',hybrid:'Applied to Pump.fun tokens only · ignored on DEX tokens'},
    {id:'bundle',test:t=>/\bbundle\b/i.test(t),dex:'No DEX bundle signal in this build · inactive',hybrid:'No DEX bundle signal · Pump branch only'},
    {id:'sniper',test:t=>/\bsniper\b/i.test(t),dex:'No DEX sniper signal in this build · inactive',hybrid:'No DEX sniper signal · Pump branch only'}
  ];

  const state={mode:null,busy:false,source:null,dex:null,pump:null,card:null,value:null,menu:null,observer:null,poll:null,mountTimer:null,compatibilityTimer:null};

  const css=`
  #mfds-panel-v1{display:none!important}
  .mfdx-platform-card-v2{position:relative!important;cursor:pointer!important;-webkit-tap-highlight-color:transparent;touch-action:manipulation;user-select:none;transition:border-color .14s ease,background .14s ease,box-shadow .14s ease,transform .08s ease}
  .mfdx-platform-card-v2:active{transform:scale(.987)}
  .mfdx-platform-card-v2::after{content:"";position:absolute;right:15px;top:50%;width:7px;height:7px;margin-top:-5px;border-right:1.5px solid #6f8995;border-bottom:1.5px solid #6f8995;transform:rotate(45deg);pointer-events:none;transition:transform .14s ease,margin-top .14s ease,border-color .14s ease}
  .mfdx-platform-card-v2.mfdx-menu-open::after{transform:rotate(225deg);margin-top:0;border-color:#53dffb}
  .mfdx-platform-card-v2.mfdx-menu-open{border-color:rgba(60,194,224,.52)!important;box-shadow:inset 0 0 0 1px rgba(60,194,224,.06),0 0 22px rgba(24,163,197,.07)}
  #mfdx-platform-menu-v2{position:fixed;z-index:2147483000;width:min(300px,calc(100vw - 24px));padding:6px;border:1px solid #203842;border-radius:16px;background:linear-gradient(180deg,rgba(6,15,20,.985),rgba(2,8,12,.995));box-shadow:0 18px 55px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.025);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);transform-origin:top center;animation:mfdxMenuIn .12s ease-out}
  @keyframes mfdxMenuIn{from{opacity:0;transform:translateY(-4px) scale(.985)}to{opacity:1;transform:translateY(0) scale(1)}}
  .mfdx-menu-option-v2{width:100%;min-height:54px;display:grid;grid-template-columns:1fr auto;align-items:center;gap:10px;padding:9px 11px;border:1px solid transparent;border-radius:11px;background:transparent;color:#8fa6b0;text-align:left;font:inherit;outline:none;-webkit-tap-highlight-color:transparent;touch-action:manipulation}
  .mfdx-menu-option-v2+.mfdx-menu-option-v2{margin-top:2px}.mfdx-menu-option-v2:active{background:rgba(55,184,216,.07)}
  .mfdx-menu-option-v2.active{color:#edfaff;border-color:rgba(51,197,231,.24);background:rgba(36,151,177,.08)}
  .mfdx-menu-option-v2[data-mode="dex"].active{border-color:rgba(65,225,162,.25);background:rgba(39,170,117,.08)}
  .mfdx-menu-option-v2[data-mode="hybrid"].active{border-color:rgba(132,113,255,.25);background:rgba(102,82,212,.08)}
  .mfdx-menu-name-v2{display:block;color:inherit;font-size:13px;line-height:1.2;font-weight:740;letter-spacing:.01em}
  .mfdx-menu-desc-v2{display:block;margin-top:3px;color:#5f7884;font-size:9px;line-height:1.25;font-weight:500;letter-spacing:.02em}
  .mfdx-menu-check-v2{width:18px;height:18px;display:grid;place-items:center;border:1px solid #29414b;border-radius:50%;color:transparent;font-size:11px;font-weight:900}
  .mfdx-menu-option-v2.active .mfdx-menu-check-v2{color:#07110e;border-color:#47e5ac;background:#47e5ac;box-shadow:0 0 12px rgba(71,229,172,.25)}
  .mfdx-menu-option-v2[disabled]{opacity:.5}
  .mfdx-compat-card-v2{position:relative!important;transition:border-color .16s ease,background .16s ease,filter .16s ease}
  .mfdx-compat-card-v2.mfdx-dex-inactive-v2{border-color:rgba(101,124,134,.16)!important;background-color:rgba(3,8,11,.72)!important;filter:saturate(.35)}
  .mfdx-compat-card-v2.mfdx-dex-inactive-v2>*:not(.mfdx-compat-note-v2){opacity:.40!important}
  .mfdx-compat-card-v2.mfdx-hybrid-note-v2{border-color:rgba(103,87,186,.18)!important}
  .mfdx-compat-card-v2.mfdx-hybrid-note-v2>*:not(.mfdx-compat-note-v2){opacity:.78}
  .mfdx-compat-note-v2{display:block;width:100%;margin-top:8px;padding-top:7px;border-top:1px solid rgba(102,126,136,.11);color:#657d88;font-size:8.5px;line-height:1.3;letter-spacing:.035em;opacity:1!important}
  .mfdx-dex-inactive-v2 .mfdx-compat-note-v2{color:#718791}.mfdx-hybrid-note-v2 .mfdx-compat-note-v2{color:#8176ae}
  .mfdx-compat-card-v2.mfdx-block-events-v2 input,.mfdx-compat-card-v2.mfdx-block-events-v2 select,.mfdx-compat-card-v2.mfdx-block-events-v2 textarea,.mfdx-compat-card-v2.mfdx-block-events-v2 button,.mfdx-compat-card-v2.mfdx-block-events-v2 [role="switch"],.mfdx-compat-card-v2.mfdx-block-events-v2 [role="slider"],.mfdx-compat-card-v2.mfdx-block-events-v2 [contenteditable="true"]{pointer-events:none!important}
  .mfdx-platform-busy-v2{opacity:.62!important;pointer-events:none!important}
  @media(max-width:560px){#mfdx-platform-menu-v2{width:min(286px,calc(100vw - 20px))}.mfdx-menu-option-v2{min-height:52px}}
  `;

  function installStyle(){if(document.getElementById('mfdx-style-v2'))return;const s=document.createElement('style');s.id='mfdx-style-v2';s.textContent=css;(document.head||document.documentElement).appendChild(s)}
  function cleanText(el){return(el?.innerText||el?.textContent||'').replace(/\s+/g,' ').trim()}
  function leaves(root=document.body){return Array.from(root.querySelectorAll('*')).filter(el=>{if(el.id==='mfdx-platform-menu-v2'||el.closest?.('#mfdx-platform-menu-v2')||el.classList?.contains('mfdx-compat-note-v2'))return false;if(el.children.length)return false;return Boolean(cleanText(el))})}
  function leafExact(text){const wanted=String(text).trim().toLowerCase();return leaves().find(el=>cleanText(el).toLowerCase()===wanted)||null}

  function findPlatformCard(){
    const label=leafExact('Platform'); if(!label)return null;
    let n=label.parentElement,best=null;
    for(let depth=0;n&&n!==document.body&&depth<7;depth++,n=n.parentElement){
      const txt=cleanText(n).toLowerCase();
      if(!txt.includes('platform'))continue;
      if(txt.includes('ai policy')||txt.includes('kill switch'))break;
      const r=n.getBoundingClientRect?.();
      if(!r||r.width<90||r.height<42||r.height>150||txt.length>120)continue;
      best=n;
      const border=getComputedStyle(n).borderTopWidth;
      if(border&&border!=='0px')break;
    }
    return best;
  }

  function findPlatformValue(card){
    if(!card)return null;
    const options=leaves(card).filter(el=>cleanText(el).toLowerCase()!=='platform');
    const preferred=options.find(el=>['pump.fun','pump','dex','hybrid'].includes(cleanText(el).toLowerCase()));
    return preferred||options[options.length-1]||null;
  }

  const labelForMode=mode=>mode==='pump'?'Pump.fun':mode==='dex'?'DEX':mode==='hybrid'?'Hybrid':'—';

  function closeMenu(){state.menu?.remove?.();state.menu=null;state.card?.classList.remove('mfdx-menu-open')}

  function menuOption(mode,name,desc){
    const b=document.createElement('button');b.type='button';b.className='mfdx-menu-option-v2';b.dataset.mode=mode;
    b.innerHTML=`<span><span class="mfdx-menu-name-v2">${name}</span><span class="mfdx-menu-desc-v2">${desc}</span></span><span class="mfdx-menu-check-v2">✓</span>`;
    b.classList.toggle('active',mode===state.mode);b.disabled=state.busy;
    b.addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();switchMode(mode)});
    return b;
  }

  function positionMenu(){
    if(!state.menu||!state.card?.isConnected)return;
    const rect=state.card.getBoundingClientRect(),maxWidth=Math.min(300,window.innerWidth-24);
    let left=Math.max(12,Math.min(rect.left,window.innerWidth-maxWidth-12));
    let top=rect.bottom+7; const h=174;
    if(top+h>window.innerHeight-10)top=Math.max(10,rect.top-h-7);
    state.menu.style.left=`${Math.round(left)}px`;state.menu.style.top=`${Math.round(top)}px`;
    state.menu.style.width=`${Math.round(Math.min(maxWidth,Math.max(rect.width,220)))}px`;
  }

  function openMenu(){
    if(state.busy||!state.card)return;
    if(state.menu){closeMenu();return}
    const menu=document.createElement('div');menu.id='mfdx-platform-menu-v2';menu.setAttribute('role','menu');
    menu.append(menuOption('pump','Pump.fun','Earliest Pump create + trade stream'),menuOption('dex','DEX','Confirmed SOL-paired DEX pools'),menuOption('hybrid','Hybrid','Pump.fun and DEX in parallel'));
    document.body.appendChild(menu);state.menu=menu;state.card.classList.add('mfdx-menu-open');positionMenu();
  }

  function bindPlatformCard(){
    const card=findPlatformCard();if(!card)return false;
    state.card=card;state.value=findPlatformValue(card);
    card.classList.add('mfdx-platform-card-v2');card.setAttribute('role','button');card.setAttribute('tabindex','0');card.setAttribute('aria-label','Choose discovery source');
    if(card.dataset.mfdxBoundV2!=='1'){
      card.dataset.mfdxBoundV2='1';
      card.addEventListener('click',ev=>{if(ev.target.closest?.('#mfdx-platform-menu-v2'))return;ev.preventDefault();openMenu()});
      card.addEventListener('keydown',ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();openMenu()}});
    }
    updatePlatformValue();return true;
  }

  function updatePlatformValue(){
    if(!state.card?.isConnected)bindPlatformCard();
    state.value=findPlatformValue(state.card);
    if(state.value&&state.mode)state.value.textContent=labelForMode(state.mode);
    state.card?.classList.toggle('mfdx-platform-busy-v2',state.busy);
  }

  function minimalFieldCard(label){
    if(!label)return null;
    let n=label.parentElement,fallback=null;
    for(let depth=0;n&&n!==document.body&&depth<7;depth++,n=n.parentElement){
      if(n===state.card||n.closest?.('#mfdx-platform-menu-v2'))return null;
      const txt=cleanText(n);if(txt.length>360)break;
      const r=n.getBoundingClientRect?.();if(!r||r.width<100||r.height<42||r.height>240)continue;
      const controls=n.querySelectorAll('input,select,textarea,button,[role="switch"],[role="slider"],[contenteditable="true"]').length;
      const cs=getComputedStyle(n),hasBorder=parseFloat(cs.borderTopWidth||'0')>0||parseFloat(cs.borderLeftWidth||'0')>0;
      if(!fallback&&txt.length<220)fallback=n;
      if(hasBorder&&controls<=4&&txt.length<260)return n;
    }
    return fallback;
  }

  function matchingRule(text){const t=String(text||'').replace(/\s+/g,' ').trim();return PUMP_ONLY_RULES.find(rule=>rule.test(t))||null}
  function candidateLabels(){return leaves().filter(el=>{const t=cleanText(el);return t.length<=100&&Boolean(matchingRule(t))})}

  function blockEvent(ev){
    const card=ev.currentTarget;if(!card.classList.contains('mfdx-block-events-v2'))return;
    ev.preventDefault();ev.stopImmediatePropagation();
  }
  function bindBlocker(card){
    if(card.dataset.mfdxBlockerV2==='1')return;card.dataset.mfdxBlockerV2='1';
    for(const type of ['click','pointerdown','touchstart','keydown','input','change'])card.addEventListener(type,blockEvent,true);
  }

  function clearCompatibility(){
    document.querySelectorAll('.mfdx-compat-card-v2').forEach(card=>{
      card.classList.remove('mfdx-compat-card-v2','mfdx-dex-inactive-v2','mfdx-hybrid-note-v2','mfdx-block-events-v2');
      card.removeAttribute('aria-disabled');card.querySelector(':scope > .mfdx-compat-note-v2')?.remove();
    });
  }

  function applyCompatibility(){
    clearTimeout(state.compatibilityTimer);
    state.compatibilityTimer=setTimeout(()=>{
      const mode=state.mode;clearCompatibility();if(!mode||mode==='pump')return;
      const seen=new Set();
      for(const label of candidateLabels()){
        const rule=matchingRule(cleanText(label)),card=minimalFieldCard(label);
        if(!rule||!card||seen.has(card))continue;seen.add(card);
        card.classList.add('mfdx-compat-card-v2');card.dataset.mfdxCompatibility=rule.id;
        const note=document.createElement('div');note.className='mfdx-compat-note-v2';
        if(mode==='dex'){card.classList.add('mfdx-dex-inactive-v2','mfdx-block-events-v2');card.setAttribute('aria-disabled','true');note.textContent=rule.dex;bindBlocker(card)}
        else if(mode==='hybrid'){card.classList.add('mfdx-hybrid-note-v2');note.textContent=rule.hybrid}
  ...
  function render(){
    updatePlatformValue();applyCompatibility();
    if(state.menu){state.menu.querySelectorAll('.mfdx-menu-option-v2').forEach(btn=>{btn.classList.toggle('active',btn.dataset.mode===state.mode);btn.disabled=state.busy});positionMenu()}
  }

  async function loadStatus(){
    if(document.hidden)return;
    try{
      const r=await fetch('/api/discovery-source',{method:'GET',cache:'no-store',credentials:'same-origin',headers:{accept:'application/json'}});
      const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j?.message||j?.error||`HTTP ${r.status}`);
      state.source=j?.source||null;state.mode=String(j?.source?.mode||'').toLowerCase()||state.mode;state.dex=j?.dex||null;state.pump=j?.pump||null;render();
    }catch(e){console.warn('[DISCOVERY UI V2] status',e?.message||e)}
  }

  async function switchMode(mode){
    mode=String(mode||'').toLowerCase();if(!['pump','dex','hybrid'].includes(mode)||state.busy)return;
    if(mode===state.mode){closeMenu();return}
    const previous=state.mode;state.busy=true;state.mode=mode;closeMenu();render();
    try{
      const r=await fetch('/api/discovery-source',{method:'POST',cache:'no-store',credentials:'same-origin',headers:{'content-type':'application/json',accept:'application/json'},body:JSON.stringify({mode})});
      const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j?.message||j?.error||`HTTP ${r.status}`);
      state.source=j?.source||null;state.mode=String(j?.source?.mode||mode).toLowerCase();state.dex=j?.dex||null;state.pump=j?.pump||null;
    }catch(e){state.mode=previous;console.error('[DISCOVERY UI V2] switch failed',e)}
    finally{state.busy=false;render();setTimeout(loadStatus,450)}
  }

  function queueRefresh(){clearTimeout(state.mountTimer);state.mountTimer=setTimeout(()=>{bindPlatformCard();applyCompatibility()},70)}

  function start(){
    installStyle();document.getElementById('mfds-panel-v1')?.remove?.();bindPlatformCard();loadStatus();
    state.observer=new MutationObserver(queueRefresh);state.observer.observe(document.body,{childList:true,subtree:true});
    state.poll=setInterval(loadStatus,2500);
    document.addEventListener('click',ev=>{if(!state.menu||state.menu.contains(ev.target)||state.card?.contains(ev.target))return;closeMenu()},true);
    window.addEventListener('resize',()=>{if(state.menu)positionMenu()},{passive:true});
    window.addEventListener('scroll',()=>{if(state.menu)positionMenu()},{passive:true,capture:true});
    document.addEventListener('visibilitychange',()=>{if(!document.hidden){bindPlatformCard();loadStatus()}});
  }
