...
export function createPepeFaceController({parent,baseUrl='/game-assets/rocket-v34/face/',radius=0.315}={}){
  ...
  let idleMood='idle',idleMoodLeft=1.2;
  ...
  function chooseBase(state){
    ...
    if(boost>0.58)return'boost';
    if(d>0.72&&thrust>0.82)return'pump';
    if(d>0.18)return'up';

    // SHOCKED is reserved for a genuinely violent dump.
    if(d<-0.82&&vol>0.65)return'shocked';

    // Normal DOWN = worried face.
    if(d<-0.16)return'down';

    // Neutral flight occasionally changes between relaxed expressions.
    return idleMood;
  }
  function update(t,dt,state={}){
    ...
    idleMoodLeft-=dt;
    if(idleMoodLeft<=0&&Math.abs(d)<0.16&&boost<0.20){
      idleMood=Math.random()<0.34?'cruise':'idle';
      idleMoodLeft=0.9+Math.random()*2.4;
    }

    nextBlink-=dt;
    if(nextBlink<=0&&boost<0.25&&Math.abs(d)<0.75){
      blinkLeft=0.12+Math.random()*0.05;
