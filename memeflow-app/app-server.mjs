...
async function handler(req,res){const url=new URL(req.url,'http://x');
 ...
 if(url.pathname==='/api/discovery/status')return json(res,200,{...discovery,...discMetrics,rpcRetries:rpc.metrics.retries,rpcTimeouts:rpc.metrics.timeouts,processing:processing.size,metrics:store.state.metrics,tokens:store.tokens().length,users:Object.keys(store.state.users).length});
