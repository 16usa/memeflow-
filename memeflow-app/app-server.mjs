...
function __mfDropScannerToken(
  ...
  // MEMEFLOW_GLOBAL_INSTANT_PRUNE_V1
  // Capacity eviction is reversible RAM hygiene; real prune events create a
  // session tombstone that rejects late async writes and live-feed responses.
  const sessionPruned=skipStoreRemoval!==true;
...
// MEMEFLOW_FAST_HOLDER_PREVIEW_V4_VISIBLE_FIRST
//
// DISPLAY ONLY.
// Never writes canonical holderCount.
// Never affects Score, BUY READY, holder gates or trading decisions.
//
// Priority:
//   1. Visible cards from /api/system/live-token-card-batch
//   2. Remaining tokens by canonical V5 relevance:
//      OPEN POSITION > BUY READY > WATCH > WAITING > BLOCKED
//   3. Higher Score first inside the same lane.
//
// getTokenLargestAccounts is only a FAST preview:
//   <20 accounts -> "N"
//   20 accounts  -> "20+"
// Exact getProgramAccounts holderCount remains authoritative.

const __mfVisibleHolderMintsV4=new Map();
const __mfFastHolderAttemptAtV4=new Map();
const __mfFastHolderActiveV4=new Set();

const FAST_HOLDER_VISIBLE_TTL_MS_V4=10000;
const FAST_HOLDER_RETRY_MS_V4=5000;
const FAST_HOLDER_REFRESH_MS_V4=60000;
const FAST_HOLDER_MAX_CONCURRENT_V4=3;

function __mfTouchVisibleHolderMintsV4(mints=[]){
  const now=Date.now();
  ...
  for(let i=0;i<mints.length;i++){
    ...
    __mfVisibleHolderMintsV4.set(
      mint,
      {seenAt:now,order:i}
    );
  }

  for(const [mint,row] of __mfVisibleHolderMintsV4){
    if(
      !row ||
      now-Number(row.seenAt||0)>FAST_HOLDER_VISIBLE_TTL_MS_V4
    ){
      __mfVisibleHolderMintsV4.delete(mint);
    }
  }
}
...
function __mfFastHolderEligibleV4(token,visible,now){
  ...
  if(__mfFastHolderActiveV4.has(token.mint)){
    return false;
  }

  const lastAttempt=
    Number(__mfFastHolderAttemptAtV4.get(token.mint)||0);

  const lastSuccess=
    Number(token?.fastHolderPreviewAt||0);

  const hasPreview=
    typeof token?.fastHolderPreviewDisplay==='string' &&
    token.fastHolderPreviewDisplay.length>0;

  if(
    hasPreview &&
    lastSuccess &&
    now-lastSuccess<FAST_HOLDER_REFRESH_MS_V4
  ){
    return false;
  }
  ...
  const retryMs=
    visible
      ? FAST_HOLDER_RETRY_MS_V4
      : FAST_HOLDER_REFRESH_MS_V4;
...
function __mfFastHolderCandidatesV4(limit=FAST_HOLDER_MAX_CONCURRENT_V4){
  ...
  for(const [mint,meta] of __mfVisibleHolderMintsV4){
    if(
      now-Number(meta?.seenAt||0)>
      FAST_HOLDER_VISIBLE_TTL_MS_V4
    ){
      continue;
    }
...
async function __mfRunFastHolderPreviewV4(token){
  ...
  try{
    const result=await __mfPreOpenRpc.call(
      'getTokenLargestAccounts',
      [
        mint,
    ...
    const vals=(result?.value||[])
      .map(row=>Number(row?.uiAmountString))
    ...
    store.setToken(
      mint,
      {
        fastHolderPreviewDisplay:display,
        fastHolderPreviewAt:Date.now(),
        fastHolderPreviewSource:
          'getTokenLargestAccounts-v4-visible-first'
      }
    );
    ...
    __mfFastHolderActiveV4.delete(mint);
  }
}
...
/* MEMEFLOW_TOKEN_TRAJECTORY_MONITOR_V23_8
...
/* MEMEFLOW_TOKEN_INTELLIGENCE_SCORECARD_MONITOR_V23_15
...
/* MEMEFLOW_SHADOW_OUTCOME_REVIEW_MONITOR_V23_16
...
/* MEMEFLOW_SHADOW_ERROR_PATTERN_LEARNER_MONITOR_V23_17
...
/* MEMEFLOW_SHADOW_ERROR_AWARE_CONFIDENCE_MONITOR_V23_18
...
/* MEMEFLOW_SHADOW_POLICY_CANDIDATE_BUILDER_MONITOR_V23_20
...
/* MEMEFLOW_SHADOW_POLICY_SIMULATOR_MONITOR_V23_21
...
/* MEMEFLOW_SHADOW_POLICY_REVIEW_GATE_MONITOR_V23_22
...
/* MEMEFLOW_V23_E2E_READINESS_FREEZE_MONITOR_V23_23
 ...
 if(url.pathname==='/api/debug/token-lifecycle'){
    ...
    return json(res,200,{
      ...
      token:token?{
        ...
        fastHolderPreviewDisplay:token.fastHolderPreviewDisplay??null,
        fastHolderPreviewAt:token.fastHolderPreviewAt??null,
