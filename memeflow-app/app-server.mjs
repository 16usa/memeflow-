...
import {enrichToken,makeEnrichDiag} from './src/enrich.mjs';
...
// Delegates all per-step logic to src/enrich.mjs for testability and partial-result support.
async function enrich(mint,curve){
  return enrichToken(mint,curve,{rpc,store,tradeWindows,evaluateAll,publish,ensurePriceTimer,discMetrics,enrichDiag});
}
...
async function handler(req,res){const url=new URL(req.url,'http://x');
 ...
 if(url.pathname==='/api/discovery/status')return json(res,200,{...discovery,...discMetrics,...enrichDiag,rpcRetries:rpc.metrics.retries,rpcTimeouts:rpc.metrics.timeouts,processing:processing.size,metrics:store.state.metrics,tokens:store.tokens().length,users:Object.keys(store.state.users).length});
