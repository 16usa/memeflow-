...
function platformFactorRows(rows=[]){
  if(!Array.isArray(rows)||!rows.length){
    return `
      <div class="oi-row">
        <span>Waiting for trades with saved entry snapshots. Older backfilled trades may not contain these entry metrics; new trades will populate this factor automatically.</span>
        <strong>—</strong>
      </div>
    `;
  }
