...
(() => {
  ...
  const PATCH = 'MEMEFLOW_DISCOVERY_NATIVE_PLATFORM_V3';
  if (window.__mfDiscoveryNativePlatformV3) return;
  window.__mfDiscoveryNativePlatformV3 = true;
  ...
  const PUMP_ONLY = [
    {
      id: 'bonding',
      test: t => /\bbonding\s*curve\b/i.test(t),
      dex: 'Pump.fun only · inactive in DEX mode',
      hybrid: 'Pump.fun only · ignored by the DEX branch'
    },
    {
      id: 'fees',
      test: t => /\btotal\s+fees?\b/i.test(t),
      dex: 'Pump.fun only · inactive in DEX mode',
      hybrid: 'Pump.fun only · ignored by the DEX branch'
    },
    {
      id: 'developer',
      test: t => /\bdeveloper\b/i.test(t) && (
        /\bshare\b/i.test(t) ||
        /\bconcentration\b/i.test(t) ||
        /\bwallet\b/i.test(t) ||
        /\bblacklist\b/i.test(t) ||
        /\bmin(?:imum)?\b/i.test(t) ||
        /\bmax(?:imum)?\b/i.test(t) ||
        /%/.test(t)
      ),
      dex: 'Creator/developer signal is unavailable from the DEX pool · inactive',
      hybrid: 'Applied to Pump.fun tokens only · ignored by the DEX branch'
    },
    {
      id: 'bundle',
      test: t => /\bbundle\b/i.test(t),
      dex: 'No DEX bundle signal in this build · inactive',
      hybrid: 'Pump.fun branch only · no DEX bundle signal'
    },
    {
      id: 'sniper',
      test: t => /\bsniper\b/i.test(t),
      dex: 'No DEX sniper signal in this build · inactive',
      hybrid: 'Pump.fun branch only · no DEX sniper signal'
    }
  ];

  const css = `
    /* Kill all superseded custom discovery menus/panels. */
    #mfds-panel-v1,
    #mfdx-platform-menu-v2 {
      display: none !important;
      pointer-events: none !important;
    }

    .mfdn-platform-card-v3 {
    ...
    .mfdn-platform-card-v3::after {
      ...
      height: 7px;
      margin-top: -5px;
      border-right: 1.4px solid #6e8995;
      border-bottom: 1.4px solid #6e8995;
    ...
    .mfdn-platform-card-v3.mfdn-busy-v3 {
      opacity: .62 !important;
    }

    .mfdn-compat-card-v3 {
      position: relative !important;
      transition: opacity .14s ease, filter .14s ease, border-color .14s ease !important;
    }

    .mfdn-compat-card-v3.mfdn-dex-off-v3 {
      filter: grayscale(.55) saturate(.25) !important;
      border-color: rgba(95,118,128,.14) !important;
    }

    .mfdn-compat-card-v3.mfdn-dex-off-v3 > *:not(.mfdn-note-v3) {
      opacity: .38 !important;
    }

    .mfdn-compat-card-v3.mfdn-hybrid-v3 > *:not(.mfdn-note-v3) {
    ...
    .mfdn-note-v3 {
      ...
      border-top: 1px solid rgba(105,130,140,.10) !important;
      color: #647d88 !important;
    ...
    .mfdn-hybrid-v3 .mfdn-note-v3 {
      color: #8378ad !important;
    }

    .mfdn-dex-off-v3 input,
    .mfdn-dex-off-v3 select,
    .mfdn-dex-off-v3 textarea,
    .mfdn-dex-off-v3 button,
    .mfdn-dex-off-v3 [role="switch"],
    .mfdn-dex-off-v3 [role="slider"],
    .mfdn-dex-off-v3 [contenteditable="true"] {
      pointer-events: none !important;
    }
  `;

  function installStyle() {
    if (document.getElementById('mfdn-style-v3')) return;
  ...
  function leafNodes(root = document.body) {
    return Array.from(root.querySelectorAll('*')).filter(el => {
      if (el.id === 'mfdn-native-platform-v3') return false;
      if (el.classList?.contains('mfdn-note-v3')) return false;
      if (el.children.length) return false;
      return Boolean(text(el));
    });
  }

  function leafExact(value) {
    const wanted = String(value).trim().toLowerCase();
    return leafNodes().find(el => text(el).toLowerCase() === wanted) || null;
  }
  ...
  function findPlatformCard() {
    ...
    for (let depth = 0; node && node !== document.body && depth < 7; depth++, node = node.parentElement) {
      ...
      if (!rect || rect.width < 90 || rect.height < 42 || rect.height > 150) continue;
  ...
  function findPlatformValue(card) {
    ...
    const leaves = leafNodes(card).filter(el => text(el).toLowerCase() !== 'platform');
    const known = leaves.find(el =>
      ['pump.fun', 'pump', 'dex', 'hybrid'].includes(text(el).toLowerCase())
    );

    return known || leaves[leaves.length - 1] || null;
  }

  function label(mode) {
    return mode === 'pump' ? 'Pump.fun'
      : mode === 'dex' ? 'DEX'
      : mode === 'hybrid' ? 'Hybrid'
      : '—';
  }
  ...
  function createNativeSelect() {
    ...
    select.id = 'mfdn-native-platform-v3';
    ...
    // Critical: stop the old card click handler from ever seeing the native select tap.
    for (const eventName of ['pointerdown', 'touchstart', 'mousedown', 'click']) {
      select.addEventListener(eventName, ev => {
        ev.stopPropagation();
      });
    }
  ...
  function mountPlatform() {
    const card = findPlatformCard();
    if (!card) return false;

    state.card = card;
    state.valueEl = findPlatformValue(card);

    card.classList.add('mfdn-platform-card-v3');

    // Remove any stale V3 select caused by a settings re-render.
    const current = card.querySelector('#mfdn-native-platform-v3');
    if (current) {
      state.select = current;
    } else {
      state.select = createNativeSelect();
      card.appendChild(state.select);
    }
  ...
  function updateVisiblePlatform() {
    if (!state.card?.isConnected) mountPlatform();

    state.valueEl = findPlatformValue(state.card);

    if (state.valueEl && state.mode) {
      state.valueEl.textContent = label(state.mode);
    }

    if (state.select && state.mode && state.select.value !== state.mode) {
      state.select.value = state.mode;
    }

    if (state.select) state.select.disabled = state.busy;
    state.card?.classList.toggle('mfdn-busy-v3', state.busy);
  }

  function ruleFor(s) {
    const t = String(s || '').replace(/\s+/g, ' ').trim();
    return PUMP_ONLY.find(rule => rule.test(t)) || null;
  }

  function fieldCardFromLeaf(leaf) {
    let node = leaf?.parentElement;
    let fallback = null;

    for (let depth = 0; node && node !== document.body && depth < 7; depth++, node = node.parentElement) {
      if (node === state.card) return null;

      const t = text(node);
      if (t.length > 360) break;

      const rect = node.getBoundingClientRect?.();
      if (!rect || rect.width < 100 || rect.height < 42 || rect.height > 240) continue;

      const controls = node.querySelectorAll(
        'input,select,textarea,button,[role="switch"],[role="slider"],[contenteditable="true"]'
      ).length;
      ...
      if (!fallback && t.length < 220) fallback = node;
      if (bordered && controls <= 4 && t.length < 270) return node;
    }

    return fallback;
  }

  function blockInactive(ev) {
    const card = ev.currentTarget;
    if (!card.classList.contains('mfdn-dex-off-v3')) return;
    ev.preventDefault();
    ev.stopImmediatePropagation();
  }

  function installBlocker(card) {
    if (card.dataset.mfdnBlockerV3 === '1') return;
    card.dataset.mfdnBlockerV3 = '1';

    for (const eventName of [
      'pointerdown', 'touchstart', 'mousedown', 'click',
      'keydown', 'input', 'change'
    ]) {
      card.addEventListener(eventName, blockInactive, true);
    }
  }

  function clearCompatibility() {
    document.querySelectorAll('.mfdn-compat-card-v3').forEach(card => {
      ...
      card.querySelector(':scope > .mfdn-note-v3')?.remove();
    });
  }

  function applyCompatibility() {
    clearTimeout(state.compatTimer);

    state.compatTimer = setTimeout(() => {
      clearCompatibility();
      if (!state.mode || state.mode === 'pump') return;
      ...
      for (const leaf of leafNodes()) {
        ...
        const card = fieldCardFromLeaf(leaf);
        if (!card || seen.has(card)) continue;
        seen.add(card);

        card.classList.add('mfdn-compat-card-v3');

        const note = document.createElement('div');
        note.className = 'mfdn-note-v3';

        if (state.mode === 'dex') {
          card.classList.add('mfdn-dex-off-v3');
          card.setAttribute('aria-disabled', 'true');
          note.textContent = rule.dex;
          installBlocker(card);
        } else {
          card.classList.add('mfdn-hybrid-v3');
          note.textContent = rule.hybrid;
        }
  ...
  async function loadStatus() {
    ...
    try {
      ...
      console.warn('[DISCOVERY NATIVE V3] status:', e?.message || e);
    }
  }
  ...
  async function switchMode(mode) {
    ...
    if (!['pump', 'dex', 'hybrid'].includes(mode) || state.busy) {
      updateVisiblePlatform();
      return;
    }
    ...
    try {
      ...
      state.mode = previous;
      console.error('[DISCOVERY NATIVE V3] switch failed:', e);
    } finally {
      state.busy = false;
      render();
  ...
  function start() {
    ...
    // Any old custom dropdown left in the live DOM is removed immediately.
    document.getElementById('mfdx-platform-menu-v2')?.remove?.();
    document.getElementById('mfds-panel-v1')?.remove?.();
