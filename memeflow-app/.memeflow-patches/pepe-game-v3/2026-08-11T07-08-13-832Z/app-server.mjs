...
const game=new GameEngine(store); // MF_PEPE_ROCKET_GAME_INSTANCE
...
async function handler(req,res){const url=new URL(req.url,'http://x');
 ...
 if(req.method==='GET'&&!url.pathname.startsWith('/api/')){
   const p=url.pathname==='/'?'index.html':url.pathname==='/game'?'game.html':url.pathname.slice(1);const f=path.resolve(root,p); // MF_PEPE_ROCKET_GAME_ROUTE_ALIAS
