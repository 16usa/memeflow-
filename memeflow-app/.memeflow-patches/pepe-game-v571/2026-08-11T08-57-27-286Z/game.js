(()=>{
  ...
  const ui={
    ...
    multiplier:$('#multiplier'),peakHud:$('#peakHud'),drawdownHud:$('#drawdownHud'),thrustHud:$('#thrustHud'),autoDistance:$('#autoDistance'),stopDistance:$('#stopDistance'),flightAssist:$('#flightAssist'),flightAssistState:$('#flightAssistState'),flightAssistText:$('#flightAssistText'),world:$('#world'),fx:$('#fxCanvas'),
    ...
    center:$('#centerState'),centerKicker:$('#centerKicker'),centerValue:$('#centerValue'),centerLabel:$('#centerLabel'),milestone:$('#milestone'),milestoneValue:$('#milestoneValue'),milestoneText:$('#milestoneText'),stale:$('#staleCover'),shockwave:$('#shockwave'),emergencyFlash:$('#emergencyFlash'),targetReticle:$('#targetReticle'),
  ...
  function setMode(mode,message){
    ...
    const labels={idle:'READY',searching:'SCANNING',live:'LIVE',settling:'CASHING OUT',complete:'COMPLETE'};ui.stateLabel.textContent=labels[mode]||mode.toUpperCase();if(message)ui.stateMessage.textContent=message;syncButtons();void syncWakeLock();
  }
  ...
  function updateSearchRadar(){
    ...
    const phases=[['decision','DECISION'],['price','PRICE'],['holders','HOLDERS'],['coherence','COHERENCE']];
    ...
    if(!game.selectorDiag&&elapsed>700){
      const copy={decision:'Checking current BUY READY decisions…',price:'Checking accepted live price freshness…',holders:'Checking fresh holder evidence…',coherence:'Checking decision / price / holder coherence…'};
  ...
  function lockSelector(s){
    ...
    ui.selectorStatus.dataset.step='coherence';ui.selectorTitle.textContent=`Target acquired · ${symbol}`;ui.selectorText.textContent='Fresh coherent server entry locked at 1.00×.';
  }
  ...
  function renderVisual(multiplier){
    ...
    const tiltTarget=clamp(-3-v*18-game.acceleration*46,-18,12);game.visualTilt+= (tiltTarget-game.visualTilt)*(Math.abs(tiltTarget-game.visualTilt)>8?.18:.11);ui.rocket.style.transform=`translate(-50%,0) rotate(${game.visualTilt.toFixed(2)}deg)`;
    ...
    if(danger==='high'&&game.dangerLevel!=='high'&&Date.now()-game.lastDangerHapticAt>2600){game.lastDangerHapticAt=Date.now();haptic([18,32,18]);tone(165,.09,'sawtooth',.012,-35);}game.dangerLevel=danger;
    ...
    ui.world.style.setProperty('--camera-scale',cameraScale.toFixed(4));ui.world.style.setProperty('--camera-y',`${cameraY.toFixed(2)}px`);ui.world.style.setProperty('--camera-x',`${cameraX.toFixed(2)}px`);ui.world.style.setProperty('--engine-scale',engineScale.toFixed(3));ui.world.style.setProperty('--speed-opacity',(0.12+speed*.62).toFixed(3));ui.world.style.setProperty('--danger-opacity',clamp(dangerScore*.72,0,.86).toFixed(3));ui.world.style.setProperty('--boost-opacity',clamp(boost*.74,0,.78).toFixed(3));
  ...
  function apply(payload,{allowResult=true}={}){
    ...
    game.lastEventAt=Date.now();if(payload.history)renderHistory(payload.history);if(payload.stats)renderStats(payload.stats);if(payload.balance!==undefined)ui.balance.textContent=money(payload.balance);if(payload.selector){const d=payload.selector;game.selectorDiag=d;if(d.selectedMint){ui.selectorText.textContent=`Eligible ${d.eligible||0} · quality ${num(d.selectedQuality)===null?'—':Math.round(num(d.selectedQuality))}/100 · crowding ${Math.max(0,Math.floor(num(d.selectedCrowding)||0))}`;}else{ui.selectorText.textContent=`Eligible ${d.eligible||0} · stale price ${d.stalePrice||0} · holder age ${(d.staleHolderAge||0)+(d.staleHolders||0)} · coherence ${(d.decisionBehindPrice||0)+(d.decisionBehindHolder||0)}`;}}
    ...
    if(!s){game.countdownSeq++;ui.center.hidden=true;game.session=null;game.lastSessionId=null;game.lastSessionUpdatedAt=0;game.lastSessionRevision=0;game.targetMultiplier=1;game.displayMultiplier=1;game.points=[];game.dangerLevel='none';game.acceleration=0;game.visualTilt=-3;game.selectorDiag=null;ui.game.dataset.danger='none';ui.game.dataset.launch='idle';ui.game.dataset.outcome='none';ui.game.dataset.bank='neutral';game.pendingResultId=null;game.completionSeq++;ui.shockwave?.classList.remove('is-active');ui.world.style.setProperty('--camera-scale','1');ui.world.style.setProperty('--camera-y','0px');ui.world.style.setProperty('--camera-x','0px');ui.world.style.setProperty('--engine-scale','1');ui.world.style.setProperty('--speed-opacity','.12');ui.world.style.setProperty('--danger-opacity','0');ui.world.style.setProperty('--boost-opacity','0');document.title='MEMEFLOW · Pepe Rocket';drawTrace();if(game.mode!=='searching'){if(!ui.result.hidden){game.resultFxSeq++;ui.result.hidden=true;ui.game.inert=false;ui.game.dataset.outcome='none';ui.game.dataset.launch='idle';ui.resultCard.dataset.tone='';ui.shockwave?.classList.remove('is-active');game.showingResult=null;}setMode('idle','Set a paper stake and launch when ready.');resetToken();previewStake();}return;}
  ...
  async function startRound(){
    ...
    while(seq===game.searchSeq){
      ...
      try{
        ...
        if(error.code==='NO_CANDIDATE'){const d=error.data?.selector||{};game.selectorDiag=d;const priceWait=(d.stalePrice||0)+(d.futurePrice||0),holderWait=(d.staleHolderAge||0)+(d.staleHolders||0)+(d.futureHolder||0),decisionWait=(d.staleDecision||0)+(d.futureDecision||0),coherence=(d.decisionBehindPrice||0)+(d.decisionBehindHolder||0);const blockers=[['fresh price',priceWait],['fresh holders',holderWait],['fresh decision',decisionWait],['coherent evidence',coherence]].sort((a,b)=>b[1]-a[1]);ui.selectorText.textContent=`Eligible ${d.eligible||0} · waiting mostly on ${blockers[0][1]?blockers[0][0]:'new BUY READY candidates'} · checked ${d.buyReady||0} BUY READY`;ui.selectorPhase.textContent='WAIT';ui.stateMessage.textContent='No launch meets every freshness/coherence requirement yet — scanning continues…';await wait(1100+Math.random()*900);continue;}
  ...
  async function queueResult(s){if(!s||s.state!=='COMPLETE'||game.showingResult===s.id||game.pendingResultId===s.id)return;game.pendingResultId=s.id;const seq=++game.completionSeq;const valid=()=>seq===game.completionSeq&&game.pendingResultId===s.id&&game.session?.id===s.id&&game.session?.state==='COMPLETE';const profit=num(s.profit)||0,voided=s.voided===true,reason=String(s.reason||'');if(!reducedMotion&&game.pageVisible&&!voided){const crash=reason.includes('STOP_LOSS')||profit<-.005;if(crash){ui.game.dataset.outcome='crash';ui.center.hidden=false;ui.centerKicker.textContent=reason.includes('STOP_LOSS')?'STOP LOSS':'HARD LANDING';ui.centerValue.textContent='↓';ui.centerLabel.textContent='EMERGENCY DESCENT';haptic([35,24,45]);await wait(560);}else{ui.game.dataset.outcome='secure';ui.center.hidden=false;ui.centerKicker.textContent=reason.includes('AUTO_CASH_OUT')?'AUTO TARGET':'CASH OUT';ui.centerValue.textContent='✓';ui.centerLabel.textContent='POSITION SECURED';pulseShockwave(1.1);await wait(300);}}if(!valid()){if(game.pendingResultId===s.id)game.pendingResultId=null;ui.center.hidden=true;ui.game.dataset.outcome='none';return;}ui.center.hidden=true;game.pendingResultId=null;showResult(s);}
  ...
  function bind(){
    ...
    addEventListener('beforeunload',()=>{game.completionSeq++;game.pendingResultId=null;game.stream?.close?.();stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }
