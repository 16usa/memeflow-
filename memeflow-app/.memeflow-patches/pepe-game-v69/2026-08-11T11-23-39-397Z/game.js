(()=>{
  ...
  const game={
    ...
    lastBalance:null,balancePulseTimer:null,summaryRefreshTimer:null,summaryRefreshInFlight:false,roundResetTimer:null
  };
  ...
  function updateTriggerDistances(m){
    if(!game.session||game.session.state!=='LIVE'){ui.autoDistance.textContent='—';ui.stopDistance.textContent='—';return;}
  ...
  function bind(){
    ...
    addEventListener('beforeunload',()=>{clearTimeout(game.historyClearTimer);clearTimeout(game.wakeRetryTimer);clearTimeout(game.resultFocusTimer);clearTimeout(game.balancePulseTimer);clearTimeout(game.summaryRefreshTimer);clearTimeout(game.roundResetTimer);game.wakeRetryTimer=null;game.resultFocusTimer=null;game.balancePulseTimer=null;game.summaryRefreshTimer=null;game.roundResetTimer=null;game.wakeRequestSeq++;game.searchResumeCleanup?.();game.searchResumeCleanup=null;stopClock();clearVisualTimers();game.completionSeq++;game.pendingResultId=null;stopVisualLoop();game.stream?.close?.();game.stream=null;stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }
