(()=>{
  ...
  function recordPoint(multiplier,serverTime=Date.now()){
    ...
    if(last&&Math.abs(last.m-multiplier)<1e-6&&serverTime-last.t<350)return;game.points.push({t:serverTime,m:multiplier});if(game.points.length>100)game.points.splice(0,game.points.length-100);drawTrace();
  }
  ...
  function waitForSearchResume(seq){
    ...
    return new Promise(resolve=>{
      let done=false;const finish=()=>{if(done)return;done=true;document.removeEventListener('visibilitychange',check);removeEventListener('online',check);clearTimeout(timer);if(game.searchResumeCleanup===finish)game.searchResumeCleanup=null;resolve();};
      const check=()=>{if(seq!==game.searchSeq||game.pageVisible&&navigator.onLine!==false)finish();};
      document.addEventListener('visibilitychange',check);addEventListener('online',check);const timer=setTimeout(finish,30000);game.searchResumeCleanup=finish;
    });
  }
