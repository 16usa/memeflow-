(()=>{
  ...
  const game={
    ...
    raf:null,lifecyclePaused:false,flightState:'idle',localFeedVisualStale:false,lastStage:'ground',stageTimer:null,historyClearTimer:null,lastVisualAt:0,fallbackInFlight:false,resyncPromise:null,startCancelArmedAt:0,resetInFlight:false,clearHistoryInFlight:false,streamOpenedAt:0,streamAcceptedAt:0,streamReconnectAt:0,wakeRetryTimer:null,wakeRequestSeq:0,wakeRequestPending:false,resultFocusTimer:null,viewportTimer:null,searchResumeCleanup:null
  };
  ...
  function updateFlightProgress(m){if(!ui.flightProgress)return;const order=['ground','clouds','strato','orbit','moon','deep','hyper'],current=stageFor(m)[0],peakStage=stageFor(Math.max(m,num(game.session?.peak)||m))[0],currentIndex=Math.max(0,order.indexOf(current)),peakIndex=Math.max(currentIndex,order.indexOf(peakStage));[...ui.flightProgress.querySelectorAll('[data-flight-stage]')].forEach((node,index)=>{node.classList.toggle('reached',index<=peakIndex);node.classList.toggle('current',index===currentIndex);});}
  function recordPoint(multiplier,serverTime=Date.now()){
    ...
    if(last&&serverTime===last.t){if(Math.abs(last.m-multiplier)<1e-6)return;last.m=multiplier;drawTrace();return;}
    if(last&&Math.abs(last.m-multiplier)<1e-6&&serverTime-last.t<350)return;game.points.push({t:serverTime,m:multiplier});if(game.points.length>120){const first=game.points[0];game.points=[first,...game.points.slice(-118)];}drawTrace();
  }
  ...
  function renderVisual(multiplier){
    ...
    const bet=num(game.session?.bet)??currentBet(),value=bet*m,profit=value-bet,peak=Math.max(m,num(game.session?.peak)||m);ui.paperValue.textContent=money(value);ui.profit.textContent=`${profit>0?'+':''}${money(profit)}`;ui.profit.className=profit>.005?'positive':profit<-.005?'negative':'';
    const [stage,label]=stageFor(m);if(stage!==game.lastStage)showStageTransition(stage,label,m);ui.game.dataset.stage=stage;ui.stage.textContent=label;const bottom=7+levelNorm(m)*62;ui.rocket.style.bottom=`${bottom.toFixed(2)}%`;if(ui.altimeterMarker){ui.altimeterMarker.style.bottom=lineBottom(m);ui.altimeterPeak.style.bottom=lineBottom(peak);ui.altimeterValue.textContent=`${m.toFixed(2)}× · ${label}`;}
    ...
    ui.world.style.setProperty('--camera-scale',cameraScale.toFixed(4));ui.world.style.setProperty('--camera-y',`${cameraY.toFixed(2)}px`);ui.world.style.setProperty('--camera-x',`${cameraX.toFixed(2)}px`);ui.world.style.setProperty('--engine-scale',engineScale.toFixed(3));if(ui.flightDirector){ui.flightDirector.style.setProperty('--director-roll',`${clamp(game.visualTilt,-18,18).toFixed(2)}deg`);ui.flightDirector.style.setProperty('--director-pitch',`${clamp(-v*120,-10,10).toFixed(2)}px`);}ui.world.style.setProperty('--speed-opacity',(0.12+speed*.62).toFixed(3));ui.world.style.setProperty('--danger-opacity',clamp(dangerScore*.72,0,.86).toFixed(3));ui.world.style.setProperty('--boost-opacity',clamp(boost*.74,0,.78).toFixed(3));const sceneEnergy=clamp(Math.abs(v)*6.5+Math.max(0,m-1)*.028+(stage==='hyper'?.32:stage==='deep'?.14:0),0,1);const px=clamp(game.visualTilt*.42+v*54,-11,11),py=clamp(-v*38+fall*15,-8,8);ui.world.style.setProperty('--scene-energy',sceneEnergy.toFixed(3));ui.world.style.setProperty('--pf-x',`${(-px*.28).toFixed(2)}px`);ui.world.style.setProperty('--pf-y',`${(-py*.22).toFixed(2)}px`);ui.world.style.setProperty('--pm-x',`${(-px*.62).toFixed(2)}px`);ui.world.style.setProperty('--pm-y',`${(-py*.48).toFixed(2)}px`);ui.world.style.setProperty('--pn-x',`${(-px*1.1).toFixed(2)}px`);ui.world.style.setProperty('--pn-y',`${(-py*.82).toFixed(2)}px`);ui.world.style.setProperty('--smoke-opacity',(stage==='ground'||stage==='clouds'?clamp(.18+sceneEnergy*.42,.12,.66):.03).toFixed(3));ui.world.style.setProperty('--vignette-opacity',clamp(.16+speed*.16+dangerScore*.15,.14,.58).toFixed(3));
    updateTriggerDistances(m);updateFlightAssist(m);updateFlightProgress(m);
    if(ui.flightPositionCurrent)ui.flightPositionCurrent.textContent=`${m.toFixed(2)}×`;
    if(ui.flightPositionPeak)ui.flightPositionPeak.textContent=`${peak.toFixed(2)}×`;
  ...
  function apply(payload,{allowResult=true}={}){
    ...
    if(payload.history)renderHistory(payload.history);if(payload.stats)renderStats(payload.stats);if(payload.balance!==undefined)ui.balance.textContent=money(payload.balance);
  ...
  function createFx(){
    ...
    const ro=globalThis.ResizeObserver?new ResizeObserver(resize):null;ro?.observe(canvas);if(!ro)addEventListener('resize',resize,{passive:true});resize();
    return{pause,resume,stop(){pause();ro?.disconnect();if(!ro)removeEventListener('resize',resize);ctx.clearRect(0,0,w,h);}};
  }

  function syncViewportHeight(){const h=globalThis.visualViewport?.height||innerHeight;if(Number.isFinite(h)&&h>0)document.documentElement?.style?.setProperty?.('--vvh',`${Math.round(h)}px`);}
  function scheduleViewportHeight(){clearTimeout(game.viewportTimer);game.viewportTimer=setTimeout(()=>{game.viewportTimer=null;syncViewportHeight();},80);}

  function bind(){
    syncViewportHeight();globalThis.visualViewport?.addEventListener?.('resize',scheduleViewportHeight,{passive:true});globalThis.visualViewport?.addEventListener?.('scroll',scheduleViewportHeight,{passive:true});addEventListener('orientationchange',scheduleViewportHeight,{passive:true});
    ...
    addEventListener('pageshow',()=>{game.pageVisible=!document.hidden;game.lifecyclePaused=false;syncViewportHeight();ui.game.dataset.lifecycle='active';syncClockActivity();syncVisualActivity();void resyncStatus().catch(()=>startFallback());void syncWakeLock();});
    ...
    addEventListener('beforeunload',()=>{clearTimeout(game.historyClearTimer);clearTimeout(game.wakeRetryTimer);clearTimeout(game.resultFocusTimer);clearTimeout(game.viewportTimer);game.wakeRetryTimer=null;game.resultFocusTimer=null;game.viewportTimer=null;game.wakeRequestSeq++;game.searchResumeCleanup?.();game.searchResumeCleanup=null;globalThis.visualViewport?.removeEventListener?.('resize',scheduleViewportHeight);globalThis.visualViewport?.removeEventListener?.('scroll',scheduleViewportHeight);removeEventListener('orientationchange',scheduleViewportHeight);stopClock();clearVisualTimers();game.completionSeq++;game.pendingResultId=null;stopVisualLoop();game.stream?.close?.();game.stream=null;stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }
