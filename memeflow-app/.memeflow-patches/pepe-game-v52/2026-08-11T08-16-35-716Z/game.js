(()=>{
  ...
  const game={
    ...
    lastPayloadServerTime:0,lastStateRevision:0,lastEventSeq:0,engineEpoch:null,lastSessionId:null,lastSessionUpdatedAt:0,lastSessionRevision:0,wakeLock:null
  };
  ...
  function checkMilestone(m){for(const [level,label] of [[1.08,'Liftoff'],[1.2,'Cloud deck cleared'],[1.5,'Stratosphere'],[2,'Orbit reached'],[3,'Lunar approach'],[5,'Deep space'],[10,'Hyperspace']]){if(m>=level&&!game.milestones.has(level)){game.milestones.add(level);showMilestone(level,label);}}}
  ...
  function apply(payload,{allowResult=true}={}){
    ...
    if(!s){game.session=null;game.lastSessionId=null;game.lastSessionUpdatedAt=0;game.lastSessionRevision=0;game.targetMultiplier=1;game.displayMultiplier=1;game.points=[];document.title='MEMEFLOW · Pepe Rocket';drawTrace();if(game.mode!=='searching'){if(!ui.result.hidden){ui.result.hidden=true;game.showingResult=null;}setMode('idle','Set a paper stake and launch when ready.');resetToken();previewStake();}return;}
    ...
    if(s.state==='COMPLETE'){setMode('complete',s.voided?'Market feed lost · paper stake returned safely.':`${String(s.reason||'ROUND COMPLETE').replaceAll('_',' ')} · server settled.`);if(allowResult)showResult(s);}
  }
  ...
  function startFallback(){if(game.fallback)return;const sync=()=>{if(!game.pageVisible||!navigator.onLine)return;void api('/api/game/status').then(x=>apply(x)).catch(()=>{});};sync();game.fallback=setInterval(sync,4000);}
  ...
  function bind(){
    ...
    addEventListener('pageshow',()=>{game.pageVisible=!document.hidden;void api('/api/game/status').then(x=>apply(x)).catch(()=>{});void syncWakeLock();});
