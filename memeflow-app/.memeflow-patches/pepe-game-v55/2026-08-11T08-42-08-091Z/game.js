(()=>{
  ...
  const ui={
    ...
    result:$('#result'),resultCanvas:$('#resultCanvas'),resultCard:$('#resultCard'),resultReason:$('#resultReason'),resultMultiplier:$('#resultMultiplier'),resultTitle:$('#resultTitle'),resultCopy:$('#resultCopy'),resultStake:$('#resultStake'),resultPayout:$('#resultPayout'),resultProfit:$('#resultProfit'),resultPeak:$('#resultPeak'),resultDrawdown:$('#resultDrawdown'),resultAdverse:$('#resultAdverse'),resultPeakTime:$('#resultPeakTime'),resultDuration:$('#resultDuration'),resultSettlement:$('#resultSettlement'),resultQuality:$('#resultQuality'),resultUpdates:$('#resultUpdates'),resultEntryPrice:$('#resultEntryPrice'),resultExitPrice:$('#resultExitPrice'),playAgain:$('#playAgain')
  };
  ...
  const game={
    ...
    dangerLevel:'none',lastDangerHapticAt:0,focusMode:false,cameraScale:1,cameraY:0,cameraX:0,engineScale:1
  };
  ...
  function renderVisual(multiplier){
    ...
    const v=game.velocity,dd=Math.max(0,num(game.session?.drawdownPct)||0),fall=Math.max(0,-v);const direction=v>.012?'up':v<-.012?'down':'flat';ui.game.dataset.direction=direction;const tilt=clamp(-3-v*22,-14,8);ui.rocket.style.transform=`translate(-50%,0) rotate(${tilt.toFixed(2)}deg)`;
  ...
  function showMilestone(level,label){ui.milestoneValue.textContent=`${level.toFixed(2)}×`;ui.milestoneText.textContent=label;ui.milestone.hidden=false;sfx('milestone');haptic([15,24,15]);cinematicKick(level>=5?1.55:level>=2?1.2:1);clearTimeout(showMilestone.timer);showMilestone.timer=setTimeout(()=>ui.milestone.hidden=true,reducedMotion?450:1300);}

  async function countdown(s){if(!s?.id||game.launchSeen.has(s.id))return;game.launchSeen.add(s.id);if(game.launchSeen.size>24)game.launchSeen=new Set([s.id]);const seq=++game.countdownSeq;const valid=()=>seq===game.countdownSeq&&game.session?.id===s.id&&game.session?.state==='LIVE';ui.center.hidden=false;ui.centerKicker.textContent='ENTRY LOCKED';ui.centerLabel.textContent='LIVE';for(const value of ['3','2','1']){if(!valid()){ui.center.hidden=true;return;}ui.centerValue.textContent=value;tone(300+Number(value)*65,.045,'triangle',.015,35);await wait(reducedMotion?60:190);}if(!valid()){ui.center.hidden=true;return;}ui.centerValue.textContent='GO';sfx('launch');haptic(28);cinematicKick(1.15);await wait(reducedMotion?80:190);if(valid())ui.center.hidden=true;}
  ...
  function apply(payload,{allowResult=true}={}){
    ...
    if(!s){game.countdownSeq++;ui.center.hidden=true;game.session=null;game.lastSessionId=null;game.lastSessionUpdatedAt=0;game.lastSessionRevision=0;game.targetMultiplier=1;game.displayMultiplier=1;game.points=[];game.dangerLevel='none';ui.game.dataset.danger='none';ui.world.style.setProperty('--camera-scale','1');ui.world.style.setProperty('--camera-y','0px');ui.world.style.setProperty('--camera-x','0px');ui.world.style.setProperty('--engine-scale','1');ui.world.style.setProperty('--speed-opacity','.12');ui.world.style.setProperty('--danger-opacity','0');ui.world.style.setProperty('--boost-opacity','0');document.title='MEMEFLOW · Pepe Rocket';drawTrace();if(game.mode!=='searching'){if(!ui.result.hidden){game.resultFxSeq++;ui.result.hidden=true;ui.game.inert=false;game.showingResult=null;}setMode('idle','Set a paper stake and launch when ready.');resetToken();previewStake();}return;}
