(()=>{
  ...
  async function queueResult(s){if(!s||s.state!=='COMPLETE'||game.showingResult===s.id||game.pendingResultId===s.id)return;game.pendingResultId=s.id;const seq=++game.completionSeq;const valid=()=>seq===game.completionSeq&&game.pendingResultId===s.id&&game.session?.id===s.id&&game.session?.state==='COMPLETE';const profit=num(s.profit)||0,voided=s.voided===true,reason=String(s.reason||'');if(!reducedMotion&&game.pageVisible&&!voided){const crash=reason.includes('STOP_LOSS')||profit<-.005;if(crash){ui.game.dataset.outcome='crash';ui.center.hidden=false;ui.centerKicker.textContent=reason.includes('STOP_LOSS')?'STOP LOSS':'HARD LANDING';ui.centerValue.textContent='↓';ui.centerLabel.textContent='EMERGENCY DESCENT';haptic([35,24,45]);await wait(720);}else{ui.game.dataset.outcome='secure';ui.center.hidden=false;ui.centerKicker.textContent=reason.includes('AUTO_CASH_OUT')?'AUTO TARGET':'CASH OUT';ui.centerValue.textContent='✓';ui.centerLabel.textContent='POSITION SECURED';pulseShockwave(1.1);await wait(380);}}if(!valid()){if(game.pendingResultId===s.id)game.pendingResultId=null;ui.center.hidden=true;ui.game.dataset.outcome='none';return;}ui.center.hidden=true;game.pendingResultId=null;showResult(s);}
