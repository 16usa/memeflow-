(()=>{
  ...
  const game={
    ...
    raf:null,lifecyclePaused:false,flightState:'idle',localFeedVisualStale:false
  };
  ...
  function setMode(mode,message){
    ...
    const labels={idle:'READY',searching:'SCANNING',live:'LIVE',settling:'CASHING OUT',complete:'COMPLETE'};ui.stateLabel.textContent=labels[mode]||mode.toUpperCase();if(message)ui.stateMessage.textContent=message;setFlightState(resolveFlightState());syncButtons();void syncWakeLock();
  }
  ...
  function updateTriggerLines(){const auto=num(ui.auto.value)||0,stop=num(ui.stop.value)||0;ui.autoTrigger.hidden=!(auto>1);if(auto>1){ui.autoTrigger.style.bottom=lineBottom(auto);ui.autoTriggerLabel.textContent=`AUTO ${auto.toFixed(2)}×`;}ui.stopTrigger.hidden=!(stop>0&&stop<1);if(stop>0&&stop<1){ui.stopTrigger.style.bottom=lineBottom(stop);ui.stopTriggerLabel.textContent=`STOP ${stop.toFixed(2)}×`;}updateRiskPreview();}
  ...
  function createFx(){
    const canvas=ui.fx,ctx=canvas.getContext('2d');if(!ctx)return null;let w=1,h=1,dpr=1,running=false,raf=null;const stars=[];const saveData=navigator.connection?.saveData===true;const count=saveData?28:innerWidth<700?44:82;
  ...
  function bind(){
    ...
    document.addEventListener('visibilitychange',()=>{game.pageVisible=!document.hidden;void syncWakeLock();if(game.pageVisible){game.lifecyclePaused=false;ui.game.dataset.lifecycle='active';game.fx?.resume?.();startVisualLoop();void api('/api/game/status').then(x=>{apply(x);connectStream();}).catch(()=>startFallback());}});
    ...
    addEventListener('pageshow',()=>{game.pageVisible=!document.hidden;game.lifecyclePaused=false;ui.game.dataset.lifecycle='active';game.fx?.resume?.();startVisualLoop();void api('/api/game/status').then(x=>{apply(x);connectStream();}).catch(()=>startFallback());void syncWakeLock();});
    addEventListener('beforeunload',()=>{game.completionSeq++;game.pendingResultId=null;stopVisualLoop();game.stream?.close?.();game.stream=null;stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }
