...
function __mfPipelineHolderTruthV26(token={},mint='',now=Date.now()){
  ...
  const pumpCount=
    finiteValue(token?.pumpReportedHolderCount);

  const pumpAt=
    finiteValue(token?.pumpReferenceAt);
  ...
  const observed=
    finiteValue(token?.observedHolderCount) ??
    ledgerCount ??
    (
      token?.holderCountIsLowerBound===true
        ? storedCount
        : null
    );

  if(pumpCount!==null && pumpCount>=0){
    return {
      count:pumpCount,
      observed,
  ...
  if(
    ...
    return {
      ...
      authoritative:true,
      lowerBound:false,
      fresh:token?.holderFresh===true,
      updatedAt:
        finiteValue(token?.holderUpdatedAt) ??
        finiteValue(token?.updatedAt)
    };
  }
  ...
  return {
    ...
    observed:
      observed!==null && observed>=0
        ? observed
        : null,
    source:'ws-event-ledger-lower-bound',
    authoritative:false,
    lowerBound:true,
    fresh:false,
    updatedAt:
...
function __mfLiveCardViewV14(token,decision){
  ...
  return {
    ...
    observedHolderCount:holderTruth.observed,
