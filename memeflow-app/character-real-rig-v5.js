...
import {
  createPepeRealRigV2
} from '/character-real-rig-v2.js?v=skeleton5';

export function createPepeSkeletonV5({
  parent,
  baseUrl = '/game-assets/character/'
} = {}) {

  if (!parent) {
    throw new Error('[PEPE V5] parent required');
  }
  ...
  const ready = base.ready.then(() => {
    ...
    */
    ...
    parts.handLeft.mesh.position.x += 26 * UNIT;
    parts.handLeft.mesh.position.y += 92 * UNIT;
    parts.handLeft.mesh.scale.setScalar(1.24);

    parts.handRight.mesh.position.x -= 26 * UNIT;
    parts.handRight.mesh.position.y += 92 * UNIT;
    parts.handRight.mesh.scale.setScalar(1.24);
