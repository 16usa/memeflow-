...
export function createPepeRealRigV16({ parent, baseUrl='/game-assets/character-v16/' }={}) {
  ...
  function attachHandToArm(handName, armName) {
    ...
    if (!hand || !arm || !cfg) {
      throw new Error(
        `[PEPE V31] missing ${handName}/${armName}`
      );
    }

    if (cfg.armName !== armName) {
      throw new Error(
        `[PEPE V31] arm mapping mismatch for ${handName}`
      );
    }

    /*
      Real sleeve socket in arm texture space.
    */
    ...
    /*
      Real cuff center in hand texture space.
    */
    ...
    */
    ...
    /*
      Move the artwork inside handPivot so the
      authored cuff point becomes local 0,0.
    */
    ...
    /*
      Re-parent the complete hand pivot to its own
      wrist joint.
    */
    ...
    /*
      The hand pivot itself sits exactly at the
      sleeve socket. No handSeat offsets.
    */
    ...
    /*
      ...
      The original authored relation was mirrored
      across each forearm axis.

      Add exactly 180 degrees around the cuff pivot.
      This changes finger direction without moving
      the cuff away from the sleeve.
    */

    const palmUpRotation =
      (-2 * arm.baseRot) +
      Math.PI;
    ...
    /*
      Hands stay behind the sleeve artwork.
    */
    ...
    /*
      Verify that the cuff origin is exactly at
      its wrist socket.
    */
    ...
    const wristError =
      cuffCheck.distanceTo(
        socketCheck
      );

    if (wristError > 0.00001) {
      throw new Error(
        `[PEPE V31] ${handName} wrist lock error: ${wristError}`
      );
    }

    console.log(
      `[PEPE V31] ${handName} wrist locked`,
      wristError
    );
  }
  ...
  const ready = Promise.all(defs.map(addPart)).then(() => {
    ...
    console.log(
      '[PEPE V31] CLEAN WRIST RIG READY'
    );
  ...
  function update(t, dt=1/60) {
    ...
    parts.legLeft.pivot.rotation.z =
      parts.legLeft.baseRot +
      leg;

    parts.legRight.pivot.rotation.z =
      parts.legRight.baseRot -
      leg;
  }
