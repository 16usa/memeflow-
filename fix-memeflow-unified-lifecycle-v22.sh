...
BACKUP=".memeflow-backups/unified-lifecycle-v22-$STAMP"
mkdir -p "$BACKUP"
...
echo
echo "=== V22 SYNTAX ==="
for f in "$APP" "$PAPER" "$LIFECYCLE" "$TEST_LIFECYCLE" "$TEST_ASSIST"; do
  node --check "$f"
done
...
echo
echo "=== V22 FULL PROJECT TEST SUITE ==="
(
  cd memeflow-app
  npm test
)
...
ALLOWED_RE='^memeflow-app/(app-server\.mjs|package\.json|src/paper-engine\.mjs|src/position-decision\.mjs|tests/lifecycle-decision-v22\.mjs|tests/assist-fresh-decision-v22\.mjs)$'
...
git commit -m "refactor: unify MEMEFLOW entry and position lifecycle decisions v22"
