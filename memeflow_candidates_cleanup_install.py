...
'''
    ...
    cache_bust("trading.css", "candidates-canonical-cleanup-v1-20260909")
    cache_bust("trading-light-polish-v61.css", "candidates-cleanup-v1-20260909")
    cache_bust("trading-visual-hierarchy-v67.css", "candidates-cleanup-v1-20260909")
    cache_bust("memeflow-dark-x-surface-v131.css", "candidates-cleanup-v1-20260909")
    cache_bust("memeflow-trading-white-surfaces-v154.css", "candidates-cleanup-v1-20260909")

    # Verify old assets are no longer referenced before deleting them.
    for asset in ["memeflow-candidate-state-filters-v119.css", "memeflow-candidate-edit-fill-v121.css"]:
        refs = []
        for p in ROOT.rglob("*"):
            if not p.is_file() or ".git" in p.parts or ".memeflow-backups" in p.parts or p.name == asset:
                continue
            ...
            try:
                ...
                pass
        if refs:
            raise RuntimeError(f"{asset} still referenced by: {', '.join(refs)}")
