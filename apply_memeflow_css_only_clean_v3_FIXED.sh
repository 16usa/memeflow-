...
# MEMEFLOW CSS-ONLY CLEANUP V3 FIXED
# ------------------------------------------------------------
...
if [[ "$REMOTE_HEAD" != "$EXPECTED_HEAD" ]]; then
  echo "ERROR: GitHub main changed since V3 FIXED was audited." >&2
...
BACKUP="/tmp/memeflow-css-only-cleanup-v3-fixed-$STAMP"
mkdir -p "$BACKUP"
for f in "${FILES[@]}"; do
...
restore_on_error() {
  ...
  if [[ "$rc" -ne 0 && "$APPLIED" -eq 1 && "$COMMITTED" -eq 0 ]]; then
    ...
    echo "Nothing from V3 FIXED remains."
  fi
  exit "$rc"
}
...
def mutate_decl(text, selector, prop, old_value, new_value, label):
    """Allow selector duplicates (base + media); edit only the rule with audited declaration."""
    rule_rx = re.compile(re.escape(selector) + r"\s*\{(?P<body>.*?)\}", re.S)
    rules = list(rule_rx.finditer(text))
    if not rules:
        raise SystemExit(f"ERROR [{label}]: selector not found: {selector}")
    decl_rx = re.compile(
        r"(?m)^(\s*" + re.escape(prop) + r"\s*:\s*)" + re.escape(old_value) + r"\s*;"
    )
    candidates = [m for m in rules if len(decl_rx.findall(m.group('body'))) == 1]
    if len(candidates) != 1:
        raise SystemExit(
            f"ERROR [{label}]: selector occurs {len(rules)} time(s), but exact "
            f"{prop}: {old_value}; matched {len(candidates)} rule(s), expected 1."
        )
    m = candidates[0]
    body = decl_rx.sub(lambda mm: mm.group(1) + new_value + ";", m.group('body'), count=1)
...
echo
echo "V3 FIXED guardrails passed."
echo "Staged files only:"
git diff --cached --name-only

git commit \
  -m "Finish CSS-only quiet border cleanup without layout changes" \
  -- "${FILES[@]}"
