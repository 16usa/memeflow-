...
PATCH_ID="MEMEFLOW_GMGN_SORT_STYLE_V25_5_POLISH"
OLD_VERSION="gmgn-sort-v25-4-mockup-20260827"
NEW_VERSION="gmgn-sort-v25-5-polish-20260827"
DO_PUSH=0
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: bash memeflow_sort_polish.sh [--push|--no-push]" >&2
      exit 2
      ;;
  esac
done
...
grep -Fq "MEMEFLOW_GMGN_SORT_STYLE_V25_4_MOCKUP" system-tokens.css || {
  echo "ERROR: V25.4 MOCKUP CSS marker is missing." >&2
  exit 1
}
...
if [[ "$(grep -Fc "$OLD_VERSION" system-tokens.html)" -ne 2 ]]; then
  echo "ERROR: expected exactly two V25.4 browser asset URLs." >&2
  exit 1
fi
...
BACKUP_DIR=".patch-backups/gmgn-sort-v25-5-polish-$STAMP"
mkdir -p "$BACKUP_DIR"
...
rollback() {
  ...
  if [[ $rc -ne 0 ]]; then
    echo
    echo "Patch failed. Restoring V25.4 MOCKUP files..."
...
old_marker = "/* MEMEFLOW_GMGN_SORT_STYLE_V25_4_MOCKUP"
marker_at = css.find(old_marker)

if marker_at < 0:
    raise SystemExit(
        "PRECHECK FAILED: V25.4 MOCKUP CSS marker was not found."
    )
...
if re.search(r"\.(?:mf-sort-|mf-age-list-v25)", prefix):
    raise SystemExit(
        ...
        "outside the canonical style block."
    )
...
unexpected = [
    ...
    if marker != "MEMEFLOW_GMGN_SORT_STYLE_V25_4_MOCKUP"
]
...
polish_css = r'''
/* MEMEFLOW_GMGN_SORT_STYLE_V25_5_POLISH
...
canonical = tests_dir / "token-sort-v25-5-polish.mjs"
...
assert.match(
  css,
  /background:[\s\S]*rgba\(24,37,46,.975\)/
);
...
assert.equal(
  (
    html.match(
      /gmgn-sort-v25-5-polish-20260827/g
    )||[]
  ).length,
  2
);

console.log('token sort v25.5 polish ok');
''',
    encoding="utf-8"
)
...
for path in sorted(tests_dir.glob("token-sort*.mjs")):
    ...
    path.write_text(
        "import './token-sort-v25-5-polish.mjs';\n",
        encoding="utf-8"
    )
...
if css.count(PATCH_ID) != 1:
    raise SystemExit(
        "POSTCHECK FAILED: V25.5 polish CSS layer must exist exactly once."
    )

css_path.write_text(css,encoding="utf-8")
html_path.write_text(html,encoding="utf-8")

print("V25.5 polish visual refinement installed.")
...
node --check tests/token-sort-v25-5-polish.mjs

echo "==> [3/7] Polish visual regression"
node tests/token-sort-v25-5-polish.mjs
...
marker = "MEMEFLOW_GMGN_SORT_STYLE_V25_5_POLISH"

if css.count(marker) != 1:
    raise SystemExit(
        "ERROR: V25.5 polish sorting CSS layer is not unique."
    )

if "MEMEFLOW_GMGN_SORT_STYLE_V25_4_MOCKUP" in css:
    raise SystemExit(
...
if marker_at < 0:
    raise SystemExit(
        "ERROR: V25.5 polish CSS marker was not found."
    )
...
for rule in required:
    if rule not in css:
        raise SystemExit(
            f"ERROR: required polish rule is missing: {rule}"
        )

if html.count(
    "gmgn-sort-v25-5-polish-20260827"
) != 2:
    raise SystemExit(
        "ERROR: expected exactly two V25.5 browser asset URLs."
    )
...
echo
echo "V25.5 POLISH validated successfully."
...
if git diff --cached --quiet; then
  ...
  git commit -m "fix(token-flow): polish sorting sheet to approved mockup"
fi

if [[ "$DO_PUSH" -eq 1 ]]; then
  echo
  echo "Pushing validated V25.5 POLISH commit..."
  git push
else
  echo
...
echo
echo "DONE V25.5 POLISH:"
echo "  - overlay is almost transparent like the mockup"
echo "  - sheet surface is lighter and more glass-like"
echo "  - option labels use lighter typography"
echo "  - icons are brighter"
echo "  - active radio is cyan instead of mint green"
echo "  - inactive radios are brighter and thinner"
echo "  - trigger chevron is neutral instead of green"
