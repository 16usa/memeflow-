...
PATCH_ID="MEMEFLOW_GMGN_SORT_STYLE_V25_3_EXACT"
OLD_VERSION="gmgn-sort-v25-2-final4-20260827"
NEW_VERSION="gmgn-sort-v25-3-exact-20260827"
DO_PUSH=0
...
grep -Fq "MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL4" system-tokens.js || {
  echo "ERROR: V25.2 FINAL4 JavaScript marker is missing." >&2
  exit 1
}

grep -Fq "MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL4" system-tokens.css || {
  echo "ERROR: V25.2 FINAL4 CSS marker is missing." >&2
  exit 1
}

if [[ "$(grep -Fc "$OLD_VERSION" system-tokens.html)" -ne 2 ]]; then
  echo "ERROR: expected exactly two V25.2 FINAL4 browser asset URLs." >&2
  exit 1
fi

# Patch-owned files must be clean.
...
# Never absorb unrelated staged work into the patch commit.
...
BACKUP_DIR=".patch-backups/gmgn-sort-v25-3-exact-$STAMP"
mkdir -p "$BACKUP_DIR"
...
rollback() {
  ...
  if [[ $rc -ne 0 ]]; then
    echo
    echo "Patch failed. Restoring V25.2 FINAL4 files..."
...
if "MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL4" not in ui:
    raise SystemExit(
        "PRECHECK FAILED: V25.2 FINAL4 JavaScript marker is missing."
    )

old_css_marker = "/* MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL4"
marker_at = css.find(old_css_marker)

if marker_at < 0:
    raise SystemExit(
        "PRECHECK FAILED: V25.2 FINAL4 CSS marker is missing."
    )

# The current sort style is expected to be one final canonical block.
...
if re.search(r"\.(?:mf-sort-|mf-age-list-v25)", prefix):
    raise SystemExit(
        ...
        "outside the current canonical style block."
    )
...
unexpected = [
    ...
    if marker != "MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL4"
]

if unexpected:
    raise SystemExit(
        "STYLE CONFLICT PRECHECK FAILED: another MEMEFLOW CSS block "
        "exists after the current sorting layer: "
        + ", ".join(unexpected)
    )

exact_css = r'''
/* MEMEFLOW_GMGN_SORT_STYLE_V25_3_EXACT
 * Exact visual alignment with the approved mockup.
 * This block fully replaces the previous sorting visual layer.
 */

/* Trigger: span the complete Search + Refresh grid width. */
.mf-sort-toolbar-v25 {
  display:block;
  grid-column:1 / -1;
  width:100%;
...
.mf-sort-trigger-v25.is-active {
  ...
  background:rgba(77,230,161,.026);
  color:#bac9d1;
}
...
/* Overlay: dim the page but keep it visually sharp like the mockup. */
...
/* Bottom sheet: shorter, inset, and fully rounded. */
.mf-sort-sheet-v25 {
...
.mf-sort-direction-v25 button.is-active {
  border-color:rgba(87,219,214,.52);
  background:rgba(87,219,214,.024);
  color:#e9f2f5;
  box-shadow:none;
}

/* One compact shared list surface. */
...
/* Mobile: keep the sheet floating inside the viewport. */
...
# Cache-bust both page assets with one version to preserve existing test contracts.
...
# Update exact browser-version assertions in all JavaScript regression files.
...
canonical_test = tests_dir / "token-sort-v25-3-exact.mjs"
...
# Keep every older token-sort regression compatible with the canonical visual test.
for path in sorted(tests_dir.glob("token-sort*.mjs")):
    if path == canonical_test:
        continue
...
# No stale FINAL4 browser-version assertion may remain in test code.
stale_tests = []
...
if stale_tests:
    raise SystemExit(
        "POSTCHECK FAILED: stale FINAL4 browser-version assertions remain in: "
        + ", ".join(stale_tests)
    )

if css.count(PATCH_ID) != 1:
    raise SystemExit(
        "POSTCHECK FAILED: V25.3 exact CSS layer must exist exactly once."
    )

css_path.write_text(css,encoding="utf-8")
html_path.write_text(html,encoding="utf-8")

print("V25.3 exact visual refinement installed.")
...
node --check tests/token-sort-v25-3-exact.mjs

echo "==> [3/7] Exact visual regression"
node tests/token-sort-v25-3-exact.mjs
...
if css.count(
    ...
    raise SystemExit(
        "ERROR: V25.3 exact sorting CSS layer is not unique."
    )

if "MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL4" in css:
    raise SystemExit(
        "ERROR: the previous FINAL4 sorting CSS layer still exists."
    )
...
if marker_at < 0:
    raise SystemExit(
        "ERROR: V25.3 exact CSS marker was not found."
    )
...
for rule in required_rules:
    if rule not in css:
        raise SystemExit(
            f"ERROR: required mockup-alignment rule is missing: {rule}"
        )

if html.count(
    "gmgn-sort-v25-3-exact-20260827"
) != 2:
    raise SystemExit(
        "ERROR: expected exactly two V25.3 browser asset URLs."
    )
...
echo
echo "V25.3 EXACT validated successfully."
...
if git diff --cached --quiet; then
  ...
  git commit -m "fix(token-flow): align sorting UI exactly with mockup"
fi

if [[ "$DO_PUSH" -eq 1 ]]; then
  echo
  echo "Pushing validated V25.3 EXACT commit..."
  git push
else
  echo
...
echo
echo "DONE V25.3 EXACT:"
echo "  - sort trigger spans Search plus Refresh width"
echo "  - trigger height matches the compact mockup"
echo "  - page remains sharp under the overlay"
echo "  - bottom sheet is shorter and starts lower"
echo "  - bottom sheet is inset and rounded on all sides"
echo "  - direction control is shorter"
echo "  - option rows are shorter"
echo "  - separators run across the full list width"
echo "  - system-tokens.js was not changed"
echo "  - package.json was not changed"
echo "  - unrelated runtime files remain untouched and unstaged"
