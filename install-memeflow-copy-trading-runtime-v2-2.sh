...
PATCH_NAME="MEMEFLOW Copy Trading runtime V2.2"
...
s,n=re.subn(
    ...
    'href="/trading.css?v=buy-ready-yellow-v1-copy-trading-v2-2-20260826"',
    s,
    count=1
)
if n!=1:
...
s,n=re.subn(
    ...
    'src="/trading.js?v=chart-match-panel-v3-copy-trading-v2-2-20260826"',
    s,
    count=1
)
if n!=1:
...
checks=[
 ...
 ("JS cache bumped", "chart-match-panel-v3-copy-trading-v2-2-20260826" in html),
 ("CSS cache bumped", "buy-ready-yellow-v1-copy-trading-v2-2-20260826" in html),
]
...
decoded=feed.find("const e=decodeTradeEvent(b);")
preprocess=feed.find("preprocessTrade?.(event)")
known=feed.find("const known=tokenFromStore(store,e.mint);")
dedupe=feed.find("const key=tradeEventKey(e,signature,i);")
if not (decoded>=0 and preprocess>decoded and known>preprocess and dedupe>known):
    raise SystemExit(
        "ERROR: scanner order must remain "
        "decode -> copy preprocess -> const known gate -> dedupe"
    )

print("Scanner order OK: decode -> copy preprocess -> const known gate -> dedupe")
...
if git diff --cached --quiet; then
  echo "No new changes to commit; runtime V2.2 already appears installed."
else
...
echo
echo "DONE."
echo "MEMEFLOW Copy Trading runtime V2.2 installed successfully."
