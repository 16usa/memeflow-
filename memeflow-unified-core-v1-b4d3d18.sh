...
PATCH_NAME="MEMEFLOW_UNIFIED_CORE_V1"
...
new="""    const deployed = openPositions.reduce(
    ...
    const tradableCapital =
      s.tradingCapital > 0
        ? Math.max(0, s.tradingCapital - s.feeReserve)
        : 0;
...
log "Backup: $BACKUP"
log "Key behavior:"
log "  - incomplete evidence => WAITING, not premature low-score BLOCKED"
...
log "  - feeReserve is enforced by the paper capital gate"
