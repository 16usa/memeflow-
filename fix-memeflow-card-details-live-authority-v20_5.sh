...
TEST="memeflow-app/tests/card-details-live-authority-v20_5.mjs"
OLDTEST="memeflow-app/tests/live-truth-no-dynamic-cache-v20_3.mjs"

for f in "$JS" "$HTML"; do
  [[ -f "$f" ]] || { echo "ERROR: missing $f"; exit 1; }
done

echo "=== MEMEFLOW CARD DETAILS LIVE AUTHORITY V20.5 ==="
...
for marker in required:
  if marker not in js:
    raise SystemExit("V20.5 REFUSED: current JS marker missing: "+marker)

if "MEMEFLOW_CARD_DETAILS_LIVE_AUTHORITY_V20_5" in js:
  raise SystemExit("V20.5 REFUSED: patch is already installed")

print("CURRENT_CARD_DETAILS_ARCHITECTURE_OK")
PY

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP=".memeflow-backups/card-details-live-authority-v20_5-$STAMP"
...
rollback(){
  ...
  if [[ $rc -ne 0 ]]; then
    ...
    if [[ -f "$BACKUP/$OLDTEST" ]]; then cp "$BACKUP/$OLDTEST" "$OLDTEST" || true; fi
    rm -f "$TEST"
    git reset -- "$JS" "$HTML" "$OLDTEST" "$TEST" >/dev/null 2>&1 || true
...
if start<0 or end<=start:
  raise SystemExit("V20.5 REFUSED: card analysis function boundaries not found")
...
for marker in (
  ...
  if marker not in old:
    raise SystemExit("V20.5 REFUSED: existing card analysis shape changed: "+marker)

new=r'''// MEMEFLOW_CARD_DETAILS_LIVE_AUTHORITY_V20_5
...
html2,n=re.subn(
  ...
  'src="/system-tokens.js?v=card-live-authority-v20-5-20260902"',
  html,
  count=1
)
if n!=1:
  raise SystemExit(f"V20.5 REFUSED: system-tokens.js asset URL count={n}")
HTML.write_text(html2,encoding="utf-8")

# The V20.3 regression had a brittle assertion for the exact historical
# browser asset revision. Keep the semantic invariant (cache-busted JS URL)
# without pinning future patches to the old v20-2 string.
oldtest=Path("memeflow-app/tests/live-truth-no-dynamic-cache-v20_3.mjs")
if oldtest.exists():
  test_text=oldtest.read_text(encoding="utf-8")
  exact="assert.match(html,/system-tokens\\.js\\?v=live-truth-v20-2-20260902/);"
  stable="assert.match(html,/system-tokens\\.js\\?v=[^\\\"']+/);"
  if exact in test_text:
    test_text=test_text.replace(exact,stable,1)
    oldtest.write_text(test_text,encoding="utf-8")
  elif stable not in test_text:
    raise SystemExit("V20.5 REFUSED: V20.3 asset assertion has an unknown shape")

print("V20_5_TRANSFORM_OK")
PY
...
git diff --check -- "$JS" "$HTML" "$OLDTEST" "$TEST"

echo "VALIDATION_OK"

git reset >/dev/null
git add "$JS" "$HTML" "$OLDTEST" "$TEST"

BAD="$(
  git diff --cached --name-only |
  grep -Ev '^memeflow-app/(system-tokens\.js|system-tokens\.html|tests/live-truth-no-dynamic-cache-v20_3\.mjs|tests/card-details-live-authority-v20_5\.mjs)$' ||
  true
)"
...
git commit -m "fix: unify details with live token truth and stabilize regression"
