...
echo "=== MEMEFLOW UNIFIED ANALYSIS ENGINE V21.5 ==="

[[ "$(git branch --show-current)" == "$BRANCH" ]] || {
  echo "V21.5 REFUSED: expected branch $BRANCH"
  echo "actual: $(git branch --show-current)"
  exit 1
}

[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V21.5 REFUSED: audited HEAD changed"
...
for f in "${FILES[@]}"; do
  [[ -f "$f" ]] || { echo "V21.5 REFUSED: missing $f"; exit 1; }
  git diff --quiet -- "$f" || { echo "V21.5 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V21.5 REFUSED: staged changes in $f"; exit 1; }
done
...
for file,markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(f"V21.5 REFUSED: audited marker missing in {file}: {marker}")
print("AUDITED_V21_INPUT_OK")
PY

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP=".memeflow-backups/unified-analysis-engine-v21-5-$STAMP"
mkdir -p "$BACKUP"
...
def once(text,old,new,label):
    ...
    if count!=1:
        raise SystemExit(f"V21.5 REFUSED: {label}: expected 1 match, got {count}")
...
if n!=1: raise SystemExit(f"V21.5 REFUSED: smart rank block count={n}")
...
if mstart<0 or mend<=mstart: raise SystemExit("V21.5 REFUSED: mergedCandidates boundaries")
old_merge=tr[mstart:mend]
for marker in ("MEMEFLOW_TERMINAL_WATCH_DUPLICATE_MERGE_V37","liveWatchCandidates","__pipelineWatch"):
    if marker not in old_merge: raise SystemExit("V21.5 REFUSED: old Terminal merge shape: "+marker)
...
if lstart<0 or lend<=lstart: raise SystemExit("V21.5 REFUSED: loadCandidates boundaries")
...
for marker in (
 ...
 "MEMEFLOW_TERMINAL_WATCH_RESILIENT_V30"
    ...
    if marker not in old_load: raise SystemExit("V21.5 REFUSED: old Terminal load shape: "+marker)
...
"""
if old not in t: raise SystemExit("V21.5 REFUSED: opportunity-engine assertion anchor missing")
...
assert(qualified.amountSol > 0);
"""
if old not in t: raise SystemExit("V21.5 REFUSED: adaptive assertion anchor missing")
p.write_text(t.replace(old,new,1),encoding="utf-8")
PY
...
import re
updates={
 "memeflow-app/system-tokens.html":(r'/system-tokens\.js\?v=[^"]+','/system-tokens.js?v=unified-analysis-v21-50260903'),
 "memeflow-app/trading.html":(r'/trading\.js\?v=[^"]+','/trading.js?v=unified-analysis-v21-50260903'),
 "memeflow-app/system.html":(r'/system\.js\?v=[^"]+','/system.js?v=unified-analysis-v21-50260903'),
 "memeflow-app/settings.html":(r'/settings-page\.js\?v=[^"]+','/settings-page.js?v=unified-analysis-v21-50260903')
}
for file,(pattern,replacement) in updates.items():
    p=Path(file)
    t=p.read_text(encoding="utf-8")
    t,n=re.subn(pattern,replacement,t,count=1)
    if n!=1: raise SystemExit(f"V21.5 REFUSED: asset update {file}: {n}")
    p.write_text(t,encoding="utf-8")
print("ASSET_VERSIONS_OK")
PY

echo
echo "=== V21.5 REGRESSION CONTRACT PRECHECK ==="
...
echo
echo "=== V21.5 FULL PROJECT TEST SUITE ==="
(
 cd memeflow-app
 npm test
)
...
echo
echo "=== V21.5 STATIC WHOLE-PROJECT AUDIT ==="
python3 - <<'PY'
from pathlib import Path
...
asset_checks={
 "memeflow-app/system-tokens.html":"system-tokens.js?v=unified-analysis-v21-50260903",
 "memeflow-app/trading.html":"trading.js?v=unified-analysis-v21-50260903",
 "memeflow-app/system.html":"system.js?v=unified-analysis-v21-50260903",
 "memeflow-app/settings.html":"settings-page.js?v=unified-analysis-v21-50260903"
}
...
git commit -m "refactor: unify MEMEFLOW analysis into one score engine v21.5"
