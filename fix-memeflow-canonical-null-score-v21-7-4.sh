...
echo "=== MEMEFLOW CANONICAL NULL SCORE V21.7.4 ==="

[[ "$(git branch --show-current)" == "$BRANCH" ]] || {
  echo "V21.7.4 REFUSED: expected branch $BRANCH"
  echo "actual: $(git branch --show-current)"
  exit 1
}

[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V21.7.4 REFUSED: audited HEAD changed"
...
for f in "${FILES[@]}"; do
  [[ -f "$f" ]] || { echo "V21.7.4 REFUSED: missing $f"; exit 1; }
  git diff --quiet -- "$f" || { echo "V21.7.4 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V21.7.4 REFUSED: staged changes in $f"; exit 1; }
done

[[ ! -e "$NEW_TEST" ]] || {
  echo "V21.7.4 REFUSED: $NEW_TEST already exists"
  exit 1
}
...
for file,markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(f"V21.7.4 REFUSED: audited marker missing in {file}: {marker}")

print("AUDITED_V21_7_INPUT_OK")
PY

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP=".memeflow-backups/canonical-null-score-v21-7-4-$STAMP"
mkdir -p "$BACKUP"
...
def once(text,old,new,label):
    ...
    if count!=1:
        raise SystemExit(f"V21.7.4 REFUSED: {label}: expected 1 exact match, got {count}")
...
grep -q "MEMEFLOW_STATIC_AUDIT_EXACT_ASSERT_V21_7_4" "$0"
...
echo
echo "=== V21.7.4 FULL PROJECT TEST SUITE ==="
(
  cd memeflow-app
  npm test
)
...
echo
echo "=== V21.7.4 STATIC CONTRACT AUDIT ==="
python3 - <<'PY'
from pathlib import Path
...
# MEMEFLOW_STATIC_AUDIT_EXACT_ASSERT_V21_7_4
...
if "assert.doesNotMatch(" not in lt or (
    "/tradeEligible:isOpen\\\\?true:eligible&&liveTruth\\\\.pass===true/" not in lt
):
