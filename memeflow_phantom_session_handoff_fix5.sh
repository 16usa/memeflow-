...
echo "== MEMEFLOW Phantom Session Handoff — FIX 5 =="
...
BACKUP=".phantom-session-handoff-fix5-backup-$STAMP"
mkdir -p "$BACKUP"

for f in live-bootstrap.mjs phantom-connect-client.mjs settings.html package.json; do
  [[ -f "$f" ]] && cp -p "$f" "$BACKUP/$f"
done

echo "Backup: $PWD/$BACKUP"

cat > patch-session-handoff-fix5.mjs <<'EOF_PATCH'
...
echo "Applying session handoff patch..."
node patch-session-handoff-fix5.mjs
...
grep -q "MEMEFLOW_PHANTOM_SESSION_HANDOFF_FIX5" live-bootstrap.mjs
grep -q "/api/session/handoff/redeem" live-bootstrap.mjs
grep -q "redeemSessionHandoffIfPresent" phantom-connect-client.mjs
grep -q "phase1-fix5-session-handoff-20260827" settings.html

echo
echo "== PHANTOM SESSION HANDOFF FIX 5 INSTALLED =="
echo
echo "This fixes the current error:"
echo "  LIVE trading environment requires an active Pro subscription or owner entitlement."
echo
echo "Why it happened:"
echo "  Safari and Phantom in-app browser had different mf_session cookies."
echo
echo "Now:"
echo "  Safari -> Open in Phantom creates a 2-minute one-time handoff token."
echo "  Token is carried in the URL fragment (#), not sent as a referrer."
echo "  Phantom redeems it once and receives the SAME HttpOnly MEMEFLOW session."
echo "  Owner/Pro entitlement, settings and user state therefore follow into Phantom."
echo
echo "STOP -> RUN."
echo "Then start from normal Safari again:"
echo "  System Settings -> Use Phantom wallet -> Phantom opens -> connect -> Approve each trade."
