...
PATCH_ID = "MEMEFLOW_LIVE_TOKEN_STATES_V6"
...
BACKUP = ROOT / f".memeflow-live-token-states-v6-backup-{stamp}"
...
try:
 ...
 if(url.pathname==='/api/system/live-token-states'&&req.method==='GET'){
  ...
  return json(res,200,{
    ...
    total:_views.length,
    limit:_lim,
    source:'system-live-token-states-v6',
...
"""
    ...
    new_load_tokens = r"""async function loadTokens() {
  ...
  try {
    ...
    js = js[:start] + new_load_tokens + js[end:]
    ...
    checks = [
        ...
        (server_now, "source:'system-live-token-states-v6'", "backend source marker"),
        ...
        (html_now, "live-token-states-v6", "Safari cache bust"),
    ]
