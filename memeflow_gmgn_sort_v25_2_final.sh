...
PATCH_ID="MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL"
OLD_VERSION="gmgn-sort-v25-1-20260827"
NEW_VERSION="gmgn-sort-v25-2-final-20260827"
DO_PUSH=0
...
# Strict V25.1 -> V25.2 FINAL prerequisites.
...
BACKUP_DIR=".patch-backups/gmgn-sort-v25-2-final-$STAMP"
mkdir -p "$BACKUP_DIR/token-sort-tests"

cp -p system-tokens.js system-tokens.css system-tokens.html "$BACKUP_DIR/"

if [[ -d tests ]]; then
  find tests -maxdepth 1 -type f -name 'token-sort*.mjs' -print0 \
    | while IFS= read -r -d '' f; do
        cp -p "$f" "$BACKUP_DIR/token-sort-tests/$(basename "$f")"
      done
fi
...
rollback() {
  ...
  if [[ $rc -ne 0 ]]; then
    ...
    mkdir -p tests
    rm -f tests/token-sort*.mjs

    if compgen -G "$BACKUP_DIR/token-sort-tests/*.mjs" >/dev/null; then
      cp -p "$BACKUP_DIR"/token-sort-tests/*.mjs tests/
    fi
...
final_css = r'''
/* MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL
...
# ---------------------------------------------------------------------------
# 7. One canonical regression. package.json is NOT modified.
#    Any legacy token-sort test referenced by the existing npm test script is
#    converted into a tiny compatibility wrapper that imports this test.
# ---------------------------------------------------------------------------
canonical_test = tests_dir / "token-sort-v25-2-final.mjs"
...
assert.match(
  html,
  /system-tokens\.js\?v=gmgn-sort-v25-2-final-20260827/
);

assert.match(
  html,
  /system-tokens\.css\?v=gmgn-sort-v25-2-final-20260827/
);

assert.doesNotMatch(
  html,
  /gmgn-sort-v25-1-20260827/
);

console.log('token sort v25.2 final ok');
''',
    encoding="utf-8"
)
...
# Known prior sort tests are also kept compatible if they already exist.
for known in (
    "token-sort-ui-v25.mjs",
    "token-sort-style-v25-1.mjs",
    "token-sort-style-v25-2.mjs",
    "token-sort-v25-2-clean.mjs",
):
    path = tests_dir / known
    if path.exists():
        referenced_sort_tests.add(known)
...
for name in sorted(referenced_sort_tests):
    ...
    wrapper.write_text(
        "import './token-sort-v25-2-final.mjs';\n",
        encoding="utf-8"
    )
...
print("V25.2 FINAL visual patch installed.")
...
node --check tests/token-sort-v25-2-final.mjs

echo "==> [3/7] Canonical sort regression"
node tests/token-sort-v25-2-final.mjs
...
if html.count(
    "gmgn-sort-v25-2-final-20260827"
) != 2:
    raise SystemExit(
...
echo
echo "V25.2 FINAL validated successfully."
...
if [[ "$DO_PUSH" -eq 1 ]]; then
  echo
  echo "Pushing validated V25.2 FINAL commit..."
  git push
else
  echo
