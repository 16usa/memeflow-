...
const VERSION = 'MEMEFLOW_PRIMARY_IDENTITY_STABLE_V8';
const STYLE_START = '<!-- MF_PRIMARY_IDENTITY_STABLE_V8_STYLE_START -->';
const STYLE_END = '<!-- MF_PRIMARY_IDENTITY_STABLE_V8_STYLE_END -->';
const SCRIPT_START = '<!-- MF_PRIMARY_IDENTITY_STABLE_V8_SCRIPT_START -->';
const SCRIPT_END = '<!-- MF_PRIMARY_IDENTITY_STABLE_V8_SCRIPT_END -->';

const NEW_HEAD = `<div class="token-head mf-primary-stable-head" data-mf-primary-layout="v8"><div class="mf-primary-stable-identity"><div class="mf-primary-stable-avatar" id="primaryAvatar" aria-hidden="true" hidden><span id="primaryAvatarFallback" hidden></span><img id="primaryAvatarImage" alt="" hidden></div><div class="mf-primary-stable-copy"><div class="token-name" id="primaryName">—</div><div class="score-caption" id="primaryMeta">No token selected</div></div></div><div class="mf-primary-stable-score"><div class="big-score" id="primaryScore">—</div><div class="score-caption">AI SCORE</div></div></div>`;

const STYLE = `${STYLE_START}
<style id="mf-primary-identity-stable-v8-style">
/*
...
/*
  Critical V8 rule: the static layout has exactly two direct children in the
...
<script id="mf-primary-identity-stable-v8-script">
(() => {
  'use strict';
  if (window.__MF_PRIMARY_IDENTITY_STABLE_V8__) return;
  window.__MF_PRIMARY_IDENTITY_STABLE_V8__ = true;
...
function stripBlocks(html) {
  const blockPatterns = [
    ...
    /<!-- MF_PATCH_MANUAL_TOKEN_LOGO_AI_SCORE_START -->[\s\S]*?<!-- MF_PATCH_MANUAL_TOKEN_LOGO_AI_SCORE_END -->/g,
  ...
  const oldIds = [
    'mf-manual-token-logo-ai-score-style','mf-manual-token-logo-ai-score-script',
...
console.log('V8: one authoritative token avatar; duplicate legacy logo visuals are suppressed.');
console.log('V8: token name + symbol stay in one fixed text column; AI SCORE stays fixed at right.');
console.log('V8: no MutationObserver, no setInterval, no transform positioning.');
