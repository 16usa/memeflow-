...
# Only production/runtime files count as live references. Versioned regression tests
# intentionally preserve old selectors/assets and must not block a production cleanup.
REFERENCE_SCAN_IGNORED_DIRS = {"tests", "test", "__tests__", "node_modules", "coverage"}
REFERENCE_SCAN_SUFFIXES = {".html", ".css", ".js", ".mjs", ".cjs", ".json"}


def _is_runtime_reference_path(path: Path) -> bool:
    try:
        rel = path.relative_to(APP)
    except ValueError:
        return False
    if any(part in REFERENCE_SCAN_IGNORED_DIRS for part in rel.parts[:-1]):
        return False
    return path.suffix.lower() in REFERENCE_SCAN_SUFFIXES
...
def ensure_no_live_reference(term: str, exclude_names: set[str] | None = None) -> None:
    ...
    refs = []
    for p in APP.rglob("*"):
        if not p.is_file() or p.name in exclude_names or not _is_runtime_reference_path(p):
            continue
        try:
...
""",
    ...
    new, count = re.subn(
        ...
        r'\1open-position-popover-canonical-v4-20260909\2',
        text,
        count=1,
    )
    if count != 1:
    ...
    cache_bust("trading.css", "open-positions-canonical-cleanup-v4-20260909")
    cache_bust("trading-light-polish-v61.css", "open-positions-cleanup-v4-20260909")
    cache_bust("trading-visual-hierarchy-v67.css", "open-positions-cleanup-v4-20260909")
    cache_bust("memeflow-dark-x-surface-v131.css", "open-positions-cleanup-v4-20260909")
    cache_bust("memeflow-trading-white-surfaces-v154.css", "open-positions-cleanup-v4-20260909")
