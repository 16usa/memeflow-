...
trap 'echo "ERROR: AI Workspace V4.1 failed; restoring exact pre-install index.html."; rollback' ERR
...
# ---- wrap deep content only, SCOPED TO THE SINGLE #ai-analysis NODE ----
# V4.1 intentionally ignores every .mission-actions outside #ai-analysis.
ai_open = re.search(
    r'<details\b(?=[^>]*\bid=["\']ai-analysis["\'])[^>]*>',
    src, flags=re.I
)
if not ai_open:
    raise SystemExit("AI host opening tag not found before deep-workspace rewrite.")

# At this point the V3/V4 host contains no nested <details> yet, so the first
# closing </details> after the host opening is the host close.
ai_close_start = src.find("</details>", ai_open.end())
if ai_close_start < 0:
    raise SystemExit("AI host closing </details> not found.")

ai_end = ai_close_start + len("</details>")
...
if not tree_start:
    raise SystemExit("decisionTree not found inside #ai-analysis.")

actions_open_re = re.compile(
    r'<div\b[^>]*\bclass=["\'][^"\']*\bmission-actions\b[^"\']*["\'][^>]*>',
    flags=re.I
)
actions_open = list(actions_open_re.finditer(ai))
if len(actions_open) != 1:
    raise SystemExit(
        f"Expected one mission-actions inside #ai-analysis; found {len(actions_open)}."
    )
...
def matching_div_end(html, open_match):
    ...
    for tag in tag_re.finditer(html, open_match.start()):
        if tag.group(0).lower().startswith("<div"):
            ...
            if depth == 0:
                return tag.end()
    raise SystemExit("Unbalanced mission-actions <div> inside #ai-analysis.")

actions_end = matching_div_end(ai, actions_open[0])

if tree_start.start() >= actions_open[0].start():
    raise SystemExit("Unexpected AI structure: decisionTree must precede mission-actions.")
...
# Rewrite only the isolated AI node. Nothing outside it is structurally touched.
ai = ai[:actions_end] + deep_close + ai[actions_end:]
ai = ai[:tree_start.start()] + deep_open + ai[tree_start.start():]
...
print("AI Workspace V4.1 source consolidation prepared.")
...
echo
echo "OK: AI ANALYSIS WORKSPACE V4.1 installed cleanly."
echo
