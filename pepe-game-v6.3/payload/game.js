(()=>{
  ...
  const ui={
    ...
    multiplier:$('#multiplier'),peakHud:$('#peakHud'),drawdownHud:$('#drawdownHud'),thrustHud:$('#thrustHud'),autoDistance:$('#autoDistance'),stopDistance:$('#stopDistance'),flightAssist:$('#flightAssist'),flightAssistState:$('#flightAssistState'),flightAssistText:$('#flightAssistText'),cashoutTelemetry:$('#cashoutTelemetry'),cashoutWindow:$('#cashoutWindow'),cashoutValue:$('#cashoutValue'),cashoutProfit:$('#cashoutProfit'),cashoutCapture:$('#cashoutCapture'),cashoutDistance:$('#cashoutDistance'),cashoutMeterFill:$('#cashoutMeterFill'),cashoutMeterPeak:$('#cashoutMeterPeak'),world:$('#world'),fx:$('#fxCanvas'),
    ...
    center:$('#centerState'),centerKicker:$('#centerKicker'),centerValue:$('#centerValue'),centerLabel:$('#centerLabel'),milestone:$('#milestone'),milestoneValue:$('#milestoneValue'),milestoneText:$('#milestoneText'),stale:$('#staleCover'),shockwave:$('#shockwave'),emergencyFlash:$('#emergencyFlash'),secureFlash:$('#secureFlash'),stageTransition:$('#stageTransition'),stageTransitionLabel:$('#stageTransitionLabel'),stageTransitionMeta:$('#stageTransitionMeta'),targetReticle:$('#targetReticle'),
  ...
  const game={
    ...
    dangerLevel:'none',lastDangerHapticAt:0,focusMode:false,cameraScale:1,cameraY:0,cameraX:0,engineScale:1,completionSeq:0,pendingResultId:null,searchStartedAt:0,selectorDiag:null,visualTilt:-3,acceleration:0,lastReversalHapticAt:0,bankTimer:null,
    raf:null,lifecyclePaused:false,flightState:'idle',localFeedVisualStale:false,lastStage:'ground',stageTimer:null,historyClearTimer:null,lastVisualAt:0,fallbackInFlight:false,resyncPromise:null,startCancelArmedAt:0,resetInFlight:false,clearHistoryInFlight:false
  };
  ...
  function syncVisualActivity(){
    const active=game.pageVisible&&!game.lifecyclePaused&&['live','settling'].includes(game.mode);
    if(active&&!reducedMotion){game.fx?.resume?.();startVisualLoop();}else{game.fx?.pause?.();stopVisualLoop();}
  }
  ...
  function setMode(mode,message){
    game.mode=mode;ui.game.dataset.state=mode;
    const focus=['live','settling'].includes(mode);game.focusMode=focus;ui.game.dataset.focus=focus?'live':'normal';
    const labels={idle:'READY',searching:'SCANNING',live:'LIVE',settling:'CASHING OUT',complete:'COMPLETE'};ui.stateLabel.textContent=labels[mode]||mode.toUpperCase();if(message)ui.stateMessage.textContent=message;setFlightState(resolveFlightState());syncButtons();syncVisualActivity();syncClockActivity();void syncWakeLock();
  }
  function syncButtons(){
    ...
    const cancelArmed=searching&&Date.now()>=game.startCancelArmedAt;ui.start.disabled=live||settling||complete;ui.mobileStart.disabled=live||settling||complete;ui.start.querySelector('b').textContent=searching?(cancelArmed?'CANCEL':'SEARCHING…'):'START';ui.mobileStart.querySelector('b').textContent=searching?(cancelArmed?'CANCEL':'SEARCHING…'):'START';
  ...
  function updateRiskPreview(){
    const bet=Math.max(0,currentBet()),auto=num(ui.auto.value)||0,stop=num(ui.stop.value)||0,[profile,label]=targetProfile(auto);
  ...
  function updateTriggerLines(){const locked=game.session&&['LIVE','COMPLETE'].includes(game.session.state);const auto=num(locked?game.session.autoCashout:ui.auto.value)||0,stop=num(locked?game.session.stopLoss:ui.stop.value)||0;ui.autoTrigger.hidden=!(auto>1);if(auto>1){ui.autoTrigger.style.bottom=lineBottom(auto);ui.autoTriggerLabel.textContent=`AUTO ${auto.toFixed(2)}×`;}ui.stopTrigger.hidden=!(stop>0&&stop<1);if(stop>0&&stop<1){ui.stopTrigger.style.bottom=lineBottom(stop);ui.stopTriggerLabel.textContent=`STOP ${stop.toFixed(2)}×`;}updateRiskPreview();}
  ...
  function updateFlightAssist(m){
    ...
    if(!s||s.state!=='LIVE'){ui.flightAssist.dataset.tone='neutral';ui.flightAssistState.textContent='STANDBY';ui.flightAssistText.textContent='Waiting for a server-locked launch.';return;}
    if(s.feedFresh===false){ui.flightAssist.dataset.tone='caution';ui.flightAssistState.textContent='FEED HOLD';ui.flightAssistText.textContent='Fresh quote required before manual cash out can unlock.';return;}
    ...
    ui.flightAssist.dataset.tone=toneName;ui.flightAssistState.textContent=state;ui.flightAssistText.textContent=copy;
  }
  ...
  function showStageTransition(stage,label,m){
    if(!ui.stageTransition)return;game.lastStage=stage;ui.stageTransitionLabel.textContent=label;ui.stageTransitionMeta.textContent=`${m.toFixed(2)}×`;ui.stageTransition.classList.remove('is-active');if(reducedMotion||game.mode!=='live')return;void ui.stageTransition.offsetWidth;ui.stageTransition.classList.add('is-active');clearTimeout(game.stageTimer);game.stageTimer=setTimeout(()=>ui.stageTransition?.classList.remove('is-active'),1050);
  }
  ...
  function renderResultTrace(){
    ...
    if(pts.length<2){
      ui.resultTrace.dataset.state='empty';ui.resultTracePath.setAttribute('d','M 24 118 L 976 118');ui.resultTracePeak.setAttribute('cx','24');ui.resultTracePeak.setAttribute('cy','118');ui.resultTraceExit.setAttribute('cx','976');ui.resultTraceExit.setAttribute('cy','118');ui.resultTraceMeta.textContent='Live path unavailable after reload';return;
    }
    ...
    const coords=pts.map(p=>[24+(p.t-t0)/(t1-t0)*952,132-(p.m-minM)/range*104]);
  ...
  function renderVisual(multiplier){
    ...
    const [stage,label]=stageFor(m);if(stage!==game.lastStage)showStageTransition(stage,label,m);ui.game.dataset.stage=stage;ui.stage.textContent=label;const bottom=7+levelNorm(m)*62;ui.rocket.style.bottom=`${bottom.toFixed(2)}%`;if(ui.altimeterMarker){ui.altimeterMarker.style.bottom=lineBottom(m);const peak=Math.max(m,num(game.session?.peak)||m);ui.altimeterPeak.style.bottom=lineBottom(peak);ui.altimeterValue.textContent=`${m.toFixed(2)}× · ${label}`;}
    ...
    const tiltTarget=clamp(-3-v*18-game.acceleration*46,-18,12);game.visualTilt+= (tiltTarget-game.visualTilt)*(Math.abs(tiltTarget-game.visualTilt)>8?.18:.11);const bob=game.mode==='live'&&!reducedMotion?Math.sin(performance.now()/510)*(0.6+Math.min(2.1,Math.abs(v)*12)):0;ui.rocket.style.setProperty('--rocket-bob',`${bob.toFixed(2)}px`);ui.rocket.style.transform=`translate(-50%,var(--rocket-bob,0px)) rotate(${game.visualTilt.toFixed(2)}deg)`;
    ...
    if(danger==='high'&&game.dangerLevel!=='high'&&Date.now()-game.lastDangerHapticAt>2600){game.lastDangerHapticAt=Date.now();haptic([18,32,18]);tone(165,.09,'sawtooth',.012,-35);}game.dangerLevel=danger;setFlightState(resolveFlightState());
  ...
  function checkMilestone(m){if(game.mode!=='live'||game.session?.state!=='LIVE')return;for(const [level,label] of [[1.08,'Liftoff'],[1.2,'Cloud deck cleared'],[1.5,'Stratosphere'],[2,'Orbit secured'],[3,'Lunar slingshot'],[5,'Deep-space burn'],[10,'Hyperspace']]){if(m>=level&&!game.milestones.has(level)){game.milestones.add(level);showMilestone(level,label);}}}
  ...
  async function countdown(s){if(!s?.id||game.launchSeen.has(s.id))return;game.launchSeen.add(s.id);if(game.launchSeen.size>24)game.launchSeen=new Set([s.id]);const seq=++game.countdownSeq;const valid=()=>seq===game.countdownSeq&&game.pageVisible&&!game.lifecyclePaused&&game.session?.id===s.id&&game.session?.state==='LIVE';const step=async(phase,kicker,value,label,ms)=>{if(!valid())return false;ui.game.dataset.launch=phase;ui.center.hidden=false;ui.centerKicker.textContent=kicker;ui.centerValue.textContent=value;ui.centerLabel.textContent=label;await wait(reducedMotion?55:ms);return valid();};lockSelector(s);if(!await step('verified','TARGET VERIFIED','PASS',text(s.token?.symbol,s.token?.name,'MEMEFLOW'),150)){ui.center.hidden=true;return;}tone(420,.045,'triangle',.012,35);if(!await step('locked','ENTRY LOCKED','1.00×','SERVER PAPER ENTRY',170)){ui.center.hidden=true;return;}tone(520,.045,'triangle',.013,45);ui.game.dataset.launch='ignition';ui.centerKicker.textContent='IGNITION';ui.centerLabel.textContent='ENGINE START';for(const value of ['3','2','1']){if(!valid()){ui.center.hidden=true;ui.game.dataset.launch='idle';return;}ui.centerValue.textContent=value;tone(290+Number(value)*70,.05,'triangle',.016,50);await wait(reducedMotion?55:165);}if(!valid()){ui.center.hidden=true;ui.game.dataset.launch='idle';return;}ui.game.dataset.launch='go';ui.centerKicker.textContent='LAUNCH';ui.centerValue.textContent='GO';ui.centerLabel.textContent='LIVE MARKET';sfx('launch');haptic([22,28,34]);cinematicKick(1.25);pulseShockwave(1);await wait(reducedMotion?70:240);if(valid()){ui.center.hidden=true;ui.game.dataset.launch='live';setSelectorState('live','LIVE');}}
  ...
  function apply(payload,{allowResult=true}={}){
    if(!payload)return false;const incomingEpoch=text(payload.engineEpoch);if(incomingEpoch&&game.engineEpoch&&incomingEpoch!==game.engineEpoch)resetOrderingForEngineEpoch(incomingEpoch);else if(incomingEpoch&&!game.engineEpoch)game.engineEpoch=incomingEpoch;const eventSeq=Math.max(0,Math.floor(num(payload.eventSeq)||0)),stateRevision=Math.max(0,Math.floor(num(payload.stateRevision)||0)),serverTime=num(payload.serverTime),previousStateRevision=game.lastStateRevision;
    if(eventSeq>0&&game.lastEventSeq>0&&eventSeq<game.lastEventSeq)return false;if(eventSeq>0)game.lastEventSeq=Math.max(game.lastEventSeq,eventSeq);
    if(stateRevision>0&&previousStateRevision>0&&stateRevision<previousStateRevision)return false;const revisionAdvanced=stateRevision>previousStateRevision;if(stateRevision>0)game.lastStateRevision=Math.max(game.lastStateRevision,stateRevision);
    if(serverTime!==null&&game.lastPayloadServerTime>0&&serverTime+250<game.lastPayloadServerTime&&!revisionAdvanced)return false;if(serverTime!==null)game.lastPayloadServerTime=Math.max(game.lastPayloadServerTime,serverTime);
    game.lastEventAt=Date.now();if(payload.history)renderHistory(payload.history);if(payload.stats)renderStats(payload.stats);if(payload.balance!==undefined)ui.balance.textContent=money(payload.balance);if(payload.selector){const d=payload.selector;game.selectorDiag=d;if(d.selectedMint){ui.selectorText.textContent=`MEMEFLOW BUY READY ${d.buyReady||0} · usable ${d.eligible||0} · selected score ${num(d.selectedScore)===null?'—':Math.round(num(d.selectedScore))}`;}else{ui.selectorText.textContent=`MEMEFLOW BUY READY ${d.buyReady||0} · usable ${d.eligible||0} · missing token ${d.noToken||0} · missing price ${d.noPrice||0}`;}}
    const accepted=payload.session?applySession(payload.session,serverTime??Date.now()):true;if(payload.session&&!accepted)return false;const s=payload.session;game.status={...(game.status||{}),...payload};
    if(!s){game.countdownSeq++;ui.center.hidden=true;game.session=null;game.lastSessionId=null;game.lastSessionUpdatedAt=0;game.lastSessionRevision=0;game.targetMultiplier=1;game.displayMultiplier=1;game.points=[];resetMotionState();game.selectorDiag=null;ui.game.dataset.launch='idle';ui.game.dataset.outcome='none';game.pendingResultId=null;game.completionSeq++;ui.shockwave?.classList.remove('is-active');ui.world.style.setProperty('--camera-scale','1');ui.world.style.setProperty('--camera-y','0px');ui.world.style.setProperty('--camera-x','0px');ui.world.style.setProperty('--engine-scale','1');ui.world.style.setProperty('--speed-opacity','.12');ui.world.style.setProperty('--danger-opacity','0');ui.world.style.setProperty('--boost-opacity','0');ui.world.style.setProperty('--scene-energy','0');ui.world.style.setProperty('--pf-x','0px');ui.world.style.setProperty('--pf-y','0px');ui.world.style.setProperty('--pm-x','0px');ui.world.style.setProperty('--pm-y','0px');ui.world.style.setProperty('--pn-x','0px');ui.world.style.setProperty('--pn-y','0px');ui.world.style.setProperty('--smoke-opacity','.18');ui.world.style.setProperty('--vignette-opacity','.22');ui.rocket.style.setProperty('--rocket-bob','0px');document.title='MEMEFLOW · Pepe Rocket';drawTrace();if(game.mode!=='searching'){if(!ui.result.hidden){game.resultFxSeq++;ui.result.hidden=true;ui.game.inert=false;ui.game.dataset.outcome='none';ui.game.dataset.launch='idle';ui.game.dataset.cashzone='idle';ui.game.dataset.cashpulse='idle';ui.resultCard.dataset.tone='';ui.shockwave?.classList.remove('is-active');game.showingResult=null;}setMode('idle','Set a paper stake and launch when ready.');resetToken();previewStake();}return true;}
    ...
    if(s.state==='COMPLETE'){game.countdownSeq++;ui.center.hidden=true;ui.game.dataset.launch='idle';setSelectorState('complete','SETTLED');setMode('complete',s.voided?'Market feed lost · paper stake returned safely.':`${String(s.reason||'ROUND COMPLETE').replaceAll('_',' ')} · server settled.`);if(allowResult)void queueResult(s);}return true;
  }
  ...
  function connectStream(){
    ...
    const es=new EventSource('/api/game/stream',{withCredentials:true});game.stream=es;game.streamHealthy=false;ui.streamState.textContent='Game stream connecting';startFallback();
    const receive=(event)=>{try{const data=JSON.parse(event.data||'{}');const accepted=apply(data);if(accepted===false){game.streamHealthy=false;ui.streamState.textContent='Stream packet ignored · fallback sync active';startFallback();return;}game.streamHealthy=true;ui.network.hidden=true;ui.streamState.textContent='Server-authoritative game stream live';stopFallback();}catch{game.streamHealthy=false;startFallback();}};
  ...
  function startFallback(){if(game.fallback)return;const sync=async()=>{if(!game.pageVisible||navigator.onLine===false||game.fallbackInFlight)return;game.fallbackInFlight=true;try{const x=await api('/api/game/status');apply(x);if('EventSource' in window&&(!game.stream||game.stream.readyState===EventSource.CLOSED))connectStream();}catch{}finally{game.fallbackInFlight=false;}};void sync();game.fallback=setInterval(()=>void sync(),4000);}
  ...
  function resyncStatus({connect=true}={}){
    ...
    game.resyncPromise=api('/api/game/status').then(status=>{apply(status);if(connect)connectStream();return status;}).catch(error=>{startFallback();throw error;}).finally(()=>{game.resyncPromise=null;});
  ...
  function createFx(){
    const canvas=ui.fx,ctx=canvas.getContext('2d');if(!ctx)return null;let w=1,h=1,dpr=1,running=false,raf=null,lastFrame=0;const stars=[];const saveData=navigator.connection?.saveData===true;const desiredCount=()=>saveData?24:w<430?34:w<700?44:w<1100?64:82;
    function resize(){const r=canvas.getBoundingClientRect();w=Math.max(1,r.width);h=Math.max(1,r.height);dpr=Math.min(1.5,devicePixelRatio||1);canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);const count=desiredCount();while(stars.length<count)stars.push({x:Math.random()*w,y:Math.random()*h,s:.4+Math.random()*1.2,a:.18+Math.random()*.55,z:.4+Math.random()});if(stars.length>count)stars.splice(count);for(const s of stars){s.x=clamp(s.x,0,w);s.y=clamp(s.y,-18,h);}}
    function frame(ts=performance.now()){raf=null;if(!running)return;if(ts-lastFrame<32){raf=requestAnimationFrame(frame);return;}lastFrame=ts;if(game.pageVisible&&!reducedMotion){ctx.clearRect(0,0,w,h);const stage=ui.game.dataset.stage,intensity=clamp(Math.abs(game.velocity)*7+(stage==='hyper'?.65:stage==='deep'?.30:stage==='moon'?.16:0),0,1);for(const s of stars){const base=stage==='ground'?.02:stage==='clouds'?.045:stage==='strato'?.075:stage==='hyper'?.22:.13;s.y+=(base+Math.abs(game.velocity)*3.4)*s.z;if(s.y>h){s.y=-18;s.x=Math.random()*w;}ctx.globalAlpha=s.a*(stage==='ground'?.3:1);ctx.fillStyle='#e7f7ff';const trail=1+intensity*18*s.z;ctx.fillRect(s.x,s.y,s.s,Math.max(s.s,trail));}ctx.globalAlpha=1;}raf=requestAnimationFrame(frame);}
    function resume(){if(running)return;running=true;resize();raf=requestAnimationFrame(frame);}
    function pause(){running=false;if(raf!==null){cancelAnimationFrame(raf);raf=null;}}
    const ro=globalThis.ResizeObserver?new ResizeObserver(resize):null;ro?.observe(canvas);if(!ro)addEventListener('resize',resize,{passive:true});resume();
    return{pause,resume,stop(){pause();ro?.disconnect();if(!ro)removeEventListener('resize',resize);ctx.clearRect(0,0,w,h);}};
  }
  ...
  function bind(){
    ...
    document.addEventListener('visibilitychange',()=>{game.pageVisible=!document.hidden;void syncWakeLock();if(game.pageVisible){game.lifecyclePaused=false;ui.game.dataset.lifecycle='active';syncClockActivity();syncVisualActivity();void resyncStatus().catch(()=>startFallback());}else{game.countdownSeq++;game.resultFxSeq++;ui.center.hidden=true;if(game.session?.state==='LIVE')ui.game.dataset.launch='live';syncClockActivity();syncVisualActivity();}});
    addEventListener('online',()=>{ui.network.hidden=false;ui.networkText.textContent='Connection restored. Re-syncing server-authoritative round…';void resyncStatus().catch(()=>{});});
    addEventListener('offline',()=>{ui.network.hidden=false;ui.networkText.textContent='Device offline. The PAPER round remains on the server; controls are unavailable until connection returns.';ui.streamState.textContent='Device offline';game.streamHealthy=false;syncButtons();startFallback();});
    addEventListener('pagehide',()=>{game.pageVisible=false;game.lifecyclePaused=true;game.countdownSeq++;game.resultFxSeq++;ui.center.hidden=true;if(game.session?.state==='LIVE')ui.game.dataset.launch='live';ui.game.dataset.lifecycle='paused';stopClock();stopVisualLoop();game.fx?.pause?.();game.stream?.close?.();game.stream=null;stopFallback();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
    ...
    addEventListener('beforeunload',()=>{clearTimeout(game.historyClearTimer);stopClock();clearVisualTimers();game.completionSeq++;game.pendingResultId=null;stopVisualLoop();game.stream?.close?.();game.stream=null;stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }
