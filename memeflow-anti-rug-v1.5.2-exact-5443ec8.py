...
['requireTokenLogo', 'Require token logo', 'boolean'],"""
    ...
    for key in ("maxSniperPct", "maxDeveloperPct"):
        pat = re.compile(
            rf"\[\s*(['\"]){re.escape(key)}\1\s*,.*?\]\s*,?",
            flags=re.S,
        )
    ...
    raise RuntimeError(
        "system.js could not find a unique maxSniperPct/maxDeveloperPct MF293 field"
    )
...
def patch_html(text: str) -> str:
    ...
    dev_wrapper = re.compile(
        r"<div\b[^>]*\bclass=['\"][^'\"]*\bmfs-field\b[^'\"]*['\"][^>]*>"
        r"(?:(?!</div>).)*?"
        r"<input\b[^>]*\bid=['\"]maxDeveloper['\"][^>]*>"
        r"(?:(?!</div>).)*?</div>",
        flags=re.I | re.S,
    )
    dev_matches = list(dev_wrapper.finditer(text))
    if len(dev_matches) != 1:
        raise RuntimeError(
            f"Settings HTML maxDeveloper mfs-field wrapper count={len(dev_matches)}"
        )

    extra_fields = (
        '<div class="mfs-field"><label for="maxSuspectedRiskyWalletsPct">'
        'Suspected risky wallets % · Max</label>'
        '<input id="maxSuspectedRiskyWalletsPct" min="0" max="100" step="0.1" '
        'type="number" disabled/></div>'
        '<div class="mfs-field"><label for="maxInsidersPct">Insiders % · Max</label>'
        '<input id="maxInsidersPct" min="0" max="100" step="0.1" '
        'type="number" disabled/></div>'
        '<div class="mfs-field"><label for="maxDeveloperRugHistoryPct">'
        'Developer rug history % · Max</label>'
        '<input id="maxDeveloperRugHistoryPct" min="0" max="100" step="0.1" '
        'type="number" disabled/></div>'
        '<div class="mfs-field"><label for="maxDeveloperExitPct">'
        'Developer exit % · Max</label>'
        '<input id="maxDeveloperExitPct" min="0" max="100" step="0.1" '
        'type="number" disabled/></div>'
    )

    dev = dev_matches[0]
    text = text[:dev.end()] + extra_fields + text[dev.end():]

    fresh_wrapper = re.compile(
        r"<div\b[^>]*\bclass=['\"][^'\"]*\bmfs-toggle-row\b[^'\"]*['\"][^>]*>"
        r"(?:(?!</div></div>).)*?"
        r"<input\b[^>]*\bid=['\"]requireFreshHolders['\"][^>]*>"
        r"(?:(?!</div></div>).)*?</div></div>",
        flags=re.I | re.S,
    )
    fresh_matches = list(fresh_wrapper.finditer(text))
    if len(fresh_matches) != 1:
        raise RuntimeError(
            f"Settings HTML requireFreshHolders mfs-toggle-row wrapper count={len(fresh_matches)}"
        )

    toggles = (
        '<div class="mfs-toggle-row"><div class="mfs-toggle-copy">'
        '<b>Require dev migrated</b>'
        '<span>Unknown migration evidence stays WAITING; confirmed false is BLOCKED.</span>'
        '</div><label class="mfs-switch"><input id="requireDevMigrated" '
        'type="checkbox" disabled/><i></i></label></div>'
        '<div class="mfs-toggle-row"><div class="mfs-toggle-copy">'
        '<b>Require token logo</b>'
        '<span>Unresolved metadata stays WAITING; resolved missing logo is BLOCKED.</span>'
        '</div><label class="mfs-switch"><input id="requireTokenLogo" '
        'type="checkbox" disabled/><i></i></label></div>'
    )

    fresh = fresh_matches[0]
    text = text[:fresh.start()] + toggles + text[fresh.start():]

    arrays = []
    for match in re.finditer(
        r"const\s+ids\s*=\s*\[(?P<body>.*?)\]\s*;",
        text,
        flags=re.S,
    ):
        body = match.group("body")
        if (
            re.search(r"['\"]maxDeveloper['\"]", body)
            and re.search(r"['\"]requireFreshHolders['\"]", body)
            and re.search(r"['\"]minTop10['\"]", body)
        ):
            arrays.append(match)

    if len(arrays) != 1:
        raise RuntimeError(
            f"Settings HTML production settings ids array count={len(arrays)}"
        )

    match = arrays[0]
    body = match.group("body")
    token_pat = re.compile(
        r"(?P<q>['\"])maxDeveloper(?P=q)(?P<comma>\s*,)?"
    )
    token_matches = list(token_pat.finditer(body))
    if len(token_matches) != 1:
        raise RuntimeError(
            f"Settings HTML maxDeveloper ids token count={len(token_matches)}"
        )
    ...
    replacement = ",".join(f"{q}{value}{q}" for value in values)
    if token.group("comma") is not None:
        replacement += ","

    body = body[:token.start()] + replacement + body[token.end():]
    text = text[:match.start("body")] + body + text[match.end("body"):]

    for key in NEW_KEYS:
        count = len(re.findall(rf"\bid=['\"]{re.escape(key)}['\"]", text))
        if count != 1:
            raise RuntimeError(
                f"Settings HTML post-check failed for {key}: id count={count}"
            )

    if "function serverToView" not in text or "function viewToServer" not in text:
        raise RuntimeError("Settings HTML server/view mapping functions are missing")
...
def self_test() -> None:
    ...
    h = """<form class="mfs-body" id="aiSettingsForm">
<div class="mfs-filter-pane" data-filter-pane="audit"><div class="mfs-fields">
<div class="mfs-field"><label for="minTop10">Top-10 holders % · Min</label><input id="minTop10" type="number" disabled/></div>
<div class="mfs-field"><label for="maxTop10">Top-10 holders % · Max</label><input id="maxTop10" type="number" disabled/></div>
<div class="mfs-field"><label for="minDeveloper">Developer share % · Min</label><input id="minDeveloper" type="number" disabled/></div>
<div class="mfs-field"><label for="maxDeveloper">Developer share % · Max</label><input id="maxDeveloper" type="number" disabled/></div>
<div class="mfs-field"><label for="minSniperPct">Sniper ownership % · Min</label><input id="minSniperPct" type="number" disabled/></div>
<div class="mfs-field"><label for="maxSniperPct">Sniper ownership % · Max</label><input id="maxSniperPct" type="number" disabled/></div>
</div><div class="mfs-toggle-row"><div class="mfs-toggle-copy"><b>Require fresh holder snapshot</b><span>Unknown holder data keeps the AI decision in WAITING.</span></div><label class="mfs-switch"><input id="requireFreshHolders" type="checkbox" disabled/><i></i></label></div></div>
</form>
<script id="ai-trading-settings-js">
const ids=['tradingCapital','minTop10','maxTop10','minDeveloper','maxDeveloper','minSniperPct','maxSniperPct','requireFreshHolders'];
const S2H={minTop10Pct:'minTop10',maxTop10Pct:'maxTop10',minDeveloperPct:'minDeveloper',maxDeveloperPct:'maxDeveloper'};
function serverToView(s){return s}
function viewToServer(o){return o}
fetch('/api/settings');
</script>"""
    ph = patch_html(h)
...
def main() -> int:
    ...
    if "--self-test" in sys.argv:
        log("V1.5.2 installer transformer self-test: PASS")
        return 0
    log("V1.5.2 installer transformer self-test: PASS")
    root = root_dir()
