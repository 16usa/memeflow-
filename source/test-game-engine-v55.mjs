...
const {GameEngine}=await import(pathToFileURL(enginePath).href+'?v55test='+Date.now());
...
// V5.4 flight recap metrics must be server-derived and preserved through settlement.
{
  const {store,engine}=make();
  const started=engine.start('u',{bet:100,autoCashout:0,stopLoss:0,requestId:'recap'}).session.startedAt;
  store.state.tokens[mint]={...store.state.tokens[mint],priceSol:1.30,lastPriceAt:started+1000};engine.onTokenUpdate(mint,store.state.tokens[mint]);
  store.state.tokens[mint]={...store.state.tokens[mint],priceSol:.90,lastPriceAt:started+2000};engine.onTokenUpdate(mint,store.state.tokens[mint]);
  store.state.tokens[mint]={...store.state.tokens[mint],priceSol:1.10,lastPriceAt:started+3000};engine.onTokenUpdate(mint,store.state.tokens[mint]);
  const live=engine.status('u',{sync:false}).session;
  assert.equal(live.peak,1.3);assert.ok(Math.abs(live.maxAdverseExcursionPct-10)<1e-9);assert.equal(live.timeToPeakMs,1000);assert.equal(live.priceUpdateCount,4);
  const done=engine.cashout('u');assert.equal(done.ok,true);assert.equal(done.session.state,'COMPLETE');assert.equal(done.history[0].timeToPeakMs,1000);assert.ok(Math.abs(done.history[0].maxAdverseExcursionPct-10)<1e-9);assert.equal(done.history[0].priceUpdateCount,4);
  engine.destroy();
}

console.log('PEPE GAME V5.5 LIVE ENGINE COMPAT TESTS: PASS');
