(()=>{
  ...
  const game={
    ...
    raf:null,lifecyclePaused:false,flightState:'idle',localFeedVisualStale:false,lastStage:'ground',stageTimer:null,historyClearTimer:null,lastVisualAt:0
  };
  ...
  function renderVisual(multiplier){
    ...
    const [stage,label]=stageFor(m);if(stage!==game.lastStage)showStageTransition(stage,label,m);ui.game.dataset.stage=stage;ui.stage.textContent=label;const bottom=7+levelNorm(m)*62;ui.rocket.style.bottom=`${bottom.toFixed(2)}%`;
  ...
  async function clearHistory(){if(!game.historyClearArmed){game.historyClearArmed=true;ui.clearHistory.textContent='Confirm';clearTimeout(game.historyClearTimer);game.historyClearTimer=setTimeout(()=>{game.historyClearArmed=false;ui.clearHistory.textContent='Clear';game.historyClearTimer=null;},3000);return;}game.historyClearArmed=false;clearTimeout(game.historyClearTimer);game.historyClearTimer=null;ui.clearHistory.textContent='Clear';try{apply(await api('/api/game/history/clear',{method:'POST',body:'{}'}),{allowResult:false});}catch(e){ui.stateMessage.textContent=e.message;}}
  ...
  function bind(){
    ...
    document.addEventListener('visibilitychange',()=>{game.pageVisible=!document.hidden;void syncWakeLock();if(game.pageVisible){game.lifecyclePaused=false;ui.game.dataset.lifecycle='active';syncVisualActivity();void api('/api/game/status').then(x=>{apply(x);connectStream();}).catch(()=>startFallback());}});
    ...
    addEventListener('pagehide',()=>{game.pageVisible=false;game.lifecyclePaused=true;ui.game.dataset.lifecycle='paused';stopVisualLoop();game.fx?.pause?.();game.stream?.close?.();game.stream=null;stopFallback();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
    addEventListener('pageshow',()=>{game.pageVisible=!document.hidden;game.lifecyclePaused=false;ui.game.dataset.lifecycle='active';syncVisualActivity();void api('/api/game/status').then(x=>{apply(x);connectStream();}).catch(()=>startFallback());void syncWakeLock();});
    ...
    addEventListener('beforeunload',()=>{clearTimeout(game.historyClearTimer);clearVisualTimers();game.completionSeq++;game.pendingResultId=null;stopVisualLoop();game.stream?.close?.();game.stream=null;stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }

  function startClock(){if(game.clock)return;game.clock=setInterval(()=>{updateSearchRadar();const s=game.session;if(s?.state==='LIVE'&&s.startedAt){const sec=Math.max(0,Math.floor((Date.now()-s.startedAt)/1000));ui.roundTime.textContent=`${String(Math.floor(sec/60)).padStart(2,'0')}:${String(sec%60).padStart(2,'0')}`;if(s.latestPriceAt){const age=Math.max(0,Date.now()-s.latestPriceAt);ui.feedAge.textContent=age<1000?'<1s':`${Math.round(age/1000)}s`;if(s.state==='LIVE'){const staleAt=num(game.status?.limits?.cashoutPriceMaxAgeMs)??20000,abortAt=num(game.status?.limits?.marketLossAbortMs)??90000;if(age>staleAt){game.localFeedVisualStale=true;ui.game.dataset.clientfeed='aged';if(game.session?.feedFresh===false)ui.feedQuality.textContent=age>abortAt*.75?'SAFETY':'STALE';}else{game.localFeedVisualStale=false;ui.game.dataset.clientfeed='fresh';}}}}else ui.roundTime.textContent='00:00';},1000);}
