...
V60TEST="memeflow-app/tests/ai-decisions-inventory-hotpath-v60.mjs"
V61TEST="memeflow-app/tests/live-states-prefix-hotpath-v61.mjs"
...
FILES=(
  "$EVAL" "$RANK" "$LEGACY" "$SIZE" "$PAPER" "$OPENAI" "$GAME" "$SETTINGS"
  "$APP" "$TOKENS" "$TRADING" "$SYSTEM" "$SETTINGSPAGE" "$INDEX"
  "$TOKENSHTML" "$TRADINGHTML" "$SYSTEMHTML" "$SETTINGSHTML"
  "$FEEDTEST" "$OPPTEST" "$ADAPTTEST" "$TERMINALTEST" "$V60TEST" "$V61TEST" "$OLDTEST"
)

echo "=== MEMEFLOW UNIFIED ANALYSIS ENGINE V21.6 ==="

[[ "$(git branch --show-current)" == "$BRANCH" ]] || {
  echo "V21.6 REFUSED: expected branch $BRANCH"
  echo "actual: $(git branch --show-current)"
  exit 1
}

[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V21.6 REFUSED: audited HEAD changed"
...
for f in "${FILES[@]}"; do
  [[ -f "$f" ]] || { echo "V21.6 REFUSED: missing $f"; exit 1; }
  git diff --quiet -- "$f" || { echo "V21.6 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V21.6 REFUSED: staged changes in $f"; exit 1; }
done
...
"memeflow-app/tests/ai-decisions-inventory-hotpath-v60.mjs":[
 "/\\/api\\/ai\\/decisions\\?scope=all&limit=100/"
],
"memeflow-app/tests/live-states-prefix-hotpath-v61.mjs":[
 "/\\/api\\/ai\\/decisions\\?scope=all&limit=100/",
...
for file,markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(f"V21.6 REFUSED: audited marker missing in {file}: {marker}")
print("AUDITED_V21_INPUT_OK")
PY

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP=".memeflow-backups/unified-analysis-engine-v21-6-$STAMP"
mkdir -p "$BACKUP"
...
PAPER=Path("memeflow-app/src/paper-engine.mjs")
...
def once(text,old,new,label):
    ...
    if count!=1:
        raise SystemExit(f"V21.6 REFUSED: {label}: expected 1 match, got {count}")
...
if n!=1: raise SystemExit(f"V21.6 REFUSED: smart rank block count={n}")
...
if mstart<0 or mend<=mstart: raise SystemExit("V21.6 REFUSED: mergedCandidates boundaries")
old_merge=tr[mstart:mend]
for marker in ("MEMEFLOW_TERMINAL_WATCH_DUPLICATE_MERGE_V37","liveWatchCandidates","__pipelineWatch"):
    if marker not in old_merge: raise SystemExit("V21.6 REFUSED: old Terminal merge shape: "+marker)
...
if lstart<0 or lend<=lstart: raise SystemExit("V21.6 REFUSED: loadCandidates boundaries")
...
for marker in (
 ...
 "MEMEFLOW_TERMINAL_WATCH_RESILIENT_V30"
    ...
    if marker not in old_load: raise SystemExit("V21.6 REFUSED: old Terminal load shape: "+marker)
...
""",
 """  openPosition(userId, token, decision, rawSettings = {}, idempotencyKey = null) {
    ...
    const price = num(token.priceSol);
""",
 "PaperEngine execution gate recheck"
)
...
""",
 """      initialSizeSol: size,
      ...
      positionSizing,
""",
 "PaperEngine position sizing snapshot"
)
...
# ================= stale regression contracts v60/v61 =================
v60=Path("memeflow-app/tests/ai-decisions-inventory-hotpath-v60.mjs")
t=v60.read_text(encoding="utf-8")
old="""  assert.match(
    source,
    /\\/api\\/ai\\/decisions\\?scope=all&limit=100/
  );
...
"""
new="""  // MEMEFLOW_TERMINAL_ONE_MECHANISM_REGRESSION_V21_6
...
"""
if old not in t:
    raise SystemExit("V21.6 REFUSED: v60 stale Terminal route assertion anchor missing")
v60.write_text(t.replace(old,new,1),encoding="utf-8")

v61=Path("memeflow-app/tests/live-states-prefix-hotpath-v61.mjs")
t=v61.read_text(encoding="utf-8")
old="""  assert.match(
    trading,
    /\\/api\\/ai\\/decisions\\?scope=all&limit=100/
  );
...
"""
if old not in t:
    raise SystemExit("V21.6 REFUSED: v61 stale dual-feed assertion anchor missing")
v61.write_text(t.replace(old,new,1),encoding="utf-8")
...
"""
if old not in t: raise SystemExit("V21.6 REFUSED: opportunity-engine assertion anchor missing")
...
assert(qualified.amountSol > 0);
"""
if old not in t: raise SystemExit("V21.6 REFUSED: adaptive assertion anchor missing")
p.write_text(t.replace(old,new,1),encoding="utf-8")
PY
...
import re
updates={
 "memeflow-app/system-tokens.html":(r'/system-tokens\.js\?v=[^"]+','/system-tokens.js?v=unified-analysis-v21-60260903'),
 "memeflow-app/trading.html":(r'/trading\.js\?v=[^"]+','/trading.js?v=unified-analysis-v21-60260903'),
 "memeflow-app/system.html":(r'/system\.js\?v=[^"]+','/system.js?v=unified-analysis-v21-60260903'),
 "memeflow-app/settings.html":(r'/settings-page\.js\?v=[^"]+','/settings-page.js?v=unified-analysis-v21-60260903')
}
for file,(pattern,replacement) in updates.items():
    p=Path(file)
    t=p.read_text(encoding="utf-8")
    t,n=re.subn(pattern,replacement,t,count=1)
    if n!=1: raise SystemExit(f"V21.6 REFUSED: asset update {file}: {n}")
    p.write_text(t,encoding="utf-8")
print("ASSET_VERSIONS_OK")
PY

echo
echo "=== V21.6 REGRESSION CONTRACT PRECHECK ==="
...
if ! grep -q "MEMEFLOW_CANONICAL_SCORE_COMPAT_ALIASES_V21_4" "$EVAL"; then echo "ERROR: canonical compatibility aliases missing"; exit 1; fi
if ! grep -q "MEMEFLOW_PAPER_CANONICAL_ADAPTIVE_SIZING_V21" "$PAPER"; then echo "ERROR: PaperEngine adaptive canonical sizing missing"; exit 1; fi
if ! grep -q "MEMEFLOW_PAPER_EXECUTION_GATE_RECHECK_V21_3" "$PAPER"; then echo "ERROR: PaperEngine execution safety recheck missing"; exit 1; fi
if ! grep -q "MEMEFLOW_STRATEGY_AWARE_EXECUTION_V21_5" "$PAPER"; then echo "ERROR: strategy-aware PaperEngine execution missing"; exit 1; fi
echo "REGRESSION_CONTRACTS_OK"

echo
echo "=== V21.6 SYNTAX ==="
for f in \
 "$EVAL" "$RANK" "$LEGACY" "$SIZE" "$PAPER" "$OPENAI" "$GAME" "$APP" \
 "$TOKENS" "$TRADING" "$SYSTEM" "$SETTINGSPAGE" \
 "$FEEDTEST" "$OPPTEST" "$ADAPTTEST" "$TERMINALTEST" "$V60TEST" "$V61TEST" "$OLDTEST" "$NEWTEST"
do
  node --check "$f"
done
echo "SYNTAX_OK"
...
echo
echo "=== V21.6 FULL PROJECT TEST SUITE ==="
(
 cd memeflow-app
 npm test
)
...
echo
echo "=== V21.6 STATIC WHOLE-PROJECT AUDIT ==="
python3 - <<'PY'
from pathlib import Path
...
if "MEMEFLOW_CANONICAL_SCORE_COMPAT_ALIASES_V21_4" not in Path("memeflow-app/src/evaluate.mjs").read_text(): errors.append("canonical compatibility aliases missing")
...
v60=Path("memeflow-app/tests/ai-decisions-inventory-hotpath-v60.mjs").read_text()
v61=Path("memeflow-app/tests/live-states-prefix-hotpath-v61.mjs").read_text()
if "MEMEFLOW_TERMINAL_ONE_MECHANISM_REGRESSION_V21_6" not in v60: errors.append("v60 regression still protects removed Terminal feed")
if "MEMEFLOW_TERMINAL_ONE_MECHANISM_REGRESSION_V21_6" not in v61: errors.append("v61 regression still protects removed Terminal feed")
...
if "MEMEFLOW_PAPER_CANONICAL_ADAPTIVE_SIZING_V21" not in paper or "positionSizing," not in paper: errors.append("PaperEngine canonical adaptive sizing missing")
if "MEMEFLOW_PAPER_EXECUTION_GATE_RECHECK_V21_3" not in paper or "evaluateSettingsGate" not in paper: errors.append("PaperEngine execution safety recheck missing")
if "MEMEFLOW_STRATEGY_AWARE_EXECUTION_V21_5" not in paper or "mode:'copy-fixed'" not in paper: errors.append("PaperEngine copy-trading strategy path missing")
asset_checks={
 "memeflow-app/system-tokens.html":"system-tokens.js?v=unified-analysis-v21-60260903",
 "memeflow-app/trading.html":"trading.js?v=unified-analysis-v21-60260903",
 "memeflow-app/system.html":"system.js?v=unified-analysis-v21-60260903",
 "memeflow-app/settings.html":"settings-page.js?v=unified-analysis-v21-60260903"
}
...
ALLOWED_RE='^memeflow-app/(app-server\.mjs|system-tokens\.js|trading\.js|system\.js|settings-page\.js|index\.html|system-tokens\.html|trading\.html|system\.html|settings\.html|src/evaluate\.mjs|src/feed-ranking\.mjs|src/evaluator\.mjs|src/adaptive-position-sizing\.mjs|src/paper-engine\.mjs|src/openai-intelligence\.mjs|src/game-engine\.mjs|src/settings\.mjs|tests/feed-ranking\.mjs|tests/opportunity-engine\.mjs|tests/adaptive-position-sizing\.mjs|tests/terminal-watch-merge-v37\.mjs|tests/ai-decisions-inventory-hotpath-v60\.mjs|tests/live-states-prefix-hotpath-v61\.mjs|tests/canonical-live-score-pipeline-v20_8_8\.mjs|tests/unified-analysis-engine-v21\.mjs)$'
...
git commit -m "refactor: unify MEMEFLOW analysis into one score engine v21.6"
