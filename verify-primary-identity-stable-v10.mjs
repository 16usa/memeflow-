...
if(!fs.existsSync(target)){console.error('NO index');process.exit(1)}
...
const checks={
  ...
  noMutationObserver:!(/MF_PRIMARY_IDENTITY_STABLE_V10[\s\S]*MutationObserver/.test(s)),
  noInterval:!(/MF_PRIMARY_IDENTITY_STABLE_V10[\s\S]*setInterval/.test(s)),
