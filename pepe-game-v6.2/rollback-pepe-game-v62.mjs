...
const latest=path.join(app,'.memeflow-patches','pepe-game-v62','latest.json');
const sha=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');
const fail=m=>{console.error('PEPE GAME V6.2 ROLLBACK: FAIL · '+m);process.exit(1)};
if(!fs.existsSync(latest))fail('no V6.2 backup metadata found');
...
console.log('Pepe Rocket V6.2 rollback: PASS');
console.log('Restored V6.1 visual files only. Trading/server files untouched.');
