(()=>{
  ...
  function clearVisualTimers(){
    clearTimeout(game.bankTimer);game.bankTimer=null;clearTimeout(game.stageTimer);game.stageTimer=null;clearTimeout(showResult.secureTimer);showResult.secureTimer=null;clearTimeout(cashOut.pulseTimer);cashOut.pulseTimer=null;
  }
  function resetMotionState({stage=true}={}){
    clearVisualTimers();game.velocity=0;game.acceleration=0;game.visualTilt=-3;game.dangerLevel='none';game.lastDangerHapticAt=0;game.lastReversalHapticAt=0;game.localFeedVisualStale=false;game.cameraScale=1;game.cameraX=0;game.cameraY=0;game.engineScale=1;ui.velocity.textContent='0.000×/s';ui.game.dataset.bank='neutral';ui.game.dataset.danger='none';ui.game.dataset.cashpulse='idle';ui.game.dataset.clientfeed='fresh';if(stage)game.lastStage='ground';setFlightState('idle');
  }
  ...
  function setMode(mode,message){
    ...
    const labels={idle:'READY',searching:'SCANNING',live:'LIVE',settling:'CASHING OUT',complete:'COMPLETE'};ui.stateLabel.textContent=labels[mode]||mode.toUpperCase();if(message)ui.stateMessage.textContent=message;setFlightState(resolveFlightState());syncButtons();syncVisualActivity();void syncWakeLock();
  }
  ...
  function connectStream(){
    ...
    const receive=(event)=>{try{const data=JSON.parse(event.data||'{}');game.streamHealthy=true;ui.network.hidden=true;ui.streamState.textContent='Server-authoritative game stream live';stopFallback();apply(data);}catch{game.streamHealthy=false;startFallback();}};
  ...
  function createFx(){
    const canvas=ui.fx,ctx=canvas.getContext('2d');if(!ctx)return null;let w=1,h=1,dpr=1,running=false,raf=null,lastFrame=0;const stars=[];const saveData=navigator.connection?.saveData===true;const count=saveData?28:innerWidth<700?44:82;
    function resize(){const r=canvas.getBoundingClientRect();w=Math.max(1,r.width);h=Math.max(1,r.height);dpr=Math.min(1.5,devicePixelRatio||1);canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);while(stars.length<count)stars.push({x:Math.random()*w,y:Math.random()*h,s:.4+Math.random()*1.2,a:.18+Math.random()*.55,z:.4+Math.random()});for(const s of stars){s.x=clamp(s.x,0,w);s.y=clamp(s.y,-18,h);}}
  ...
  function bind(){
    ...
    document.addEventListener('visibilitychange',()=>{game.pageVisible=!document.hidden;void syncWakeLock();if(game.pageVisible){game.lifecyclePaused=false;ui.game.dataset.lifecycle='active';startClock();syncVisualActivity();void resyncStatus().catch(()=>startFallback());}else{stopClock();syncVisualActivity();}});
    ...
    addEventListener('pagehide',()=>{game.pageVisible=false;game.lifecyclePaused=true;game.countdownSeq++;ui.center.hidden=true;if(game.session?.state==='LIVE')ui.game.dataset.launch='live';ui.game.dataset.lifecycle='paused';stopClock();stopVisualLoop();game.fx?.pause?.();game.stream?.close?.();game.stream=null;stopFallback();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
    addEventListener('pageshow',()=>{game.pageVisible=!document.hidden;game.lifecyclePaused=false;ui.game.dataset.lifecycle='active';startClock();syncVisualActivity();void resyncStatus().catch(()=>startFallback());void syncWakeLock();});
    motionQuery?.addEventListener?.('change',event=>{reducedMotion=event.matches===true;if(reducedMotion){game.displayMultiplier=game.targetMultiplier;renderVisual(game.displayMultiplier);}syncVisualActivity();});
