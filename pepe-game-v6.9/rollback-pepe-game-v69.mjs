...
const app=path.resolve(process.cwd(),'memeflow-app'),latest=path.join(app,'.memeflow-patches','pepe-game-v69','latest.json');
const sha=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');const fail=m=>{console.error('PEPE GAME V6.9 ROLLBACK: FAIL · '+m);process.exit(1)};
if(!fs.existsSync(latest))fail('no V6.9 backup metadata found');const meta=JSON.parse(fs.readFileSync(latest,'utf8')),files=['game.html','game.css','game.js'];
...
console.log('Pepe Rocket V6.9 rollback: PASS');console.log('Restored V6.8 visual files only. Trading/server files untouched.');
