...
const root=path.join(app,'.memeflow-patches','pepe-game-v56');if(!fs.existsSync(root))throw new Error('No V5.6 backup directory found.');
...
if(!backup)throw new Error('No V5.6 backup manifest found.');
const manifest=JSON.parse(fs.readFileSync(path.join(backup,'manifest.json'),'utf8'));
for(const name of manifest.files||[]){const src=path.join(backup,name),dst=path.join(app,name);if(!fs.existsSync(src))throw new Error(`Backup file missing: ${src}`);const tmp=dst+'.v56rollback';try{fs.unlinkSync(tmp)}catch{}fs.copyFileSync(src,tmp);fs.renameSync(tmp,dst);}
console.log('Pepe Rocket V5.6 rollback complete from:',backup);
