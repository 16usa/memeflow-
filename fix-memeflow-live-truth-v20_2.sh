...
TEST="memeflow-app/tests/live-truth-no-dynamic-cache-v20_2.mjs"
...
echo "=== MEMEFLOW LIVE TRUTH V20.2 PRECHECK ==="
python3 - <<'PY'
...
console.log('LIVE_TRUTH_NO_DYNAMIC_CACHE_V20_2_OK');
TESTJS
...
(
  cd memeflow-app
  node tests/live-truth-no-dynamic-cache-v20_2.mjs
  [[ -f tests/live-card-clock-v19.mjs ]] && node tests/live-card-clock-v19.mjs
...
BAD="$(git diff --cached --name-only | grep -Ev '^memeflow-app/(app-server\.mjs|system-tokens\.js|system-tokens\.html|tests/live-truth-no-dynamic-cache-v20_2\.mjs)$' || true)"
if [[ -n "$BAD" ]]; then
