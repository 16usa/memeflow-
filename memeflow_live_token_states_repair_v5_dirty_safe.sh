...
echo "[MEMEFLOW] Live Token States repair v5-dirty-safe"
...
# ---------------------------------------------------------------------------
# IMPORTANT:
# This installer intentionally ignores ALL unrelated dirty files.
#
# Your Replit workspace may have:
#   M  runtime/backup files
#   D  runtime state files
#   ?? old installers/backups
#
# Those are NOT this patch's files and must not block installation.
# We only protect the exact files this patch edits.
# ---------------------------------------------------------------------------
if ! git diff --quiet -- "${TARGETS[@]}" || \
   ! git diff --cached --quiet -- "${TARGETS[@]}"; then
  echo "ERROR: one of THIS PATCH'S target files already has local edits." >&2
  echo "Unrelated M/D/?? files are ignored; only these paths are protected:" >&2
  git status --short -- "${TARGETS[@]}"
  exit 1
fi

echo "[ok] unrelated tracked/untracked workspace changes will be ignored"

# Fetch is safe with a dirty working tree; unlike git pull, it does not try to
# rewrite local files.
...
if [[ "$LOCAL_HEAD" != "$REMOTE_HEAD" ]]; then
  if git merge-base --is-ancestor "$LOCAL_HEAD" "$REMOTE_HEAD"; then
    echo "[git] local branch is behind origin/main; trying safe fast-forward"
    git merge --ff-only "$REMOTE_HEAD"
  elif git merge-base --is-ancestor "$REMOTE_HEAD" "$LOCAL_HEAD"; then
    echo "ERROR: local branch contains commits not on origin/main." >&2
    echo "Refusing to push unrelated local commits." >&2
    exit 1
  else
    echo "ERROR: local branch and origin/main have diverged." >&2
    exit 1
  fi
fi
...
rollback() {
  ...
  if [[ $code -ne 0 ]]; then
    ...
    echo "[rollback] restoring ONLY this patch's target files"
    git restore --staged --worktree -- "${TARGETS[@]}" 2>/dev/null || true
    echo "[rollback] unrelated M/D/?? workspace files were left untouched"
...
def replace_section(text, start, end, replacement, installed_marker, label):
...
new_route = r""" if(url.pathname==='/api/system/live-token-states'&&req.method==='GET'){
  ...
  //
  // Therefore an incomplete token is visible as a state; it is never silently
  // erased just because an Entry Filter is still waiting for data.
  ...
  // Cache identity includes response shape. A limit=200 snapshot must never be
  // reused for an unlimited request or another limit.
  ...
  for(const _token of _rawTokens){
    ...
    // Never monopolize Node while Pump WebSocket events are arriving.
    _processed++;
    ...
    if(!_eligible&&!_isOpen){
      // IMPORTANT: do not run post-admission Logic early.
      // Entry Filters still block trading; Live Token States only exposes the
      // admission state instead of deleting the row.
  ...
  const _payload={
    ...
    limit:_limit,
    source:'system-live-token-states-transparent-v5',
    ...
    // Compatibility field. Nothing is silently hidden on this observability UI.
...
# ===========================================================================
# 2. FRONTEND — EVENT-DRIVEN, BURST-COALESCED
# ===========================================================================
ui = load("system-tokens.js")

if "MEMEFLOW_REALTIME_COALESCE_V5_DIRTY_SAFE" in ui:
    ...
    if i >= 0 and j > i:
        ...
        if "}, 80);" in block:
            block = block.replace(
                schedule_start,
                "/* MEMEFLOW_REALTIME_COALESCE_V5_DIRTY_SAFE */\n"
            ...
            print("[skip] frontend realtime coalescing: timer is already different")
    else:
        print("[skip] frontend realtime coalescing: anchors not found")


# ===========================================================================
# 3. FRONTEND — SCANNER/ADMISSION DIAGNOSTICS
# ===========================================================================
diag_marker = "MEMEFLOW_LIVE_DIAGNOSTICS_V5_DIRTY_SAFE"
if diag_marker in ui:
    ...
    if i >= 0 and j >= 0:
        j += len(diag_end)
        diag = r"""    // MEMEFLOW_LIVE_DIAGNOSTICS_V5_DIRTY_SAFE
    ...
    $('lastUpdate').textContent = parts.join(' · ');"""
        ...
        print("[skip] frontend live diagnostics: old telemetry block not present")

save("system-tokens.js", ui)


# ===========================================================================
# 4. SETTINGS TEXT — ACCURATE ARCHITECTURE
# ===========================================================================
...
if "MEMEFLOW_LIVE_TOKEN_VISIBILITY_V5_DIRTY_SAFE" in strict:
    ...
    # Start replacement at the nearby explanatory comment if present.
    ...
    block = r"""// MEMEFLOW_LIVE_TOKEN_VISIBILITY_V5_DIRTY_SAFE
...
assert.match(liveStatesRoute,/MEMEFLOW_LIVE_TOKEN_VISIBILITY_V5_DIRTY_SAFE/);
...
# ===========================================================================
# 7. FRESH SESSION / PERMANENT REGISTRY REGRESSION
# This old test explicitly required the broken silent-hiding behavior.
# ===========================================================================
...
if "MEMEFLOW_LIVE_TOKEN_VISIBILITY_V5_DIRTY_SAFE" in fresh:
...
if new_assertion in settings_test:
    ...
    for old in candidates:
        if old in settings_test:
            ...
            break
    if not changed:
        print("[skip] settings architecture regression: old assertion not found")
save("tests/settings-architecture-v2.mjs", settings_test)


# ===========================================================================
# 9. STATIC INSTALL-TIME INVARIANTS
# ===========================================================================
app = load("app-server.mjs")
...
required = [
    ...
    "displayOnly:!_eligible&&!_isOpen",
    "state:_blocked?'BLOCKED':'WAITING'",
    "system-live-token-states-transparent-v5",
]
for needle in required:
    if needle not in live_route:
        raise SystemExit(f"[verify] missing Live Token States invariant: {needle}")

if re.search(r"_hiddenBySettings\+\+;\s*continue", live_route):
    raise SystemExit("[verify] old silent-hiding path still exists")

# Trading feed must STILL be admitted-only.
...
if "readyState !== EventSource.OPEN" not in ui:
    raise SystemExit("[verify] disconnected-only 3s fallback disappeared")

print("[verify] visibility + trading + realtime invariants OK")
PY
...
# Stage ONLY files owned by this patch. Existing runtime M/D and old ?? scripts
# stay unstaged and untouched.
git add -- "${TARGETS[@]}"

if git diff --cached --quiet; then
  echo
  echo "DONE: all v5 changes were already present; nothing to commit."
  trap - ERR
  exit 0
fi
...
echo
echo "DONE"
echo "- unrelated workspace M / D / ?? files were ignored"
echo "- PENDING tokens are visible as WAITING"
echo "- REJECTED tokens are visible as BLOCKED"
echo "- BUY READY / execution remain strictly Entry-admitted"
echo "- open positions remain visible"
echo "- realtime stays SSE/event-driven with 3s disconnected fallback"
echo "- full test suite passed before commit/push"
