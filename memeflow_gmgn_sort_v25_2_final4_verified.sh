...
PATCH_ID="MEMEFLOW_GMGN_SORT_STYLE_V25_2_FINAL4"
OLD_VERSION="gmgn-sort-v25-1-20260827"
NEW_VERSION="gmgn-sort-v25-2-final4-20260827"
DO_PUSH=0
...
# Strict V25.1 -> V25.2 FINAL4 prerequisites.
...
# Never patch over local work in files owned by this installer.
if ! git diff --quiet -- system-tokens.js system-tokens.css system-tokens.html package.json tests 2>/dev/null; then
  echo "ERROR: patch-owned files contain uncommitted changes." >&2
  echo "Commit or stash them first. Nothing was changed." >&2
  exit 1
fi

# A commit later in this installer must never absorb unrelated staged work.
...
BACKUP_DIR=".patch-backups/gmgn-sort-v25-2-final4-$STAMP"
mkdir -p "$BACKUP_DIR"
...
canonical_test = tests_dir / "token-sort-v25-2-final4.mjs"
...
assert.equal(
  (
    html.match(
      /gmgn-sort-v25-2-final4-20260827/g
    )||[]
  ).length,
  2,
...
assert.match(
  html,
  /system-tokens\.css\?v=gmgn-sort-v25-2-final4-20260827/
);

console.log('token sort v25.2 final4 ok');
''',
    encoding="utf-8"
)
...
print("V25.2 FINAL4 visual patch installed.")
...
node --check tests/token-sort-v25-2-final4.mjs

echo "==> [3/7] Canonical sort regression"
node tests/token-sort-v25-2-final4.mjs
...
if html.count(
    "gmgn-sort-v25-2-final4-20260827"
) != 2:
    raise SystemExit(
...
echo
echo "V25.2 FINAL4 validated successfully."
...
if [[ "$DO_PUSH" -eq 1 ]]; then
  echo
  echo "Pushing validated V25.2 FINAL4 commit..."
  git push
else
  echo
...
echo "  - unrelated runtime data changes are left untouched and unstaged"
echo "  - pre-existing staged changes are rejected before patching"
