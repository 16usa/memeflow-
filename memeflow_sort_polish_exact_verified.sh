...
PATCH_ID="MEMEFLOW_GMGN_SORT_STYLE_V25_5_POLISH_EXACT"
OLD_MARKER="MEMEFLOW_GMGN_SORT_STYLE_V25_4_EXACT"
OLD_VERSION="gmgn-sort-v25-4-exact-20260827"
NEW_VERSION="gmgn-sort-v25-5-polish-exact-20260827"
DO_PUSH=0
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: bash memeflow_sort_polish_exact.sh [--push|--no-push]" >&2
      exit 2
      ;;
  esac
done
...
BACKUP_DIR=".patch-backups/gmgn-sort-v25-5-polish-exact-$STAMP"
mkdir -p "$BACKUP_DIR"
...
if marker_at < 0:
    raise SystemExit(
        "PRECHECK FAILED: V25.4 EXACT CSS marker boundary was not found."
    )
...
.mf-sort-trigger-v25 {
  ...
  border:1px solid rgba(77,230,161,.20);
  border-radius:10px;
...
.mf-sort-trigger-v25.is-active {
  ...
  background:rgba(77,230,161,.020);
  color:#c3d0d6;
}
...
.mf-sort-sheet-v25 {
  ...
  border:1px solid rgba(161,188,202,.20);
  border-radius:20px;
...
.mf-sort-handle-v25 {
  ...
  margin:1px auto 4px;
  border-radius:999px;
  background:rgba(163,182,193,.36);
}
...
.mf-sort-direction-v25 {
  ...
  border:1px solid rgba(159,187,201,.14);
  border-radius:10px;

  background:rgba(7,15,20,.27);
}
...
.mf-sort-direction-v25 button.is-active {
  border-color:#5eddf1;
  background:rgba(94,221,241,.022);
  color:#edf5f7;
  box-shadow:none;
}
...
.mf-sort-list-shell-v252 {
  ...
  border:1px solid rgba(159,187,201,.16);
  border-radius:12px;
...
.mf-sort-row-v25 + .mf-sort-row-v25::before {
  ...
  left:0;
  height:1px;
  background:rgba(159,187,201,.11);
}
...
# Update exact old cache assertions in every JavaScript regression file.
...
canonical = tests_dir / "token-sort-v25-5-polish-exact.mjs"
...
# Keep all prior token-sort test entry points compatible.
...
if css.count(PATCH_ID) != 1:
    raise SystemExit(
        "POSTCHECK FAILED: V25.5 polish exact CSS layer must exist exactly once."
    )

css_path.write_text(css,encoding="utf-8")
html_path.write_text(html,encoding="utf-8")

print("V25.5 polish exact visual refinement installed.")
...
node --check tests/token-sort-v25-5-polish-exact.mjs

echo "==> [3/7] Exact polish regression"
node tests/token-sort-v25-5-polish-exact.mjs
...
marker = "MEMEFLOW_GMGN_SORT_STYLE_V25_5_POLISH_EXACT"

if css.count(marker) != 1:
    raise SystemExit(
        "ERROR: V25.5 polish exact sorting CSS layer is not unique."
    )

if "MEMEFLOW_GMGN_SORT_STYLE_V25_4_EXACT" in css:
    raise SystemExit(
        "ERROR: previous V25.4 EXACT sorting CSS still exists."
    )
...
if marker_at < 0:
    raise SystemExit(
        "ERROR: V25.5 polish exact CSS marker was not found."
    )
...
if html.count(
    "gmgn-sort-v25-5-polish-exact-20260827"
) != 2:
    raise SystemExit(
...
echo
echo "V25.5 POLISH EXACT validated successfully."
...
if git diff --cached --quiet; then
  ...
  git -c commit.gpgsign=false commit \
    -m "fix(token-flow): polish exact sorting mockup alignment"
fi

if [[ "$DO_PUSH" -eq 1 ]]; then
  echo
  echo "Pushing validated V25.5 POLISH EXACT commit..."
  git push
else
  echo
...
echo
echo "DONE V25.5 POLISH EXACT:"
echo "  - source state matched V25.4 EXACT exactly"
echo "  - previous sorting CSS was replaced, not layered"
echo "  - overlay is lighter"
echo "  - sheet surface is slightly lighter"
echo "  - option labels are less bold"
echo "  - icons and inactive radios are brighter"
echo "  - active radio is cyan"
echo "  - trigger chevron is neutral"
