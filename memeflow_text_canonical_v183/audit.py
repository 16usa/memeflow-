...
APP = Path(sys.argv[1])
CANON = "memeflow-x-canonical-v183.css"
...
for page in PAGES:
    ...
    if sum(x==CANON for x in links)!=1:
        errors.append(f"{page}: V183 canonical count != 1")
    if not links or links[-1]!=CANON:
        errors.append(f"{page}: V183 is not LAST stylesheet")
    ...
    for stale in (
        ...
        "memeflow-x-canonical-v180.css","memeflow-x-canonical-v181.css","memeflow-x-canonical-v182.css",
    ):
        if stale in html:
...
for page in PAGES:
    ...
    for si,script in enumerate(re.findall(r'<script\b(?![^>]*\bsrc=)[^>]*>([\s\S]*?)</script>',html,re.I),1):
        if (JS_DIRECT_PROP_RE.search(script) or JS_SET_PROPERTY_RE.search(script)
            or JS_TEXT_CUSTOM_PROP_RE.search(script) or JS_STYLE_ATTR_RE.search(script) or JS_CSSTEXT_RE.search(script)):
...
for name in sorted(active_js):
    ...
    if (JS_DIRECT_PROP_RE.search(js) or JS_SET_PROPERTY_RE.search(js)
        or JS_TEXT_CUSTOM_PROP_RE.search(js) or JS_STYLE_ATTR_RE.search(js) or JS_CSSTEXT_RE.search(js)):
...
for required in (
    ...
    if required.lower() not in canon.lower():
        errors.append(f"V183 runtime canonical rule missing {required}")

if errors:
    print("V183 DEEP TEXT CONFLICT AUDIT: FAIL")
    for e in errors[:180]:
        print(" -",e)
    if len(errors)>180:
...
print("V183 DEEP TEXT CONFLICT AUDIT: PASS")
print(" - production pages fully audited: 11")
print(f" - active stylesheets audited: {len(active)}")
print(" - ONE text appearance owner: memeflow-x-canonical-v183.css")
