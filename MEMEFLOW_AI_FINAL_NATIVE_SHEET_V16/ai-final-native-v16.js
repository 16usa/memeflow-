/* MEMEFLOW AI Final Native Sheet v16.0
...
(() => {
  ...
  if (window.__MEMEFLOW_AI_FINAL_V16__) return;
  window.__MEMEFLOW_AI_FINAL_V16__ = true;

  const OPEN_BTN_ID = 'mfManualAiButton';
  const SHEET_ID = 'sheet-ai-final-v16';
  const STYLE_ID = 'mf-ai-final-v16-style';
...
`;
  ...
  function ensureSheet() {
    ...
    sheet.innerHTML = `
      <div class="sheet-top">
        ...
        <button id="mfAiV16Close" class="close-sheet" type="button" aria-label="Close AI">×</button>
      </div>

      <div class="mf-ai-body">
        <div id="mfAiV16Status" class="mf-ai-status">Ready.</div>
        ...
        <button id="mfAiV16Ask" class="btn mf-ai-ask" type="button">Ask AI</button>
    ...
    const close = sheet.querySelector('#mfAiV16Close');
    ...
    const mint = sheet.querySelector('#mfAiV16Mint');
    const prompt = sheet.querySelector('#mfAiV16Prompt');
    const ask = sheet.querySelector('#mfAiV16Ask');
  ...
  function forceHideLegacyBackend(root) {
    ...
    root.classList.add('mf-ai-v16-backend-hidden');
    ...
    if (backendOverlay && backendOverlay !== root) {
      backendOverlay.classList.add('mf-ai-v16-backend-hidden');
  ...
  function runManualScanFromAi() {
    const sheet = document.getElementById(SHEET_ID);
    const aiMint = sheet?.querySelector('#mfAiV16Mint');
    ...
    // Use the page's real MANUAL AI SCAN input + button.
    // This guarantees the same evaluator, current Settings and result pipeline.
    ...
    // Return to Home so the user sees the exact same result UI as a normal manual scan.
    closeSheet(true);
  ...
  function setStatus(value) {
    const el = document.getElementById('mfAiV16Status');
  ...
  function bindManualButton() {
    ...
    if (btn.dataset.mfAiFinalV16 === '1') return true;

    btn.dataset.mfAiFinalV16 = '1';
