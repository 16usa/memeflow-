...
PATCH_NAME="MEMEFLOW token avatar row-fit v2-safe-dirty-tree"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_BRANCH="backup/token-avatar-fit-v2-${STAMP}"
...
# Unrelated Replit/runtime changes are allowed.
# Safety rule: only the two files this patch touches must be clean.
if ! git diff --quiet -- "$CSS" "$HTML" || ! git diff --cached --quiet -- "$CSS" "$HTML"; then
  echo "ERROR: $CSS or $HTML already has local changes."
  echo "Nothing was changed. Send 'git status --short -- $CSS $HTML' before retrying."
  exit 1
fi
...
echo "==> Staging ONLY trading.css + trading.html..."
git add "$CSS" "$HTML"
echo "==> Committing ONLY patch files..."
...
echo "Unrelated existing modified/untracked files were left untouched."
echo
