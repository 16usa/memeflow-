(()=>{
  ...
  const game={
    ...
    raf:null,lifecyclePaused:false,flightState:'idle',localFeedVisualStale:false,lastStage:'ground',stageTimer:null,historyClearTimer:null,lastVisualAt:0,fallbackInFlight:false,resyncPromise:null,startCancelArmedAt:0,resetInFlight:false,clearHistoryInFlight:false,streamOpenedAt:0,streamAcceptedAt:0,streamReconnectAt:0,wakeRetryTimer:null
  };
  ...
  async function syncWakeLock(){
    ...
    if(!shouldHold){clearTimeout(game.wakeRetryTimer);game.wakeRetryTimer=null;if(game.wakeLock){try{await game.wakeLock.release();}catch{}game.wakeLock=null;}return;}
    if(game.wakeLock||!navigator.wakeLock?.request)return;
    try{
      const lock=await navigator.wakeLock.request('screen');game.wakeLock=lock;
      lock.addEventListener?.('release',()=>{if(game.wakeLock===lock)game.wakeLock=null;if(game.pageVisible&&['live','settling'].includes(game.mode)){clearTimeout(game.wakeRetryTimer);game.wakeRetryTimer=setTimeout(()=>void syncWakeLock(),700);}},{once:true});
    }catch{}
  }
  ...
  function bind(){
    ...
    addEventListener('online',()=>{ui.network.hidden=false;ui.networkText.textContent='Connection restored. Re-syncing server-authoritative round…';void resyncStatus().catch(()=>startFallback());});
    ...
    addEventListener('beforeunload',()=>{clearTimeout(game.historyClearTimer);clearTimeout(game.wakeRetryTimer);game.wakeRetryTimer=null;stopClock();clearVisualTimers();game.completionSeq++;game.pendingResultId=null;stopVisualLoop();game.stream?.close?.();game.stream=null;stopFallback();game.fx?.stop?.();game.searchAbort?.abort();if(game.wakeLock){try{game.wakeLock.release();}catch{}game.wakeLock=null;}});
  }
