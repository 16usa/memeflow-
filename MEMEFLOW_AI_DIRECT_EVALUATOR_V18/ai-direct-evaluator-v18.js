/* MEMEFLOW AI Direct Evaluator Sheet v18.0
...
(() => {
  ...
  if (window.__MEMEFLOW_AI_DIRECT_V18__) return;
  window.__MEMEFLOW_AI_DIRECT_V18__ = true;

  const OPEN_BTN_ID = 'mfManualAiButton';
  const SHEET_ID = 'sheet-ai-direct-v18';
  const STYLE_ID = 'mf-ai-direct-v18-style';
...
@media(max-width:820px){
  ...
  #mfAiV18DirectResultWrap{display:none;width:100%;margin-top:2px}
  #mfAiV18DirectResultWrap.show{display:block}
  #mfAiV18DirectResultHead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:2px 0 8px}
  #mfAiV18DirectResultHead b{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#54ddff}
  #mfAiV18DirectResultHead span{font-size:9px;color:#8290a2}
  #mfAiV18DirectResult{display:grid;gap:10px;width:100%;min-width:0}
  #mfAiV18DirectResult .mf-v18-summary{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:13px}
  #mfAiV18DirectResult .mf-v18-summary h3{margin:0 0 5px;font-size:15px;color:#f4f8fb}
  #mfAiV18DirectResult .mf-v18-summary p{margin:0;color:#93a1b2;font-size:11px;line-height:1.5}
  #mfAiV18DirectResult .mf-v18-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  #mfAiV18DirectResult .mf-v18-metric{min-width:0;border:1px solid #1d2936;border-radius:12px;background:#070c12;padding:11px}
  #mfAiV18DirectResult .mf-v18-metric small{display:block;color:#8290a2;font-size:8px;letter-spacing:.09em;text-transform:uppercase}
  #mfAiV18DirectResult .mf-v18-metric b{display:block;margin-top:5px;color:#f4f8fb;font-size:13px;overflow-wrap:anywhere}
  #mfAiV18DirectResult .mf-v18-section{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:12px}
  #mfAiV18DirectResult .mf-v18-section>small{display:block;margin-bottom:8px;color:#54ddff;font-size:8px;letter-spacing:.1em;text-transform:uppercase}
  #mfAiV18DirectResult .mf-v18-row{display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-top:1px solid rgba(255,255,255,.055);font-size:10px}
  #mfAiV18DirectResult .mf-v18-row:first-of-type{border-top:0}
  #mfAiV18DirectResult .mf-v18-row span{color:#8290a2}
  #mfAiV18DirectResult .mf-v18-row b{color:#f4f8fb;text-align:right;overflow-wrap:anywhere}
  #mfAiV18DirectResult .mf-v18-pre{margin:0;white-space:pre-wrap;overflow-wrap:anywhere;color:#dce6ef;font:10px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}
  #mfAiV18DirectResult .mf-v18-loading{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:14px;color:#8290a2;font-size:11px;line-height:1.5}
  #mfAiV18DirectResult .mf-v18-error{border:1px solid rgba(255,101,118,.28);border-radius:14px;background:#070c12;padding:14px;color:#ff6576;font-size:11px;line-height:1.5}
...
`;
  ...
  function ensureSheet() {
    ...
    sheet.innerHTML = `
      <div class="sheet-top">
        ...
        <button id="mfAiV18Close" class="close-sheet" type="button" aria-label="Close AI">×</button>
      </div>

      <div class="mf-ai-body">
        <div id="mfAiV18Status" class="mf-ai-status">Ready.</div>
        ...
        <button id="mfAiV18Ask" class="btn mf-ai-ask" type="button">Ask AI</button>

        <div id="mfAiV15Output" class="mf-ai-output">Ready.</div>

        <div id="mfAiV18DirectResultWrap">
          <div id="mfAiV18DirectResultHead">
            <b>AI Analysis &amp; Market Data</b>
            <span id="mfAiV18DirectResultState">Waiting</span>
          </div>
          <div id="mfAiV18DirectResult"></div>
        </div>
      </div>
    `;
    ...
    const close = sheet.querySelector('#mfAiV18Close');
    ...
    const mint = sheet.querySelector('#mfAiV18Mint');
    const prompt = sheet.querySelector('#mfAiV18Prompt');
    const ask = sheet.querySelector('#mfAiV18Ask');
  ...
  function forceHideLegacyBackend(root) {
    ...
    root.classList.add('mf-ai-v18-backend-hidden');
    ...
    if (backendOverlay && backendOverlay !== root) {
      backendOverlay.classList.add('mf-ai-v18-backend-hidden');
  ...
  function directConfig() {
    return window.__MEMEFLOW_AI_DIRECT_V18_CONFIG__ || {};
  }

  function setDirectResultState(label) {
    const state = document.getElementById('mfAiV18DirectResultState');
    if (state) state.textContent = label || '';
  }

  function directResultHost() {
    return {
      wrap: document.getElementById('mfAiV18DirectResultWrap'),
      host: document.getElementById('mfAiV18DirectResult')
    };
  }
  ...
  function showDirectLoading() {
    ...
    if (host) host.innerHTML = '<div class="mf-v18-loading">Running MEMEFLOW evaluator directly. MANUAL AI SCAN remains untouched.</div>';
  }
  ...
  function fmtMetric(label, value) {
    ...
    return `<div class="mf-v18-metric"><small>${escapeHtml(label)}</small><b>${escapeHtml(shown)}</b></div>`;
  }
  ...
  function renderRowsSection(title, value) {
    ...
    return `<div class="mf-v18-section"><small>${escapeHtml(title)}</small>${rows.map(([k,v]) => `<div class="mf-v18-row"><span>${escapeHtml(k)}</span><b>${escapeHtml(safeString(v))}</b></div>`).join('')}</div>`;
  }
  ...
  function renderDirectResult(raw) {
    ...
    const summary = `<div class="mf-v18-summary"><h3>${escapeHtml(decision || 'Analysis complete')}</h3><p>${escapeHtml(reason || 'MEMEFLOW evaluator returned a result for this token.')}</p></div>`;
    const grid = metricHtml ? `<div class="mf-v18-grid">${metricHtml}</div>` : '';
    const evidenceHtml = renderRowsSection('Evidence', evidence);
    const timelineHtml = renderRowsSection('Timeline', timeline);
    const rawHtml = `<details class="mf-v18-section"><summary style="cursor:pointer;color:#8290a2;font-size:9px;letter-spacing:.08em;text-transform:uppercase">Raw evaluator result</summary><pre class="mf-v18-pre">${escapeHtml(JSON.stringify(raw, null, 2))}</pre></details>`;
  ...
  function renderDirectError(message) {
    const { wrap, host } = directResultHost();
    wrap?.classList.add('show');
    if (host) host.innerHTML = `<div class="mf-v18-error">${escapeHtml(message)}</div>`;
  ...
  async function requestDirectEvaluator(mint) {
    ...
    if (method === 'GET') {
      const response = await fetch(makeGetUrl(endpoint, cfg.queryKey || 'mint', mint), {
        method:'GET',
        credentials:'include',
        cache:'no-store',
  ...
  async function runDirectEvaluatorFromAi() {
    const sheet = document.getElementById(SHEET_ID);
    const aiMint = sheet?.querySelector('#mfAiV18Mint');
  ...
  function setStatus(value) {
    const el = document.getElementById('mfAiV18Status');
  ...
  function bindManualButton() {
    ...
    if (btn.dataset.mfAiDirectV18 === '1') return true;

    btn.dataset.mfAiDirectV18 = '1';
