...
echo "=== MEMEFLOW SHADOW CONFIDENCE GOVERNOR V23.7.1 INSTALLER FIX ==="
...
# V23.7.1 INSTALLER FIX:
# Normalize EOF on generated source/test files so git diff --check cannot fail
# on "new blank line at EOF". Keep exactly one trailing newline.
python3 - <<'PY'
from pathlib import Path

for name in [
    "memeflow-app/src/shadow-confidence-governor-v23_7.mjs",
    "memeflow-app/tests/shadow-confidence-governor-v23_7.mjs",
]:
    path = Path(name)
    data = path.read_text(encoding="utf-8")
    path.write_text(data.rstrip("\n") + "\n", encoding="utf-8")

print("V23_7_EOF_NORMALIZATION_OK")
PY
