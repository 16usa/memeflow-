...
# MEMEFLOW_LIVE_CARD_BATCH_MINT_HOTPATH_V76_3_CURRENT_REMOTE_INSTALLER
#
# Exact baseline:
...
V75="5e4cf44fcbc919c630b7cdbf4b95b22e7417b1a3"
CURRENT_BASE="0c88f020d812148e6aa48bc082eff75bdf5eca8a"
...
echo "==> V76.3 PUSH-ONLY current-remote preflight"

# IMPORTANT: do not inspect/switch/reset/stash the main workspace.
# V76.3 works entirely in an isolated detached worktree.

git fetch --quiet origin "$BRANCH"

[[ "$(git rev-parse "origin/$BRANCH")" == "$CURRENT_BASE" ]] || {
  echo "expected current remote: $CURRENT_BASE"
  echo "actual remote:           $(git rev-parse "origin/$BRANCH")"
  die "remote moved again; regenerate from the new remote head"
}

[[ "$(git rev-parse "$V75^")" == "$V74" ]] || die "V75 parent is not exact V74"
[[ "$(git show -s --format=%s "$V75")" == "$V75_SUBJECT" ]] || die "V75 subject changed"

git merge-base --is-ancestor "$V75" "$CURRENT_BASE" || {
  die "current remote does not contain V75 in its ancestry"
}

echo "Current remote: exact $CURRENT_BASE"
echo "V75 ancestry:   CONFIRMED"
echo "Main workspace: WILL NOT BE MODIFIED"

echo "==> Create isolated V76.3 worktree from CURRENT remote head"

git worktree add --detach "$PATCH_WT" "$CURRENT_BASE" >/dev/null
PATCH_WT_ADDED=1
cd "$PATCH_WT"
...
mkdir -p "$BACKUP"
printf '%s\n' "$CURRENT_BASE" > "$BACKUP/base-commit.txt"
...
echo
echo "Validated:"
echo "  exact current remote baseline 0c88f020 (contains V75 + later UI commits)"
