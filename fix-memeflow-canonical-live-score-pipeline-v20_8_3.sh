...
NEWTEST="memeflow-app/tests/canonical-live-score-pipeline-v20_8_3.mjs"

for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST"; do
  [[ -f "$f" ]] || { echo "ERROR missing $f"; exit 1; }
done

[[ "$(git branch --show-current)" == "$BRANCH" ]] || { echo "V20.8.3 REFUSED: wrong branch"; exit 1; }
[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V20.8.3 REFUSED: audited HEAD mismatch";
...
for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST"; do
  git diff --quiet -- "$f" || { echo "V20.8.3 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V20.8.3 REFUSED: staged changes in $f"; exit 1; }
done

echo "=== V20.8.3 AUDITED PRECHECK ==="
python3 - <<'PY'
...
BACKUP=".memeflow-backups/canonical-live-score-v20_8_3-$STAMP"
mkdir -p "$BACKUP/memeflow-app/src" "$BACKUP/memeflow-app/tests"
for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST"; do cp "$f" "$BACKUP/$f"; done

rollback(){
 rc=$?
 if [[ $rc -ne 0 ]]; then
  echo "=== V20.8.3 FAILED — RESTORING ==="
  for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST"; do cp "$BACKUP/$f" "$f" || true; done
  rm -f "$NEWTEST"
...
r=rep(r,"""      const liveCandidate = isLiveCandidateState(view.state);
      ...
      return {
        ...view,
        decisionScore,
        // MEMEFLOW_CANONICAL_VISIBLE_AI_SCORE_V20_8_3
        score: decisionScore,
        feedScore: relevanceScore,
        relevanceScore,
        statePriority: statePriority(view.state)
...
new="""// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_3
...
route=route[:vs]+"""  // MEMEFLOW_UNIFIED_FULL_LIVE_VIEW_V20_8_3
...
const __mfLiveStatesResponseCache=new Map();""","""// MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8_3
...
a=a[:ws]+"  // MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8_3\n  // Intentionally no live-state response cache write.\n"+a[we:]
APP.write_text(a)
...
u=rep(u,"""const __MF_TOKEN_IDENTITY_KEYS_V20_2=[
  ...
  // MEMEFLOW_STATIC_IDENTITY_ONLY_V20_8_3
  'name','metadataName','symbol','metadataSymbol',
  'image','imageUrl','imageUri','logo','logoUrl','logoURI'
];""",'identity keys')
...
u=rep(u,"""    const incomingByMint=new Map(
    ...
    // MEMEFLOW_VISIBLE_INVALIDATION_SCOPE_V20_8_3
...
u=rep(u,"""        const scoreA =
        ...
        // MEMEFLOW_SCORE_FIRST_TIEBREAK_V22""","""        // MEMEFLOW_SMART_HIDDEN_FEED_RANK_V20_8_3
...
# V20.8.3: migrate stale regressions that intentionally asserted deleted architecture.
# They now assert the canonical single-score contract instead.
t=TEST.read_text()
t=rep(t,"assert.match(app,/MEMEFLOW_WAITING_PREVIEW_SCORE_V21/);","assert.match(app,/MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_3/);",'feed test old preview marker')
TEST.write_text(t)

ct=CANON.read_text()
ct=ct.replace("assert.match(app,/MEMEFLOW_CANONICAL_SCORE_STATE_V20_7/);","assert.match(app,/MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_3/);")
ct=ct.replace("const ds=app.indexOf('const __v20truth=__mfCurrentEntryTruthV20_2');","const ds=app.indexOf('// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_3');")
...
# V20.8.3: the full-suite strict/fresh scanner tests still asserted the old
...
h=HTML.read_text(); h,n=re.subn(r'src="/system-tokens\.js\?v=[^"]+"','src="/system-tokens.js?v=canonical-live-score-v20-8-3-20260903"',h,count=1)
if n!=1: raise SystemExit('asset cache-bust anchor missing')
HTML.write_text(h)
print('V20_8_3_TRANSFORM_OK')
PY
...
assert.match(rankSource,/MEMEFLOW_CANONICAL_VISIBLE_AI_SCORE_V20_8_3/);
assert.doesNotMatch(rankSource,/score:\s*liveCandidate\s*\?/);

const ds=app.indexOf('// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_3');
...
assert.match(full,/MEMEFLOW_UNIFIED_FULL_LIVE_VIEW_V20_8_3/);
...
assert.match(ui,/MEMEFLOW_VISIBLE_INVALIDATION_SCOPE_V20_8_3/);
assert.match(ui,/if\(!requestedMints\.has\(mint\)\)return previous;/);
assert.match(ui,/MEMEFLOW_SMART_HIDDEN_FEED_RANK_V20_8_3/);
assert.match(html,/system-tokens\.js\?v=canonical-live-score-v20-8-3-20260903/);

console.log('CANONICAL_LIVE_SCORE_PIPELINE_V20_8_3_OK');
TESTJS

echo "=== V20.8.3 REGRESSION CONTRACT PRECHECK ==="
...
echo "=== V20.8.3 TARGETED TESTS ==="
(
 cd memeflow-app
 node tests/canonical-live-score-pipeline-v20_8_3.mjs
...
echo "=== V20.8.3 FULL TEST SUITE ==="
(cd memeflow-app && npm test)

git diff --check -- "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$NEWTEST"

echo "=== V20.8.3 STATIC INVARIANTS ==="
python3 - <<'PY'
from pathlib import Path
