/* MEMEFLOW AI Direct Evaluator Sheet v22.0
...
(() => {
  ...
  if (window.__MEMEFLOW_AI_DIRECT_V22__) return;
  window.__MEMEFLOW_AI_DIRECT_V22__ = true;

  const OPEN_BTN_ID = 'mfManualAiButton';
  const SHEET_ID = 'sheet-ai-direct-v22';
  const STYLE_ID = 'mf-ai-direct-v22-style';
  const MORE_PROXY_ID = 'mf-mobile-more-proxy';
  const AI_NAV_ID = 'mf-ai-center-nav-v22';
  const HEADER_WALLET_ID = 'mf-header-wallet-v22';
...
@media(max-width:820px){
  ...
  #mfAiV22DirectResultWrap{display:none;width:100%;margin-top:2px}
  #mfAiV22DirectResultWrap.show{display:block}
  #mfAiV22DirectResultHead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:2px 0 8px}
  #mfAiV22DirectResultHead b{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#54ddff}
  #mfAiV22DirectResultHead span{font-size:9px;color:#8290a2}
  #mfAiV22DirectResult{display:grid;gap:10px;width:100%;min-width:0}
  #mfAiV22DirectResult .mf-v22-summary{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:13px}
  #mfAiV22DirectResult .mf-v22-summary h3{margin:0 0 5px;font-size:15px;color:#f4f8fb}
  #mfAiV22DirectResult .mf-v22-summary p{margin:0;color:#93a1b2;font-size:11px;line-height:1.5}
  #mfAiV22DirectResult .mf-v22-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  #mfAiV22DirectResult .mf-v22-metric{min-width:0;border:1px solid #1d2936;border-radius:12px;background:#070c12;padding:11px}
  #mfAiV22DirectResult .mf-v22-metric small{display:block;color:#8290a2;font-size:8px;letter-spacing:.09em;text-transform:uppercase}
  #mfAiV22DirectResult .mf-v22-metric b{display:block;margin-top:5px;color:#f4f8fb;font-size:13px;overflow-wrap:anywhere}
  #mfAiV22DirectResult .mf-v22-section{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:12px}
  #mfAiV22DirectResult .mf-v22-section>small{display:block;margin-bottom:8px;color:#54ddff;font-size:8px;letter-spacing:.1em;text-transform:uppercase}
  #mfAiV22DirectResult .mf-v22-row{display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-top:1px solid rgba(255,255,255,.055);font-size:10px}
  #mfAiV22DirectResult .mf-v22-row:first-of-type{border-top:0}
  #mfAiV22DirectResult .mf-v22-row span{color:#8290a2}
  #mfAiV22DirectResult .mf-v22-row b{color:#f4f8fb;text-align:right;overflow-wrap:anywhere}
  #mfAiV22DirectResult .mf-v22-pre{margin:0;white-space:pre-wrap;overflow-wrap:anywhere;color:#dce6ef;font:10px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}
  #mfAiV22DirectResult .mf-v22-loading{border:1px solid #1d2936;border-radius:14px;background:#070c12;padding:14px;color:#8290a2;font-size:11px;line-height:1.5}
  #mfAiV22DirectResult .mf-v22-error{border:1px solid rgba(255,101,118,.28);border-radius:14px;background:#070c12;padding:14px;color:#ff6576;font-size:11px;line-height:1.5}
...
`;
  ...
  function ensureSheet() {
    ...
    sheet.innerHTML = `
      <div class="sheet-top">
        ...
        <button id="mfAiV22Close" class="close-sheet" type="button" aria-label="Close AI">×</button>
      </div>

      <div class="mf-ai-body">
        <div id="mfAiV22Status" class="mf-ai-status">Ready.</div>
        ...
        <button id="mfAiV22Ask" class="btn mf-ai-ask" type="button">Ask AI</button>

        <div id="mfAiV15Output" class="mf-ai-output">Ready.</div>

        <div id="mfAiV22DirectResultWrap">
          <div id="mfAiV22DirectResultHead">
            <b>AI Analysis &amp; Market Data</b>
            <span id="mfAiV22DirectResultState">Waiting</span>
          </div>
          <div id="mfAiV22DirectResult"></div>
        </div>
      </div>
    `;
    ...
    const close = sheet.querySelector('#mfAiV22Close');
    ...
    const mint = sheet.querySelector('#mfAiV22Mint');
    const prompt = sheet.querySelector('#mfAiV22Prompt');
    const ask = sheet.querySelector('#mfAiV22Ask');
  ...
  function forceHideLegacyBackend(root) {
    ...
    root.classList.add('mf-ai-v22-backend-hidden');
    ...
    if (backendOverlay && backendOverlay !== root) {
      backendOverlay.classList.add('mf-ai-v22-backend-hidden');
  ...
  function directConfig() {
    return window.__MEMEFLOW_AI_DIRECT_V22_CONFIG__ || {};
  }

  function setDirectResultState(label) {
    const state = document.getElementById('mfAiV22DirectResultState');
    if (state) state.textContent = label || '';
  }

  function directResultHost() {
    return {
      wrap: document.getElementById('mfAiV22DirectResultWrap'),
      host: document.getElementById('mfAiV22DirectResult')
    };
  }
  ...
  function showDirectLoading() {
    ...
    if (host) host.innerHTML = '<div class="mf-v22-loading">Running MEMEFLOW evaluator directly. MANUAL AI SCAN remains untouched.</div>';
  }
  ...
  function fmtMetric(label, value) {
    ...
    return `<div class="mf-v22-metric"><small>${escapeHtml(label)}</small><b>${escapeHtml(shown)}</b></div>`;
  }
  ...
  function renderRowsSection(title, value) {
    ...
    return `<div class="mf-v22-section"><small>${escapeHtml(title)}</small>${rows.map(([k,v]) => `<div class="mf-v22-row"><span>${escapeHtml(k)}</span><b>${escapeHtml(safeString(v))}</b></div>`).join('')}</div>`;
  }
  ...
  function renderDirectResult(raw) {
    ...
    const summary = `<div class="mf-v22-summary"><h3>${escapeHtml(decision || 'Analysis complete')}</h3><p>${escapeHtml(reason || 'MEMEFLOW evaluator returned a result for this token.')}</p></div>`;
    const grid = metricHtml ? `<div class="mf-v22-grid">${metricHtml}</div>` : '';
    const evidenceHtml = renderRowsSection('Evidence', evidence);
    const timelineHtml = renderRowsSection('Timeline', timeline);
    const rawHtml = `<details class="mf-v22-section"><summary style="cursor:pointer;color:#8290a2;font-size:9px;letter-spacing:.08em;text-transform:uppercase">Raw evaluator result</summary><pre class="mf-v22-pre">${escapeHtml(JSON.stringify(raw, null, 2))}</pre></details>`;
  ...
  function renderDirectError(message) {
    const { wrap, host } = directResultHost();
    wrap?.classList.add('show');
    if (host) host.innerHTML = `<div class="mf-v22-error">${escapeHtml(message)}</div>`;
  ...
  async function runDirectEvaluatorFromAi() {
    const sheet = document.getElementById(SHEET_ID);
    const aiMint = sheet?.querySelector('#mfAiV22Mint');
  ...
  function setStatus(value) {
    const el = document.getElementById('mfAiV22Status');
  ...
  function bindManualButton() {
    ...
    if (btn.dataset.mfAiDirectV22 === '1') return true;

    btn.dataset.mfAiDirectV22 = '1';
