...
BASELINE_HEAD = "c8f437a7317f68242f77f4a23541379fdfa253bd"
...
if not SELFTEST:
    ...
    if head != BASELINE_HEAD:
        print(
            "NOTE: Git HEAD differs from the original tested baseline; "
            "using structural compatibility checks instead.\n"
            f"Baseline: {BASELINE_HEAD}\nCurrent:  {head}"
        )
...
if "MEMEFLOW_RECENT_TRADES_CANONICAL_END" in trading_before:
    print("✅ Recent Trades cleanup is already installed. No files changed.")
    raise SystemExit(0)
...
v89_required = (
    "MEMEFLOW_ROW_HEIGHT_CANONICAL_V89",
    "--mf-v89-row-height",
    ".bottom-history-panel",
    ".trade-history > .trade-row.trade-log-row",
)
missing_v89 = [token for token in v89_required if token not in v89_text]
if missing_v89:
    raise SystemExit(
        "ABORT: canonical row-height V89 contract is missing or changed. Missing: "
        + ", ".join(missing_v89)
    )
...
def write(path: Path, text: str) -> None:
    # Canonicalize trailing whitespace on every written line. The V2 installer
    # could reconstruct a CSS comment line with one trailing space, which makes
    # `git diff --check` fail even though the CSS is otherwise valid.
...
try:
    ...
    for filename, version in [
        ("trading.css", "recent-trades-canonical-cleanup-v3-20260909"),
        ("trading-light-polish-v61.css", "recent-trades-cleanup-v3-20260909"),
        ("memeflow-trading-white-surfaces-v154.css", "recent-trades-cleanup-v3-20260909"),
    ]:
    ...
    v89_after = read(V89)
    missing_v89_after = [token for token in v89_required if token not in v89_after]
    if missing_v89_after:
        raise RuntimeError("V89 shared row-height contract was lost: " + ", ".join(missing_v89_after))
...
""",
    ...
    if SELFTEST:
        print("Validation: self-test gate = OK")
    elif head == BASELINE_HEAD:
        print("Validation: baseline Git HEAD = OK")
    else:
        print("Validation: structural compatibility gate = OK")
