...
PATCH_NAME="MEMEFLOW_UNIFIED_CORE_V1_2"
...
# ---------------------------------------------------------------------------
# 0) TEST/INSTANCE ISOLATION
#    The integration suite launches a second app-server with its own DATA_DIR.
#    The single-instance lock must follow that DATA_DIR instead of always using
#    the production data directory, otherwise a running Replit app exits the
#    test child with code 73 before it can print Listening.
# ---------------------------------------------------------------------------
replace_once(
    "src/single-instance-lock.mjs",
    "const DATA=path.join(APP,'data');",
    "const DATA=path.resolve(APP,process.env.DATA_DIR||'data');",
    "single-instance lock DATA_DIR isolation"
)
...
log "Backup: $BACKUP"
log "Key behavior:"
log "  - integration child uses its own DATA_DIR lock, so a running Replit server does not cause exit 73"
