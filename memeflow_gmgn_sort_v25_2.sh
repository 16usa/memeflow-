...
PATCH_ID="MEMEFLOW_GMGN_SORT_STYLE_V25_2"
OLD_VERSION="gmgn-sort-v25-1-20260827"
NEW_VERSION="gmgn-sort-v25-2-20260827"
DO_PUSH=0
...
exact_css = r'''
/* MEMEFLOW_GMGN_SORT_STYLE_V25_2
...
/* V25.2 intentionally has no close/X button. */
.mf-sort-close-v25 {
...
# Update existing tests that assert the old V25.1 version.
...
# Dedicated visual regression.
test_path = tests_dir / "token-sort-style-v25-2.mjs"
...
// New visual revision.
assert.match(ui,/MEMEFLOW_GMGN_SORT_STYLE_V25_2/);
assert.match(css,/MEMEFLOW_GMGN_SORT_STYLE_V25_2/);

// Base sorting logic remains untouched.
...
assert.doesNotMatch(
  ui.slice(
    ui.indexOf('function __mfSortSheetShellV25'),
    ui.indexOf('function __mfBindSortOverlayV25')
  ),
  /data-mf-sort-close/
);
...
assert.match(css,/\.mf-sort-close-v25\s*\{\s*display:none !important;/s);
...
// V25.1 CSS block was replaced, not layered.
assert.equal(
  (css.match(/MEMEFLOW_GMGN_SORT_V25_1/g)||[]).length,
  0
);

// Fresh browser assets.
assert.match(html,/system-tokens\.js\?v=gmgn-sort-v25-2-20260827/);
assert.match(html,/system-tokens\.css\?v=gmgn-sort-v25-2-20260827/);

console.log('token sort style v25.2 ok');
''', encoding="utf-8")

pkg = json.loads(pkg_path.read_text(encoding="utf-8"))
scripts = pkg.setdefault("scripts", {})
command = "node tests/token-sort-style-v25-2.mjs"
...
if "MEMEFLOW_GMGN_SORT_V25_1" in css:
    raise SystemExit("POSTCHECK FAILED: old V25.1 CSS remains.")

if "MEMEFLOW_GMGN_SORT_STYLE_V25_2" not in css:
    raise SystemExit("POSTCHECK FAILED: new V25.2 CSS missing.")

ui_path.write_text(ui, encoding="utf-8")
css_path.write_text(css, encoding="utf-8")
html_path.write_text(html, encoding="utf-8")

print("V25.2 exact-mockup visual patch installed.")
PY

echo
echo "==> [1/7] JavaScript syntax"
node --check system-tokens.js
node --check tests/token-sort-style-v25-2.mjs

echo "==> [2/7] V25.2 visual regression"
node tests/token-sort-style-v25-2.mjs

echo "==> [3/7] V25.1 regression"
if [[ -f tests/token-sort-style-v25-1.mjs ]]; then
  node tests/token-sort-style-v25-1.mjs
fi

echo "==> [4/7] Original V25 sort regression"
if [[ -f tests/token-sort-ui-v25.mjs ]]; then
  node tests/token-sort-ui-v25.mjs
fi

echo "==> [5/7] Existing live ranking regression"
if [[ -f tests/live-ranking-reorder-v23.mjs ]]; then
  node tests/live-ranking-reorder-v23.mjs
fi

echo "==> [6/7] Full MEMEFLOW test suite"
npm test
...
echo
echo "V25.2 validated successfully."
...
if [[ "$DO_PUSH" -eq 1 ]]; then
  echo
  echo "Pushing validated V25.2 commit..."
  git push
else
  echo
