...
CANONTEST="memeflow-app/tests/canonical-score-state-v20_7.mjs"
DETAILTEST="memeflow-app/tests/card-details-live-authority-v20_5.mjs"
STRICTTEST="memeflow-app/tests/strict-entry-admission.mjs"
FRESHTEST="memeflow-app/tests/fresh-session-scanner.mjs"
REALTIMETEST="memeflow-app/tests/realtime-update-path.mjs"
NEWTEST="memeflow-app/tests/canonical-live-score-pipeline-v20_8_8.mjs"

for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$REALTIMETEST"; do
  [[ -f "$f" ]] || { echo "ERROR missing $f"; exit 1; }
done

[[ "$(git branch --show-current)" == "$BRANCH" ]] || { echo "V20.8.8 REFUSED: wrong branch"; exit 1; }
[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V20.8.8 REFUSED: audited HEAD mismatch";
...
for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$REALTIMETEST"; do
  git diff --quiet -- "$f" || { echo "V20.8.8 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V20.8.8 REFUSED: staged changes in $f"; exit 1; }
done

echo "=== V20.8.8 INSTALLER SELF-TEST ==="
TMP_SELFTEST="$(mktemp -d)"
trap 'rm -rf "$TMP_SELFTEST"' RETURN

cat > "$TMP_SELFTEST/strict.mjs" <<'EOF'
import assert from 'node:assert/strict';
const liveStatesRoute="function __mfLiveDecisionForUserV14(";
assert.match(liveStatesRoute,/state:_blocked\?'BLOCKED':'WAITING'/);
EOF

cat > "$TMP_SELFTEST/fresh.mjs" <<'EOF'
import assert from 'node:assert/strict';
const liveRoute="function __mfLiveDecisionForUserV14(";
assert.match(liveRoute,/state:_blocked\?'BLOCKED':'WAITING'/);
EOF

python3 - "$TMP_SELFTEST/strict.mjs" "$TMP_SELFTEST/fresh.mjs" <<'PYSELF'
from pathlib import Path
import sys

for raw,label,var_name in (
    (sys.argv[1],'strict-entry-admission','liveStatesRoute'),
    (sys.argv[2],'fresh-session-scanner','liveRoute')
):
    path=Path(raw)
    ...
    matches=[
        ...
        if (
            f"assert.match({var_name},/state:_blocked" in line and
            "'BLOCKED':'WAITING'" in line
        )
    ]
    if len(matches)!=1:
        raise SystemExit(
            f'{label}: expected exactly one stale inline state assertion, found {len(matches)}'
        )
    lines[matches[0]]=f"assert.match({var_name},/__mfLiveDecisionForUserV14\\(/);"
    path.write_text("\n".join(lines)+"\n")
PYSELF

node --check "$TMP_SELFTEST/strict.mjs"
node --check "$TMP_SELFTEST/fresh.mjs"
node "$TMP_SELFTEST/strict.mjs"
node "$TMP_SELFTEST/fresh.mjs"

if grep -q '\\\\n' "$TMP_SELFTEST/strict.mjs"; then
  echo "SELF-TEST FAILED: strict migration wrote literal backslash-n"
  exit 1
fi
if grep -q '\\\\n' "$TMP_SELFTEST/fresh.mjs"; then
  echo "SELF-TEST FAILED: fresh migration wrote literal backslash-n"
  exit 1
fi

rm -rf "$TMP_SELFTEST"
trap - RETURN
echo "INSTALLER_SELF_TEST_OK"

echo "=== V20.8.8 AUDITED PRECHECK ==="
python3 - <<'PY'
from pathlib import Path
req={
...
BACKUP=".memeflow-backups/canonical-live-score-v20_8_8-$STAMP"
mkdir -p "$BACKUP/memeflow-app/src" "$BACKUP/memeflow-app/tests"
for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$REALTIMETEST"; do cp "$f" "$BACKUP/$f"; done

rollback(){
 rc=$?
 if [[ $rc -ne 0 ]]; then
  echo "=== V20.8.8 FAILED — RESTORING ==="
  for f in "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$REALTIMETEST"; do cp "$BACKUP/$f" "$f" || true; done
  rm -f "$NEWTEST"
  git reset -- "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$REALTIMETEST" "$NEWTEST" >/dev/null 2>&1 || true
...
from pathlib import Path
import re
APP=Path('memeflow-app/app-server.mjs'); UI=Path('memeflow-app/system-tokens.js'); HTML=Path('memeflow-app/system-tokens.html'); RANK=Path('memeflow-app/src/feed-ranking.mjs'); TEST=Path('memeflow-app/tests/feed-ranking.mjs'); OLD=Path('memeflow-app/tests/live-truth-no-dynamic-cache-v20_3.mjs'); CANON=Path('memeflow-app/tests/canonical-score-state-v20_7.mjs'); DETAIL=Path('memeflow-app/tests/card-details-live-authority-v20_5.mjs'); STRICT=Path('memeflow-app/tests/strict-entry-admission.mjs'); FRESH=Path('memeflow-app/tests/fresh-session-scanner.mjs'); REALTIME=Path('memeflow-app/tests/realtime-update-path.mjs')
...
new="""// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_8
...
route=route[:vs]+"""  // MEMEFLOW_UNIFIED_FULL_LIVE_VIEW_V20_8_8
...
const __mfLiveStatesResponseCache=new Map();""","""// MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8_8
...
a=a[:ws]+"  // MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8_8\n  // Intentionally no live-state response cache write.\n"+a[we:]
APP.write_text(a)
...
u=rep(u,"""        const scoreA =
        ...
        // MEMEFLOW_SCORE_FIRST_TIEBREAK_V22""","""        // MEMEFLOW_SMART_HIDDEN_FEED_RANK_V20_8_8
...
# V20.8.8: migrate stale regressions that intentionally asserted deleted architecture.
# They now assert the canonical single-score contract instead.
t=TEST.read_text()
t=rep(t,"assert.match(app,/MEMEFLOW_WAITING_PREVIEW_SCORE_V21/);","assert.match(app,/MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_8/);",'feed test old preview marker')
TEST.write_text(t)

ct=CANON.read_text()
ct=ct.replace("assert.match(app,/MEMEFLOW_CANONICAL_SCORE_STATE_V20_7/);","assert.match(app,/MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_8/);")
ct=ct.replace("const ds=app.indexOf('const __v20truth=__mfCurrentEntryTruthV20_2');","const ds=app.indexOf('// MEMEFLOW_CANONICAL_LIVE_DECISION_V20_8_8');")
ct=ct.replace("assert.match(decisionBlock,/liveTruthReason:reason/);","assert.match(decisionBlock,/liveTruthReason:reason/);")
CANON.write_text(ct)

ot=OLD.read_text()
ot=ot.replace(r"assert.match(app,/tradeEligible:eligible&&__v20truth\.pass===true/);",r"assert.match(app,/tradeEligible:isOpen\?true:eligible&&liveTruth\.pass===true/);")
ot=ot.replace('force WAITING/0 and cannot remain BUY READY','force WAITING and cannot remain BUY READY')
OLD.write_text(ot)

dt=DETAIL.read_text()
dt=dt.replace(r"/system-tokens\.js\?v=card-live-authority-v20-5-20260902/",r"/system-tokens\.js\?v=[^\"']+/")
DETAIL.write_text(dt)

if OLD.exists():
 t=OLD.read_text().replace('force WAITING/0 and cannot remain BUY READY','force WAITING and cannot remain BUY READY')
 OLD.write_text(t)

# V20.8.8: the full-suite strict/fresh scanner tests still asserted the old
# duplicated inline full-route classifier. The route is now deliberately
# canonicalized through __mfLiveDecisionForUserV14(), so migrate those
# regressions to assert the new single-source contract instead.
for path,label,var_name in (
 (STRICT,'strict-entry-admission','liveStatesRoute'),
 (FRESH,'fresh-session-scanner','liveRoute')
):
 lines=path.read_text().splitlines()
 matches=[
  i for i,line in enumerate(lines)
...
# V20.8.8: realtime-update-path used to require the now-removed dynamic
# live-states response cache knob. The new contract is the opposite:
# live mutable responses must not be cached.
t=REALTIME.read_text()
old="assert.match(app,/LIVE_STATES_RESPONSE_CACHE_MS/);"
new="assert.match(app,/MEMEFLOW_NO_DYNAMIC_RESPONSE_CACHE_V20_8_8/);"
if old not in t:
 raise SystemExit('realtime-update-path: stale response-cache assertion anchor missing')
t=t.replace(old,new,1)

# Current app deliberately caches VERSIONED system-tokens.js/css assets.
# That is static code caching with a cache-busted URL, not mutable token data.
old_asset="assert.match(app,/MEMEFLOW_LIVE_TOKEN_ASSET_NO_STORE_V1/);"
new_asset="assert.match(app,/MEMEFLOW_LOAD_PERF_V1_CACHE/);"
if old_asset not in t:
 raise SystemExit('realtime-update-path: obsolete live-token asset no-store assertion anchor missing')
t=t.replace(old_asset,new_asset,1)

# Remove/rename the obsolete COMMENT marker too. V20.8.5 correctly migrated
# executable code but its precheck accidentally matched this comment.
t=t.replace(
...
insert_after="assert.match(app,/MEMEFLOW_LOAD_PERF_V1_CACHE/);"
semantic=(
...
t=t.replace(insert_after,insert_after+semantic,1)

REALTIME.write_text(t)

# Browser JS cache-bust only.
h=HTML.read_text(); h,n=re.subn(r'src="/system-tokens\.js\?v=[^"]+"','src="/system-tokens.js?v=canonical-live-score-v20-8-8-20260903"',h,count=1)
if n!=1: raise SystemExit('asset cache-bust anchor missing')
HTML.write_text(h)
print('V20_8_8_TRANSFORM_OK')
PY
...
echo "=== V20.8.8 REGRESSION CONTRACT PRECHECK ==="
echo "Audited npm-test conflicts: feed-ranking, strict-entry-admission, fresh-session-scanner, realtime-update-path"
if grep -RIn --include='*.mjs' 'MEMEFLOW_WAITING_PREVIEW_SCORE_V21' memeflow-app/tests; then
  echo "ERROR: stale WAITING preview-score regression remains"
  exit 1
fi
if grep -RIn --include='*.mjs' 'live feed score must agree with live ordering' memeflow-app/tests; then
  echo "ERROR: stale visible-feed-score regression remains"
  exit 1
fi
if grep -RIn --include='*.mjs' "assert.match(.*state:_blocked.*'BLOCKED':'WAITING'" memeflow-app/tests; then
  echo "ERROR: stale duplicated full-route classifier regression remains"
  exit 1
fi
if grep -RIn --include='*.mjs' 'assert.match(app,/LIVE_STATES_RESPONSE_CACHE_MS/)' memeflow-app/tests; then
  echo "ERROR: stale dynamic live-states cache regression remains"
  exit 1
fi
if grep -RIn --include='*.mjs' 'assert\.match(app,/MEMEFLOW_LIVE_TOKEN_ASSET_NO_STORE_V1/)' memeflow-app/tests; then
  echo "ERROR: stale live-token asset no-store executable regression remains"
  exit 1
fi
echo "REGRESSION_CONTRACTS_OK"

echo "=== V20.8.8 TRANSFORMED FILE SYNTAX ==="
node --check "$STRICTTEST"
node --check "$FRESHTEST"
node --check "$REALTIMETEST"
node --check "$RANKTEST"
echo "TRANSFORMED_TEST_SYNTAX_OK"
...
node --check "$RANKTEST"
node --check "$OLDLIVE"
node --check "$CANONTEST"
node --check "$DETAILTEST"
node --check "$STRICTTEST"
node --check "$FRESHTEST"
node --check "$REALTIMETEST"
node --check "$NEWTEST"

echo "=== V20.8.8 TARGETED TESTS ==="
(
 cd memeflow-app
...
echo "=== V20.8.8 FULL TEST SUITE ==="
(cd memeflow-app && npm test)

git diff --check -- "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$REALTIMETEST" "$NEWTEST"

echo "=== V20.8.8 STATIC INVARIANTS ==="
python3 - <<'PY'
from pathlib import Path
...
if 'MEMEFLOW_LOAD_PERF_V1_CACHE' not in app: errs.append('versioned static asset cache contract missing')
...
echo "=== V20.8.8 STAGE ==="
git reset >/dev/null
git add "$APP" "$UI" "$HTML" "$RANK" "$RANKTEST" "$OLDLIVE" "$CANONTEST" "$DETAILTEST" "$STRICTTEST" "$FRESHTEST" "$REALTIMETEST" "$NEWTEST"
BAD="$(git diff --cached --name-only | grep -Ev '^memeflow-app/(app-server\.mjs|system-tokens\.js|system-tokens\.html|src/feed-ranking\.mjs|tests/feed-ranking\.mjs|tests/live-truth-no-dynamic-cache-v20_3\.mjs|tests/canonical-score-state-v20_7\.mjs|tests/card-details-live-authority-v20_5\.mjs|tests/strict-entry-admission\.mjs|tests/fresh-session-scanner\.mjs|tests/realtime-update-path\.mjs|tests/canonical-live-score-pipeline-v20_8_8\.mjs)$' || true)"
