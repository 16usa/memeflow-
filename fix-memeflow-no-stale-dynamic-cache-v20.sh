...
# MEMEFLOW V20 — fresh mutable truth / no stale dynamic cache
...
NEWTEST="memeflow-app/tests/no-stale-dynamic-cache-v20.mjs"
...
BACKUP=".memeflow-backups/fresh-truth-v20-$STAMP"
...
if 'MEMEFLOW_DYNAMIC_MARKET_EXPIRY_V20' not in market:
 ...
 new="""  // MEMEFLOW_DYNAMIC_MARKET_EXPIRY_V20
  const tokenTradeFresh=Boolean(
    tokenTradeAt!==null&&tokenTradeAt>0&&tokenTradeAt<=now+30000&&now-tokenTradeAt<=windowMs
  );
  const tokenHasTradeEvidence=Boolean(
    tokenPrice!==null&&tokenPrice>0&&tokenTradeFresh&&
    (marketSource.includes('trade')||liveMarketCapSource.includes('trade')||finite(token?.lastMarketActivityAt)!==null)
  );"""
 if old not in market: raise SystemExit('V20 REFUSED: token trade evidence anchor missing')
 market=market.replace(old,new,1)
 old="""  const marketCapSol=
    liveMarketCapSol ??
    storedMarketCapSol;"""
 new="""  // Mutable MC is live evidence only; persisted MC cannot resurrect a dead token.
  const marketCapSol=liveMarketCapSol;"""
 if old not in market: raise SystemExit('V20 REFUSED: MC SOL fallback anchor missing')
 market=market.replace(old,new,1)
 ...
 new="""  const marketCapUsd=
    marketCapSol!==null&&marketCapSol>0&&usd!==null&&usd>0
      ? marketCapSol*usd
      : null;"""
 if old not in market: raise SystemExit('V20 REFUSED: MC USD fallback anchor missing')
 market=market.replace(old,new,1)
 ...
 if old not in market: raise SystemExit('V20 REFUSED: MC source fallback anchor missing')
...
html,n=re.subn(r'src="/system-tokens\.js\?v=[^"]+"','src="/system-tokens.js?v=fresh-truth-v20-20260902"',html,count=1)
...
console.log('NO_STALE_DYNAMIC_CACHE_V20_OK');
TESTJS
...
git commit -m "fix: remove stale dynamic token cache and gate live decisions"
