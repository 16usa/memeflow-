(()=>{
  ...
  const ui={
    ...
    tracePath:$('#tracePath'),traceDot:$('#traceDot'),autoTrigger:$('#autoTrigger'),autoTriggerLabel:$('#autoTriggerLabel'),stopTrigger:$('#stopTrigger'),stopTriggerLabel:$('#stopTriggerLabel'),rocket:$('#rocket'),pepeEmote:$('#pepeEmote'),pepeEmoteIcon:$('#pepeEmoteIcon'),pepeEmoteText:$('#pepeEmoteText'),moonBeacon:$('#moonBeacon'),
    center:$('#centerState'),centerKicker:$('#centerKicker'),centerValue:$('#centerValue'),centerLabel:$('#centerLabel'),milestone:$('#milestone'),milestoneValue:$('#milestoneValue'),milestoneText:$('#milestoneText'),stale:$('#staleCover'),shockwave:$('#shockwave'),emergencyFlash:$('#emergencyFlash'),secureFlash:$('#secureFlash'),stageTransition:$('#stageTransition'),stageTransitionLabel:$('#stageTransitionLabel'),stageTransitionMeta:$('#stageTransitionMeta'),targetReticle:$('#targetReticle'),preflightState:$('#preflightState'),preflightKicker:$('#preflightKicker'),preflightTitle:$('#preflightTitle'),preflightText:$('#preflightText'),
  ...
  function syncPreflight(mode=game.mode){
    if(!ui.preflightState)return;
    if(mode==='searching'){
      setText(ui.preflightKicker,'SCANNING MARKET');setText(ui.preflightTitle,'Searching for BUY READY');setText(ui.preflightText,'MEMEFLOW decides eligibility from your saved settings. Game adds no trading filters.');
    }else if(mode==='idle'){
      setText(ui.preflightKicker,'READY');setText(ui.preflightTitle,'Pepe is on the launchpad');setText(ui.preflightText,'Set your PAPER stake and press START when ready.');
    }else if(mode==='complete'){
      setText(ui.preflightKicker,'ROUND COMPLETE');setText(ui.preflightTitle,'Flight settled');setText(ui.preflightText,'Server result is ready.');
    }
  }
  ...
  function updateSearchRadar(){
    ...
    ui.selectorStatus.dataset.step=step;ui.selectorPhase.textContent=`${label} ${slot+1}/4`;if(ui.preflightKicker)setText(ui.preflightKicker,`SCANNING · ${label}`);
  ...
  function updateTriggerLines(){const locked=game.session&&['LIVE','COMPLETE'].includes(game.session.state),auto=num(locked?game.session.autoCashout:ui.auto.value)||0,stop=num(locked?game.session.stopLoss:ui.stop.value)||0,key=`${locked?'L':'E'}:${auto}:${stop}`;if(key===game.lastTriggerKey)return;game.lastTriggerKey=key;ui.autoTrigger.hidden=!(auto>1);if(auto>1){ui.autoTrigger.style.bottom=lineBottom(auto);setText(ui.autoTriggerLabel,`AUTO ${auto.toFixed(2)}×`);}ui.stopTrigger.hidden=!(stop>0&&stop<1);if(stop>0&&stop<1){ui.stopTrigger.style.bottom=lineBottom(stop);setText(ui.stopTriggerLabel,`STOP ${stop.toFixed(2)}×`);}updateRiskPreview();}
  ...
  function renderVisual(multiplier){
    ...
    const tiltTarget=clamp(-5-v*18-game.acceleration*46-(stage==='moon'||stage==='deep'||stage==='hyper'?4:0),-24,14);game.visualTilt+=(tiltTarget-game.visualTilt)*(Math.abs(tiltTarget-game.visualTilt)>8?.18:.11);const bob=game.mode==='live'&&!reducedMotion?Math.sin(performance.now()/510)*(0.6+Math.min(2.1,Math.abs(v)*12)):0;ui.rocket.style.setProperty('--rocket-bob',`${bob.toFixed(2)}px`);updateStoryFlight(m,direction,game.dangerLevel,stage,label,v);ui.rocket.style.transform=`translate(calc(-50% + var(--flight-path-x,0px)), calc(var(--rocket-bob,0px) - var(--flight-path-y,0px))) rotate(${game.visualTilt.toFixed(2)}deg) scale(var(--flight-path-scale,1))`;
    ...
    if(danger==='high'&&game.dangerLevel!=='high'&&Date.now()-game.lastDangerHapticAt>2600){game.lastDangerHapticAt=Date.now();haptic([18,32,18]);tone(165,.09,'sawtooth',.012,-35);}game.dangerLevel=danger;setFlightState(resolveFlightState());updateStoryFlight(m,direction,danger,stage,label,v);
  ...
  function updateStoryFlight(multiplier,direction,danger,stage,label,velocity){
    if(!ui.rocket||!ui.world)return;
    const worldRect=ui.world.getBoundingClientRect();
    const rawProgress=clamp((levelNorm(multiplier)-.08)/.84,0,1);
    const retract=clamp(Math.max(0,-velocity)*1.6,0,.12);
    const progress=clamp(rawProgress-retract,0,1);
    const arc=Math.sin(progress*Math.PI)*0.08;
    const pathX=worldRect.width*(0.30*progress+0.08*arc);
    const pathY=worldRect.height*(0.56*Math.pow(progress,.92)+0.07*arc);
    const pathScale=clamp(1.04-progress*.18+(game.flightState==='boost'?.04:0),.78,1.08);
    ui.world.style.setProperty('--flight-path-x',`${pathX.toFixed(2)}px`);
    ui.world.style.setProperty('--flight-path-y',`${pathY.toFixed(2)}px`);
    ui.world.style.setProperty('--flight-path-scale',pathScale.toFixed(3));
    if(ui.moonBeacon)ui.moonBeacon.style.setProperty('--moon-pulse',(0.18+progress*0.82).toFixed(3));

    let mood='ready',icon='🙂',word='READY';
    if(game.mode==='searching'){mood='scan';icon='🛰️';word='SCAN';}
    else if(game.mode==='complete'){
      if(ui.game.dataset.outcome==='secure'){mood='secure';icon='🏁';word='CASH';}
      else if(ui.game.dataset.outcome==='crash'){mood='crash';icon='💥';word='DROP';}
      else {mood='ready';icon='🙂';word='DONE';}
    } else if(game.mode==='live' || game.mode==='settling'){
      if(danger==='high'){mood='danger';icon='😵';word='DANGER';}
      else if(direction==='down' && velocity < -0.012){mood='dip';icon='😟';word='DIP';}
      else if(stage==='moon'||stage==='deep'||stage==='hyper'){mood='moon';icon='🌕';word='MOON';}
      else if(direction==='up' || velocity > 0.012){mood='boost';icon='😄';word='PUMP';}
      else {mood='cruise';icon='😎';word='FLY';}
    }
    ui.rocket.dataset.mood=mood;
    if(ui.pepeEmoteIcon) setText(ui.pepeEmoteIcon, icon);
    if(ui.pepeEmoteText) setText(ui.pepeEmoteText, word);

    let flameHue='0deg', flameBright='1', flameSat='1';
    if(game.mode==='searching'){flameHue='170deg';flameBright='1.05';flameSat='0.8';}
    else if(danger==='high'){flameHue='-18deg';flameBright='1.05';flameSat='1.2';}
    else if(direction==='down'){flameHue='-6deg';flameBright='.95';flameSat='.9';}
    else if(stage==='moon'||stage==='deep'||stage==='hyper'){flameHue='88deg';flameBright='1.18';flameSat='1.3';}
    else if(direction==='up'){flameHue='78deg';flameBright='1.14';flameSat='1.2';}
    ui.world.style.setProperty('--flame-hue', flameHue);
    ui.world.style.setProperty('--flame-bright', flameBright);
    ui.world.style.setProperty('--flame-sat', flameSat);
  }
  ...
  function apply(payload,{allowResult=true}={}){
    ...
    if(payload.selector){const d=payload.selector;game.selectorDiag=d;if(d.selectedMint){ui.selectorText.textContent=`MEMEFLOW BUY READY ${d.buyReady||0} · usable ${d.eligible||0} · selected score ${num(d.selectedScore)===null?'—':Math.round(num(d.selectedScore))}`;if(ui.preflightText)setText(ui.preflightText,'BUY READY target found · locking the server PAPER entry.');}else{ui.selectorText.textContent=`MEMEFLOW BUY READY ${d.buyReady||0} · usable ${d.eligible||0} · missing token ${d.noToken||0} · missing price ${d.noPrice||0}`;if(game.mode==='searching'&&ui.preflightText)setText(ui.preflightText,d.buyReady?'BUY READY exists · waiting for a valid market price.':'Waiting for MEMEFLOW BUY READY from your saved settings.');}}
  ...
  function createFx(){
    try{
      const director=globalThis.PepeSceneDirector?.create?.({root:ui.game,world:ui.world,canvas:ui.fx,rocket:ui.rocket,reducedMotion:()=>reducedMotion});
      if(director)return director;
    }catch(error){console.warn('PepeSceneDirector unavailable, using safe legacy FX',error);}
    return createLegacyFx();
  }
