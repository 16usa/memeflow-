...
echo "[MEMEFLOW] Per-mint 1s card truth + market-cap repair v18"
...
TMP="$(mktemp -d /tmp/memeflow-v18-XXXXXX)"
...
cleanup() {
  ...
  if [[ $code -ne 0 ]]; then
    echo
    echo "[FAILED] v18 made no commit/push."
...
assert.equal(traded.priceChange5mPct,25);
...
assert.doesNotMatch(mutablePatch,/\.token-name/);
assert.doesNotMatch(mutablePatch,/\.token-avatar/);
assert.doesNotMatch(mutablePatch,/\.token-pump-link/);
