...
PATCH_NAME="MEMEFLOW_ANTI_RUG_V1_4_2_3_EXACT"
RUNTIME_MARK="MEMEFLOW_ANTI_RUG_V1_4_2_EXACT"
...
if grep -q "$RUNTIME_MARK" src/evaluate.mjs; then die "V1.4.2 anti-rug runtime patch is already applied."; fi
...
log "V1.4.2.3 anti-rug regression tests..."
node --test "$NEW_TEST"
...
grep -q "$RUNTIME_MARK" src/evaluate.mjs || die "Post-patch runtime anti-rug marker is missing."
grep -q "rugRiskVersion:'V1.4.2'" src/store.mjs || die "Post-patch anti-rug persistence marker is missing."
grep -q "Rapid drawdown safety" src/evaluate.mjs || die "Post-patch rapid drawdown gate is missing."
grep -q "Selloff recovery hold" src/evaluate.mjs || die "Post-patch recovery hold is missing."
grep -Fq "antiRug:{" app-server.mjs || die "Post-patch candidate anti-rug diagnostics are missing."
