...
FILES=("$EVAL" "$APP" "$TOKENS" "$UNIFIED" "$STATE_TEST")

echo "=== MEMEFLOW CANONICAL NULL SCORE V21.7.1 ==="

[[ "$(git branch --show-current)" == "$BRANCH" ]] || {
  echo "V21.7.1 REFUSED: expected branch $BRANCH"
  echo "actual: $(git branch --show-current)"
  exit 1
}

[[ "$(git rev-parse HEAD)" == "$EXPECTED_HEAD" ]] || {
  echo "V21.7.1 REFUSED: audited HEAD changed"
...
for f in "${FILES[@]}"; do
  [[ -f "$f" ]] || { echo "V21.7.1 REFUSED: missing $f"; exit 1; }
  git diff --quiet -- "$f" || { echo "V21.7.1 REFUSED: local changes in $f"; exit 1; }
  git diff --cached --quiet -- "$f" || { echo "V21.7.1 REFUSED: staged changes in $f"; exit 1; }
done

[[ ! -e "$NEW_TEST" ]] || {
  echo "V21.7.1 REFUSED: $NEW_TEST already exists"
  exit 1
}
...
for file,markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(f"V21.7.1 REFUSED: audited marker missing in {file}: {marker}")

print("AUDITED_V21_7_INPUT_OK")
PY

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP=".memeflow-backups/canonical-null-score-v21-7-1-$STAMP"
mkdir -p "$BACKUP"
...
def once(text,old,new,label):
    ...
    if count!=1:
        raise SystemExit(f"V21.7.1 REFUSED: {label}: expected 1 exact match, got {count}")
...
echo
echo "=== V21.7.1 SYNTAX ==="
for f in "$EVAL" "$APP" "$TOKENS" "$UNIFIED" "$STATE_TEST" "$NEW_TEST"; do
  node --check "$f"
done
echo "SYNTAX_OK"

echo
echo "=== V21.7.1 TARGETED TESTS ==="
(
  cd memeflow-app
...
echo
echo "=== V21.7.1 FULL PROJECT TEST SUITE ==="
(
  cd memeflow-app
  npm test
)
...
echo
echo "=== V21.7.1 STATIC CONTRACT AUDIT ==="
python3 - <<'PY'
from pathlib import Path
...
ALLOWED_RE='^memeflow-app/(app-server\.mjs|system-tokens\.js|src/evaluate\.mjs|tests/unified-analysis-engine-v21\.mjs|tests/canonical-score-state-v20_7\.mjs|tests/canonical-null-score-v21_7\.mjs)$'
