...
const MARK = 'MEMEFLOW_CANONICAL_TOKEN_IMAGE_V3';
...
if (
  ...
  console.log('[IMAGE-V3] Already installed.');
  process.exit(0);
}

console.log('[IMAGE-V3] Preflight: target files loaded.');
console.log('[IMAGE-V3] index getImage signature:', /async\s+function\s+getImage\s*\(\s*mint\s*\)\s*\{/.test(index) ? 'FOUND' : 'MISSING');

const stamp = new Date().toISOString().replace(/[:.]/g, '-');
const backupDir = path.join(ROOT, '.patch-backups', 'canonical-token-image-v3-' + stamp);
fs.mkdirSync(backupDir, { recursive: true });

for (const p of Object.values(paths)) {
  fs.copyFileSync(p, path.join(backupDir, path.basename(p)));
}
console.log('[IMAGE-V3] Backup:', path.relative(ROOT, backupDir));
...
{
  // V3: find getImage(mint) by syntax, then replace the function by brace matching.
  // This is resilient to whitespace/comments/formatting differences in Replit.
  const signature = /async\s+function\s+getImage\s*\(\s*mint\s*\)\s*\{/g;
  const match = signature.exec(index);
  ...
  const braceStart = index.indexOf('{', start);
  ...
  if (end < 0) {
    fail('Could not find end of dashboard getImage(mint) function');
  }
...
const rollbackPath = path.join(ROOT, 'rollback-canonical-token-image-v3.mjs');
...
console.log('\n[IMAGE-V3] Verification: PASS');
console.log('[IMAGE-V3] app-server.mjs syntax: PASS');
console.log('[IMAGE-V3] trading.js syntax: PASS');
console.log('[IMAGE-V3] system-tokens.js syntax: PASS');
console.log('[IMAGE-V3] Installed successfully.');
console.log('[IMAGE-V3] Old frontend image-source chains replaced in target UI paths.');
console.log('[IMAGE-V3] Canonical UI source: /api/token-image/<mint>');
console.log('[IMAGE-V3] Upstream source: Pump CreateEvent metadata URI');
console.log('[IMAGE-V3] Rollback: node rollback-canonical-token-image-v3.mjs');
