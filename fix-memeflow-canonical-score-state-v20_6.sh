...
TEST="memeflow-app/tests/canonical-score-state-v20_6.mjs"

[[ -f "$APP" ]] || { echo "ERROR: missing $APP"; exit 1; }

echo "=== MEMEFLOW CANONICAL SCORE/STATE V20.6 ==="
...
for marker in required:
  if marker not in app:
    raise SystemExit("V20.6 REFUSED: current marker missing: "+marker)

if "MEMEFLOW_CANONICAL_SCORE_STATE_V20_6" in app:
  raise SystemExit("V20.6 REFUSED: patch already installed")

print("CURRENT_ARCHITECTURE_OK")
PY

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP=".memeflow-backups/canonical-score-state-v20_6-$STAMP"
...
new = """  // MEMEFLOW_CANONICAL_SCORE_STATE_V20_6
...
if old not in app:
  raise SystemExit("V20.6 REFUSED: score-zeroing live gate block not found")
...
if start<0 or end<=start:
  raise SystemExit("V20.6 REFUSED: V28 display classifier boundaries not found")
...
for marker in (
  ...
  if marker not in block:
    raise SystemExit("V20.6 REFUSED: V28 classifier shape changed: "+marker)

replacement = """function __mfLiveDisplayStateV28(view,settings={}){
  // MEMEFLOW_CANONICAL_SCORE_STATE_V20_6
...
assert.match(app,/MEMEFLOW_CANONICAL_SCORE_STATE_V20_6/);
...
assert.doesNotMatch(watchBlock,/relevanceScore/);
assert.doesNotMatch(watchBlock,/feedScore/);

console.log('CANONICAL_SCORE_STATE_V20_6_OK');
TESTJS
...
(
  cd memeflow-app
  node tests/canonical-score-state-v20_6.mjs
...
BAD="$(
  ...
  grep -Ev '^memeflow-app/(app-server\.mjs|tests/canonical-score-state-v20_6\.mjs)$' ||
  true
)"
