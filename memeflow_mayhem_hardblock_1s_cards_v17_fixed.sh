...
echo "[MEMEFLOW] Mayhem hard block + 1-second mutable cards v17.1-fixed"
...
cleanup() {
  ...
  if [[ $code -ne 0 ]]; then
    echo
    echo "[FAILED] v17.1 made no commit/push."
...
if git merge-base --is-ancestor "$LOCAL_HEAD" "$NEW_SHA" 2>/dev/null; then
  ...
  if git merge --ff-only "$NEW_SHA"; then
    echo "[local] workspace fast-forwarded to verified v17.1.1"
  else
