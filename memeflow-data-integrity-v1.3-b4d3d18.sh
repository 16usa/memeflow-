...
PATCH_NAME="MEMEFLOW_DATA_INTEGRITY_V1_3"
...
grep -q "holderSource:'event-ledger-v12-27-user-only'" src/event-holder-ledger.mjs || die "Expected V12.27 event-holder snapshot anchor missing."
...
end_marker="  inspect(m){return this.snapshot(m)}"
start=h.find(start_marker)
end=h.find(end_marker,start)
if start<0 or end<0 or h.find(start_marker,start+1)>=0 or h.find(end_marker,end+1)>=0:
    raise SystemExit('event-holder snapshot/apply anchor is missing or ambiguous')
end+=len(end_marker)
...
replacement=r'''  // MEMEFLOW_DATA_INTEGRITY_V1_3
  ...
  applyToStore(store,m){
    ...
    try{
      ...
      if(String(token?.holderSource||'')==='event-ledger-v12-27-user-only'){
...
repl="""      this.state={...this.state,...d};
      ...
      for(const token of Object.values(this.state.tokens||{})){
        if(String(token?.holderSource||'')!=='event-ledger-v12-27-user-only')continue;
...
function __mfV13SanitizeEvidence(token={}){
  if(String(token?.holderSource||'')!=='event-ledger-v12-27-user-only')return token;
  return {
    ...token,
