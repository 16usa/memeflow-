...
if(current.js.includes("const CLIENT_VERSION='5.6.0';")&&current.html.includes('/game.js?v=56')&&current.html.includes('/game.css?v=56')){
  console.log('Pepe Rocket V5.6 is already installed. Run the verifier.');
  process.exit(0);
}
if(!current.js.includes("const CLIENT_VERSION='5.5.0';")) throw new Error('V5.6 updater expects the current Pepe Rocket V5.5 client. No files were changed.');
if(!current.engine.includes("const GAME_VERSION = '5.4.0';")) throw new Error('V5.6 keeps the existing V5.4 Game Engine. Expected engine 5.4.0; no files were changed.');
...
if((next.html.match(/\/game\.js/g)||[]).length!==1||(next.html.match(/\/game\.css/g)||[]).length!==1) throw new Error('V5.6 preflight failed: duplicate linked Game files.');
if((next.css.match(/{/g)||[]).length!==(next.css.match(/}/g)||[]).length) throw new Error('V5.6 preflight failed: CSS block imbalance.');
const ids=[...next.html.matchAll(/id="([^"]+)"/g)].map(m=>m[1]);
if(new Set(ids).size!==ids.length) throw new Error('V5.6 preflight failed: duplicate HTML ids.');
const refs=new Set([...next.js.matchAll(/\$\('#([^']+)'\)/g)].map(m=>m[1])),idSet=new Set(ids);
for(const id of refs) if(!idSet.has(id)) throw new Error(`V5.6 preflight failed: game.js references missing #${id}.`);
for(const marker of ['targetReticle','selectorPhase','updateSearchRadar','lockSelector','game.acceleration','game.visualTilt']) if(!next.js.includes(marker)&&!next.html.includes(`id="${marker}"`)) throw new Error(`V5.6 preflight failed: ${marker} missing.`);

const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const backupDir=path.join(app,'.memeflow-patches','pepe-game-v56',stamp);
...
fs.writeFileSync(path.join(backupDir,'manifest.json'),JSON.stringify({
  version:'5.6.0',createdAt:new Date().toISOString(),files:['game.html','game.css','game.js'],
  before:{html:sha(live.html),css:sha(live.css),js:sha(live.js)},
  engineUntouched:sha(live.engine),serverUntouched:sha(live.server)
},null,2));

function atomicWrite(content,dst){const tmp=dst+'.v56tmp';try{fs.unlinkSync(tmp)}catch{}fs.writeFileSync(tmp,content);fs.renameSync(tmp,dst);}
...
console.log('Pepe Rocket V5.6 target-acquisition in-place update installed.');
