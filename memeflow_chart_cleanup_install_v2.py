...
""",
    ...
    # V108 is a later mobile Chart-header generation in this same file.
    # It duplicates/redefines the exact header geometry now canonically owned
    # by chart-header-geometry-v97.css, so remove the entire obsolete block.
    replace_between(
        v67,
        "/* ==========================================================================\n   MEMEFLOW_TRADING_HEADER_BASELINE_V108",
        "/* ==========================================================================\n   MEMEFLOW_TRADING_DARK_SURFACE_BRIDGE_V109",
        "",
        "remove obsolete V108 Chart-header geometry",
    )
    ...
    replace_link_version(html, "trading.css", "chart-canonical-cleanup-v2-20260909")
    replace_link_version(html, "trading-light-polish-v61.css", "chart-cleanup-v2-20260909")
    replace_link_version(html, "trading-visual-hierarchy-v67.css", "chart-cleanup-v2-20260909")
    replace_link_version(html, "memeflow-trading-white-surfaces-v154.css", "chart-cleanup-v2-20260909")
    ...
    v67_text = read(v67)
    forbidden_v67_header_geometry = (
        ".chart-head .price-toggle",
        ".chart-head .token-title",
        ".chart-head .token-avatar",
        ".chart-head .token-name-row",
        ".chart-head .token-meta",
        ".chart-head #tokenMint",
        ".chart-head #copyMintBtn",
        ".chart-head .token-price",
        ".chart-head .token-market",
    )
    for token in forbidden_v67_header_geometry:
        if token in v67_text:
            raise RuntimeError(f"V67 still overrides Chart-header geometry: {token}")
    if "MEMEFLOW_TRADING_HEADER_BASELINE_V108" in v67_text:
        raise RuntimeError("obsolete V108 Chart-header block still remains")
...
""",
    ...
    print("")
    print("✅ CHART CLEANUP V2 INSTALLED SUCCESSFULLY")
